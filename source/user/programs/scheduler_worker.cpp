...
constexpr char OS_USER_SCHEDULER_PID2_STEP1_MESSAGE[] =
    "[OS][USER][PID2] WORKER_STEP_1\r\n";
constexpr char OS_USER_SCHEDULER_PID2_STEP2_MESSAGE[] =
    "[OS][USER][PID2] WORKER_STEP_2\r\n";
constexpr char OS_USER_SCHEDULER_PID2_STEP3_MESSAGE[] =
    "[OS][USER][PID2] WORKER_STEP_3\r\n";
constexpr char OS_USER_SCHEDULER_PID3_STEP1_MESSAGE[] =
    "[OS][USER][PID3] WORKER_STEP_1\r\n";
constexpr char OS_USER_SCHEDULER_PID3_STEP2_MESSAGE[] =
    "[OS][USER][PID3] WORKER_STEP_2\r\n";
constexpr char OS_USER_SCHEDULER_PID3_STEP3_MESSAGE[] =
    "[OS][USER][PID3] WORKER_STEP_3\r\n";
constexpr char OS_USER_SCHEDULER_PID4_STEP1_MESSAGE[] =
    "[OS][USER][PID4] WORKER_STEP_1\r\n";
constexpr char OS_USER_SCHEDULER_PID4_STEP2_MESSAGE[] =
    "[OS][USER][PID4] WORKER_STEP_2\r\n";
constexpr char OS_USER_SCHEDULER_PID4_STEP3_MESSAGE[] =
...
volatile uint64_t workerCounter;
...
[[nodiscard]] bool WriteMessage(const char (&message)[MessageSizeBytes]) noexcept {
    return os::user::WriteLog(
               message, MessageSizeBytes - OS_USER_SCHEDULER_STRING_TERMINATOR_SIZE_BYTES) >
           OS_USER_SCHEDULER_FIRST_ERROR_RESULT;
}

[[nodiscard]] bool WriteProgressMessage(const uint64_t processId,
                                        const uint64_t round) noexcept {
    if (processId == OS_USER_SCHEDULER_FIRST_WORKER_PROCESS_ID) {
    ...
    if (processId == OS_USER_SCHEDULER_SECOND_WORKER_PROCESS_ID) {
        if (round == OS_USER_SCHEDULER_FIRST_ROUND) {
    ...
    if (processId == OS_USER_SCHEDULER_THIRD_WORKER_PROCESS_ID) {
        if (round == OS_USER_SCHEDULER_FIRST_ROUND) {
...
[[nodiscard]] bool RunWorkRound(const uint64_t round) noexcept {
    for (uint64_t iteration = 0ULL;
         iteration < OS_USER_SCHEDULER_ITERATIONS_PER_ROUND; ++iteration) {
        workerCounter = workerCounter + OS_USER_SCHEDULER_COUNTER_INCREMENT;
    }
    return workerCounter == round * OS_USER_SCHEDULER_ITERATIONS_PER_ROUND;
}

}

extern "C" [[noreturn, gnu::section(".text.os_user_entry")]] void osUserEntry() noexcept {
    const uint64_t processId = os::user::GetProcessId();
    for (uint64_t round = OS_USER_SCHEDULER_FIRST_ROUND;
         round <= OS_USER_SCHEDULER_ROUND_COUNT; ++round) {
        if (!RunWorkRound(round) || !WriteProgressMessage(processId, round)) {
