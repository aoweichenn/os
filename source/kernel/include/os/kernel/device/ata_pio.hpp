...
class AtaPioDevice final : public FileSystemBlockDevice {
  public:
    ...
    InitializeAsynchronousRequests(BlockRequest *request_storage,
                                   uint64_t request_capacity) noexcept;
    [[nodiscard]] AtaPioStatus
    SubmitAsynchronous(BlockOperation operation, uint64_t logical_block_address,
    ...
    [[nodiscard]] AtaPioStatus
    StartNextAsynchronous(AtaPioCompletion &completion, bool &request_started) noexcept;
    [[nodiscard]] AtaPioStatus
    HandleInterrupt(AtaPioCompletion &completion) noexcept;
    [[nodiscard]] AtaPioStatus
    ResolveTimeout(uint64_t now_nanoseconds, AtaPioCompletion &completion) noexcept;
  ...
  private:
    ...
    [[nodiscard]] AtaPioStatus
    PrepareAsynchronousRequest(const BlockRequest &request) noexcept;
    [[nodiscard]] AtaPioStatus
    ResolveIssuedRequest(BlockRequestResult result,
