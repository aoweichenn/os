...
TerminalStatus Terminal::TryRead(uint8_t *const destination, const uint64_t capacity_bytes,
    ...
    const uint64_t transferable_bytes =
        capacity_bytes < this->statistics_.buffered_byte_count
            ? capacity_bytes
    ...
    while (read_bytes < transferable_bytes) {
        ...
        this->input_read_index_ =
            (this->input_read_index_ + OS_KERNEL_TERMINAL_COUNTER_INCREMENT) %
...
TerminalStatus Terminal::TryWrite(
    const uint8_t *const source, const uint64_t length_bytes,
    const TerminalDeviceWriteOperation device_write_operation, void *const device_write_context,
    ...
    while (written_bytes < length_bytes) {
        ...
        const uint64_t free_bytes = OS_KERNEL_TERMINAL_OUTPUT_CAPACITY_BYTES -
                                    this->statistics_.output_pending_byte_count;
        const uint64_t remaining_bytes = length_bytes - written_bytes;
        const uint64_t queue_bytes =
            remaining_bytes < free_bytes ? remaining_bytes : free_bytes;
...
TerminalStatus Terminal::AcquireControllingSession(
    const uint64_t session_id, const uint64_t session_leader_process_id,
    const uint64_t foreground_process_group_id) noexcept {
    IrqSaveSpinLockGuard guard{this->lock_};
    if (session_id == OS_KERNEL_TERMINAL_EMPTY_VALUE ||
        session_leader_process_id != session_id ||
...
TerminalStatus Terminal::SetForegroundProcessGroup(
    const uint64_t caller_session_id,
    const uint64_t foreground_process_group_id) noexcept {
    IrqSaveSpinLockGuard guard{this->lock_};
...
bool Terminal::CanRead(const uint64_t session_id, const uint64_t process_group_id) noexcept {
    IrqSaveSpinLockGuard guard{this->lock_};
    const bool can_read =
        session_id != OS_KERNEL_TERMINAL_EMPTY_VALUE &&
        session_id == this->controlling_session_id_ &&
        process_group_id == this->foreground_process_group_id_;
    if (!can_read) {
        this->statistics_.rejected_background_read_count +=
            OS_KERNEL_TERMINAL_COUNTER_INCREMENT;
    }
    return can_read;
}
...
TerminalStatus Terminal::Validate() const noexcept {
    ...
    if (this->terminal_id_ == OS_KERNEL_TERMINAL_EMPTY_VALUE ||
        ...
        this->statistics_.read_byte_count + this->statistics_.buffered_byte_count +
                this->statistics_.editing_byte_count +
                this->statistics_.dropped_byte_count +
                this->statistics_.consumed_byte_count !=
            this->statistics_.submitted_byte_count ||
        this->statistics_.output_written_byte_count +
                this->statistics_.output_pending_byte_count !=
...
bool Terminal::CommitEditedBytes(const bool append_newline) noexcept {
    const uint64_t required_bytes =
        this->edit_length_bytes_ +
        (append_newline ? OS_KERNEL_TERMINAL_COUNTER_INCREMENT
                        : OS_KERNEL_TERMINAL_EMPTY_VALUE);
    if (required_bytes >
...
TerminalStatus Terminal::DrainOutput(
    ...
    while (this->statistics_.output_pending_byte_count != OS_KERNEL_TERMINAL_EMPTY_VALUE) {
        ...
        this->output_read_index_ =
            (this->output_read_index_ + device_written_bytes) %
            OS_KERNEL_TERMINAL_OUTPUT_CAPACITY_BYTES;
