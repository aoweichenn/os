...
OS_QEMU_MINIMUM_GUEST_MEMORY_MEBIBYTES = OS_QEMU_PRIMARY_GUEST_MEMORY_MEBIBYTES
OS_QEMU_FUNCTIONAL_GUEST_MEMORY_MEBIBYTES = OS_QEMU_PRIMARY_GUEST_MEMORY_MEBIBYTES
...
OS_QEMU_KERNEL_FILE_CACHE_DYNAMIC_READY_MARKER = "[OS][KERNEL] FILE_CACHE_DYNAMIC_READY"
OS_QEMU_KERNEL_FILE_CACHE_CAPACITY_MARKER = "[OS][KERNEL] FILE_CACHE_CAPACITY=0x"
OS_QEMU_KERNEL_FILE_CACHE_METADATA_SIZE_MARKER = (
    "[OS][KERNEL] FILE_CACHE_METADATA_BYTES=0x"
)
...
OS_QEMU_KERNEL_READAHEAD_FAILURE_MARKER = "[OS][KERNEL][READAHEAD][FAIL]"
...
OS_QEMU_KERNEL_FILE_CACHE_RECLAIMED_MARKER = "[OS][KERNEL] FILE_CACHE_RECLAIMED"
...
OS_QEMU_KERNEL_THREAD_PEAK_ACTIVE_THREE_MARKER = (
    "[OS][KERNEL] KERNEL_THREAD_PEAK_ACTIVE=0x0000000000000003"
)
OS_QEMU_KERNEL_THREAD_CREATIONS_SIX_MARKER = (
    "[OS][KERNEL] KERNEL_THREAD_CREATIONS=0x0000000000000006"
)
...
OS_QEMU_KERNEL_THREAD_EXITS_SIX_MARKER = (
    "[OS][KERNEL] KERNEL_THREAD_EXITS=0x0000000000000006"
)
...
OS_QEMU_PAGE_AGING_PROMOTIONS_MARKER = "[OS][KERNEL][AGING] PROMOTIONS=0x"
OS_QEMU_PAGE_AGING_DEMOTIONS_MARKER = "[OS][KERNEL][AGING] DEMOTIONS=0x"
OS_QEMU_PAGE_AGING_CANDIDATES_MARKER = (
    "[OS][KERNEL][AGING] CANDIDATE_OBSERVATIONS=0x"
)
OS_QEMU_PAGE_AGING_FAILURE_MARKER = "[OS][KERNEL][AGING][FAIL]"
...
OS_QEMU_BACKGROUND_RECLAIM_ANONYMOUS_ZERO_MARKER = (
    "[OS][KERNEL][RECLAIM] BACKGROUND_ANONYMOUS_PAGES=0x0000000000000000"
)
...
OS_QEMU_KERNEL_MEMORY_RECLAIM_PRESSURE_LIMIT_MARKER = (
    "[OS][KERNEL] MEMORY_PRESSURE_LIMIT_PAGES=0x0000000000002400"
)
OS_QEMU_KERNEL_MEMORY_OOM_PRESSURE_LIMIT_MARKER = (
    "[OS][KERNEL] MEMORY_PRESSURE_LIMIT_PAGES=0x0000000000003000"
)
...
OS_QEMU_KERNEL_MEMORY_RECLAIM_SWAPPED_ANONYMOUS_ZERO_MARKER = (
    "[OS][KERNEL] MEMORY_RECLAIM_SWAPPED_ANONYMOUS_PAGES=0x0000000000000000"
)
...
OS_QEMU_KERNEL_MEMORY_RECLAIM_NO_PROGRESS_COUNT_MARKER = (
    "[OS][KERNEL] MEMORY_RECLAIM_NO_PROGRESS=0x"
)
...
OS_QEMU_KERNEL_MEMORY_SWAPPINESS_ZERO_MARKER = (
    "[OS][KERNEL] MEMORY_SWAPPINESS=0x0000000000000000"
)
OS_QEMU_KERNEL_OOM_INVOCATION_COUNT_MARKER = "[OS][KERNEL] OOM_INVOCATIONS=0x"
OS_QEMU_KERNEL_OOM_KILL_COUNT_MARKER = "[OS][KERNEL] OOM_KILLS=0x"
OS_QEMU_KERNEL_OOM_LAST_VICTIM_MARKER = "[OS][KERNEL] OOM_LAST_VICTIM=0x"
...
OS_QEMU_USER_TOOL_FAILURE_MARKER = "[OS][USER][TOOLS][FAIL]"
...
OS_QEMU_USER_OOM_STARTED_MARKER = "[OS][USER][OOM] STARTED"
OS_QEMU_USER_OOM_VICTIM_READY_MARKER = "[OS][USER][OOM] VICTIM_READY"
OS_QEMU_USER_OOM_VICTIM_KILLED_MARKER = "[OS][USER][OOM] VICTIM_KILLED"
OS_QEMU_USER_OOM_COMPLETED_MARKER = "[OS][USER][OOM] COMPLETED"
OS_QEMU_USER_OOM_FAILURE_MARKER = "[OS][USER][OOM][FAIL]"
OS_QEMU_USER_INIT_OOM_PROBE_REAPED_MARKER = "[OS][USER][INIT] OOM_PROBE_REAPED"
...
def validateVgaProtocol(
    ...
    for marker, minimumCount in minimumMarkerCounts:
        actualCount = vgaOutput.count(marker)
        if actualCount < minimumCount:
            raise OsToolError(
                "VGA标记出现次数未达到下界："
                f"{marker!r} 实际 {actualCount} 次，下界 {minimumCount} 次"
            )
