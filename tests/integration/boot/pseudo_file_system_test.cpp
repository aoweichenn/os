...
constexpr std::string_view OS_TEST_PSEUDO_FS_SUITE_NAME = "kernel/pseudo_file_system/integration";
...
int main() {
    ...
    os::kernel::fs::DevfsDevice devices[OS_TEST_PSEUDO_FS_DEVICE_CAPACITY]{};
    ...
    os::kernel::fs::ProcfsSnapshot snapshot{
        ...
        .resident_limit_bytes = OS_TEST_PSEUDO_FS_MANAGED_MEMORY_BYTES,
        .swap_total_bytes = OS_TEST_PSEUDO_FS_EMPTY_VALUE,
        .swap_free_bytes = OS_TEST_PSEUDO_FS_EMPTY_VALUE,
        .committed_memory_bytes = OS_TEST_PSEUDO_FS_EMPTY_VALUE,
        .commit_limit_bytes = OS_TEST_PSEUDO_FS_MANAGED_MEMORY_BYTES,
        .oom_kill_count = OS_TEST_PSEUDO_FS_EMPTY_VALUE,
        ...
        .current_process_id = OS_TEST_PSEUDO_FS_CURRENT_PROCESS_IDENTIFIER,
        .heap_consumed_bytes = OS_TEST_PSEUDO_FS_HEAP_CONSUMED_BYTES,
        .active_file_description_count = OS_TEST_PSEUDO_FS_ACTIVE_FILE_DESCRIPTION_COUNT,
    ...
    const bool initialized =
        ...
        memfs.Initialize(
            heap, OS_TEST_PSEUDO_FS_ROOT_SUPERBLOCK_IDENTIFIER, OS_TEST_PSEUDO_FS_ROOT_NODE_LIMIT,
            OS_TEST_PSEUDO_FS_MAXIMUM_FILE_SIZE_BYTES) == os::kernel::fs::Status::Succeeded &&
        devfs.Initialize(OS_TEST_PSEUDO_FS_DEV_SUPERBLOCK_IDENTIFIER, devices,
                         OS_TEST_PSEUDO_FS_DEVICE_CAPACITY) == os::kernel::fs::Status::Succeeded &&
        devfs.RegisterCharacterDevice(
            OS_TEST_PSEUDO_FS_CONSOLE_NAME, sizeof(OS_TEST_PSEUDO_FS_CONSOLE_NAME),
            console_node_identifier) == os::kernel::fs::Status::Succeeded &&
        procfs.Initialize(OS_TEST_PSEUDO_FS_PROC_SUPERBLOCK_IDENTIFIER, CaptureSnapshot,
                          &snapshot) == os::kernel::fs::Status::Succeeded &&
        vfs.Initialize(mounts, OS_TEST_PSEUDO_FS_MOUNT_CAPACITY, memfs.GetSuperblock()) ==
        ...
        vfs.CreateDirectory(context, OS_TEST_PSEUDO_FS_PROC_PATH,
            ...
            os::kernel::fs::Status::Succeeded &&
        vfs.MountAt(context, OS_TEST_PSEUDO_FS_DEV_PATH, sizeof(OS_TEST_PSEUDO_FS_DEV_PATH),
                    devfs.GetSuperblock()) == os::kernel::fs::Status::Succeeded &&
        vfs.MountAt(context, OS_TEST_PSEUDO_FS_PROC_PATH, sizeof(OS_TEST_PSEUDO_FS_PROC_PATH),
    ...
    const bool mount_contract_valid =
        initialized &&
        vfs.Resolve(context, OS_TEST_PSEUDO_FS_CONSOLE_PATH, sizeof(OS_TEST_PSEUDO_FS_CONSOLE_PATH),
        ...
        vfs.Resolve(context, OS_TEST_PSEUDO_FS_VERSION_PATH, sizeof(OS_TEST_PSEUDO_FS_VERSION_PATH),
                    version_path) == os::kernel::fs::Status::Succeeded &&
        version_path.vnode.type == os::kernel::fs::NodeType::RegularFile &&
        vfs.ReadStatistics().mount_count == OS_TEST_PSEUDO_FS_EXPECTED_MOUNT_COUNT &&
        vfs.ReadStatistics().mount_transition_count >=
            OS_TEST_PSEUDO_FS_MINIMUM_MOUNT_TRANSITION_COUNT;
    test_context.Expect(mount_contract_valid, OS_TEST_PSEUDO_FS_MOUNT_CONTRACT);
    ...
    const bool read_only_contract_valid =
        ...
        !forbidden_file.open;
    test_context.Expect(read_only_contract_valid, OS_TEST_PSEUDO_FS_READ_ONLY_CONTRACT);
    ...
    const bool resource_contract_valid =
        ...
        usage.vnode_count >= os::kernel::fs::OS_KERNEL_PROCFS_FILE_COUNT +
    ...
    test_context.Expect(resource_contract_valid, OS_TEST_PSEUDO_FS_RESOURCE_CONTRACT);
