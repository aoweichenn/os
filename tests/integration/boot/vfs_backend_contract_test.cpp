...
constexpr std::string_view OS_TEST_VFS_BACKEND_RESOURCES =
    "memfs 销毁后必须归还全部节点与数据分配";
...
constexpr uint64_t OS_TEST_VFS_BACKEND_FILE_SIZE_LIMIT =
    sizeof(OS_TEST_VFS_BACKEND_PAYLOAD);
...
[[nodiscard]] bool RunBackendContract(os::kernel::fs::Vfs &vfs,
    ...
    if (vfs.Open(context, OS_TEST_VFS_BACKEND_FILE_PATH,
                 sizeof(OS_TEST_VFS_BACKEND_FILE_PATH), write_options,
                 open_file) != os::kernel::fs::Status::Succeeded ||
        vfs.Write(open_file, OS_TEST_VFS_BACKEND_PAYLOAD,
                  sizeof(OS_TEST_VFS_BACKEND_PAYLOAD), transferred_bytes) !=
            os::kernel::fs::Status::Succeeded ||
    ...
    if (vfs.OpenDirectory(context, OS_TEST_VFS_BACKEND_CURRENT_PATH,
                          sizeof(OS_TEST_VFS_BACKEND_CURRENT_PATH), open_file) !=
            os::kernel::fs::Status::Succeeded ||
        ...
        !BytesEqual(entry.name, OS_TEST_VFS_BACKEND_EXPECTED_FILE_NAME,
...
int main() {
    ...
    const bool memfs_ready =
        ...
        memfs.Initialize(heap, OS_TEST_VFS_BACKEND_MEMFS_IDENTIFIER,
                         OS_TEST_VFS_BACKEND_NODE_LIMIT,
    ...
    const bool memfs_contract_valid =
        memfs_ready && RunBackendContract(memfs_vfs, memfs_context);
    ...
    test_context.Expect(memfs_contract_valid && legacy_contract_valid,
                        OS_TEST_VFS_BACKEND_PARITY);
    ...
    const bool persistence_valid =
        ...
        remounted_vfs.Initialize(remounted_mounts, OS_TEST_VFS_BACKEND_MOUNT_CAPACITY,
            ...
            os::kernel::fs::Status::Succeeded &&
        remounted_vfs.InitializeContext(remounted_context) ==
            os::kernel::fs::Status::Succeeded &&
