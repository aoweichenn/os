...
constexpr std::string_view OS_TEST_THREAD_SCHEDULER_SUITE_NAME =
    "kernel/thread_scheduler/unit";
...
[[nodiscard]] bool CreateTestProcess(os::kernel::ThreadScheduler &scheduler,
                                     const uint64_t process_ordinal,
                                     uint64_t &process_index,
                                     os::kernel::ProcessId &process_id) noexcept {
    return scheduler.CreateProcess(TestAddressSpaceRoot(process_ordinal),
...
[[nodiscard]] bool CreateTestThread(os::kernel::ThreadScheduler &scheduler,
                                    ...
                                    const uint64_t thread_ordinal,
                                    uint64_t &thread_index,
                                    os::kernel::ThreadId &thread_id) noexcept {
    return scheduler.CreateThread(
...
[[nodiscard]] bool InitializeTestScheduler(
    ...
    os::kernel::ProcessEntry (&processes)
        [OS_TEST_THREAD_SCHEDULER_TEST_PROCESS_CAPACITY],
    os::kernel::ThreadEntry (&threads)
        [OS_TEST_THREAD_SCHEDULER_TEST_THREAD_CAPACITY]) noexcept {
    return scheduler.Initialize(
               processes, OS_TEST_THREAD_SCHEDULER_TEST_PROCESS_CAPACITY,
               threads, OS_TEST_THREAD_SCHEDULER_TEST_THREAD_CAPACITY,
...
[[nodiscard]] bool ValidateInitializationBoundaries() noexcept {
    ...
    return scheduler.Start(decision) ==
               os::kernel::ThreadSchedulerStatus::NotInitialized &&
           scheduler.Initialize(
               nullptr, OS_TEST_THREAD_SCHEDULER_TEST_PROCESS_CAPACITY, threads,
               OS_TEST_THREAD_SCHEDULER_TEST_THREAD_CAPACITY,
               OS_TEST_THREAD_SCHEDULER_TEST_THREADS_PER_PROCESS,
               OS_TEST_THREAD_SCHEDULER_QUANTUM_TICKS) ==
           ...
           scheduler.Initialize(
               processes, OS_TEST_THREAD_SCHEDULER_TEST_PROCESS_CAPACITY,
               nullptr, OS_TEST_THREAD_SCHEDULER_TEST_THREAD_CAPACITY,
               OS_TEST_THREAD_SCHEDULER_TEST_THREADS_PER_PROCESS,
               OS_TEST_THREAD_SCHEDULER_QUANTUM_TICKS) ==
               os::kernel::ThreadSchedulerStatus::NullThreadStorage &&
           scheduler.Initialize(
               processes, OS_TEST_THREAD_SCHEDULER_EMPTY_VALUE, threads,
               OS_TEST_THREAD_SCHEDULER_TEST_THREAD_CAPACITY,
               OS_TEST_THREAD_SCHEDULER_TEST_THREADS_PER_PROCESS,
               OS_TEST_THREAD_SCHEDULER_QUANTUM_TICKS) ==
           ...
           scheduler.Initialize(
               processes, OS_TEST_THREAD_SCHEDULER_TEST_PROCESS_CAPACITY,
               threads, OS_TEST_THREAD_SCHEDULER_TEST_PROCESS_CAPACITY -
...
[[nodiscard]] bool ValidateCapacityAndIdentifiers() noexcept {
    os::kernel::ProcessEntry
        processes[os::kernel::OS_KERNEL_PROCESS_CAPACITY_LIMIT]{};
    os::kernel::ThreadEntry
        threads[os::kernel::OS_KERNEL_THREAD_CAPACITY_LIMIT]{};
    os::kernel::ThreadScheduler scheduler{};
    if (scheduler.Initialize(
            processes, os::kernel::OS_KERNEL_PROCESS_CAPACITY_LIMIT, threads,
    ...
    for (uint64_t process_ordinal = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         process_ordinal < os::kernel::OS_KERNEL_PROCESS_CAPACITY_LIMIT;
         ++process_ordinal) {
        uint64_t process_index = os::kernel::OS_KERNEL_PROCESS_INVALID_INDEX;
        if (!CreateTestProcess(scheduler, process_ordinal, process_index,
                               last_process_id) ||
            process_index != process_ordinal ||
            last_process_id.value !=
                process_ordinal + OS_TEST_THREAD_SCHEDULER_FIRST_IDENTIFIER) {
            return false;
        }
    }
    uint64_t overflow_process_index = os::kernel::OS_KERNEL_PROCESS_INVALID_INDEX;
    os::kernel::ProcessId overflow_process_id{};
    if (scheduler.CreateProcess(
            TestAddressSpaceRoot(os::kernel::OS_KERNEL_PROCESS_CAPACITY_LIMIT),
    ...
    for (uint64_t thread_ordinal = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         thread_ordinal < os::kernel::OS_KERNEL_THREAD_CAPACITY_LIMIT;
         ++thread_ordinal) {
        const uint64_t process_index =
        ...
        if (!CreateTestThread(scheduler, process_index, thread_ordinal,
                              thread_index, last_thread_id) ||
            thread_index != thread_ordinal ||
            last_thread_id.value !=
                thread_ordinal + OS_TEST_THREAD_SCHEDULER_FIRST_IDENTIFIER) {
            return false;
        }
    }
    ...
    if (scheduler.CreateThread(
            OS_TEST_THREAD_SCHEDULER_FIRST_INDEX,
            os::kernel::OS_KERNEL_THREAD_CAPACITY_LIMIT,
            OS_TEST_THREAD_SCHEDULER_USER_STACK_BASE,
            OS_TEST_THREAD_SCHEDULER_TLS_BASE,
            OS_TEST_THREAD_SCHEDULER_SIGNAL_MASK, overflow_thread_index,
            overflow_thread_id) !=
        os::kernel::ThreadSchedulerStatus::ThreadCapacityExhausted) {
        return false;
    }
    ...
    return scheduler.Validate() ==
           ...
           statistics.owned_process_count ==
               os::kernel::OS_KERNEL_PROCESS_CAPACITY_LIMIT &&
...
[[nodiscard]] bool ValidateInvalidKernelStackRejected() noexcept {
    ...
    if (!InitializeTestScheduler(scheduler, processes, threads) ||
        !CreateTestProcess(scheduler, OS_TEST_THREAD_SCHEDULER_FIRST_INDEX,
                           process_index, process_id)) {
        return false;
    }
...
[[nodiscard]] bool ValidateProcessThreadLimit() noexcept {
    ...
    os::kernel::ThreadEntry
        threads[os::kernel::OS_KERNEL_CAPACITY_THREADS_PER_PROCESS]{};
    os::kernel::ThreadScheduler scheduler{};
    if (scheduler.Initialize(
            &process, OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT, threads,
            os::kernel::OS_KERNEL_CAPACITY_THREADS_PER_PROCESS,
            os::kernel::OS_KERNEL_CAPACITY_THREADS_PER_PROCESS,
            OS_TEST_THREAD_SCHEDULER_QUANTUM_TICKS) !=
        os::kernel::ThreadSchedulerStatus::Succeeded) {
    ...
    uint64_t process_index = os::kernel::OS_KERNEL_PROCESS_INVALID_INDEX;
    os::kernel::ProcessId process_id{};
    if (!CreateTestProcess(scheduler, OS_TEST_THREAD_SCHEDULER_FIRST_INDEX,
                           process_index, process_id)) {
        return false;
    }
    for (uint64_t thread_ordinal = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         thread_ordinal < os::kernel::OS_KERNEL_CAPACITY_THREADS_PER_PROCESS;
         ++thread_ordinal) {
        uint64_t thread_index = os::kernel::OS_KERNEL_THREAD_INVALID_INDEX;
        os::kernel::ThreadId thread_id{};
        if (!CreateTestThread(scheduler, process_index, thread_ordinal,
    ...
    const os::kernel::ThreadSchedulerStatistics before_statistics =
        scheduler.Statistics();
    uint64_t overflow_thread_index = os::kernel::OS_KERNEL_THREAD_INVALID_INDEX;
    os::kernel::ThreadId overflow_thread_id{};
    const os::kernel::ThreadSchedulerStatus overflow_status =
        scheduler.CreateThread(
            process_index,
            os::kernel::OS_KERNEL_CAPACITY_THREADS_PER_PROCESS,
            OS_TEST_THREAD_SCHEDULER_USER_STACK_BASE,
            ...
            OS_TEST_THREAD_SCHEDULER_SIGNAL_MASK, overflow_thread_index,
            overflow_thread_id);
    const os::kernel::ThreadSchedulerStatistics after_statistics =
        scheduler.Statistics();
    return overflow_status ==
               os::kernel::ThreadSchedulerStatus::ProcessThreadLimitReached &&
           before_statistics.owned_thread_count ==
               after_statistics.owned_thread_count &&
           before_statistics.created_thread_count ==
               after_statistics.created_thread_count &&
           scheduler.Validate() ==
               os::kernel::ThreadSchedulerStatus::Succeeded;
}
...
[[nodiscard]] bool ValidateStateLifecycle() noexcept {
    ...
    if (!InitializeTestScheduler(scheduler, processes, threads) ||
        !CreateTestProcess(scheduler, OS_TEST_THREAD_SCHEDULER_FIRST_INDEX,
                           process_index, first_process_id) ||
        !CreateTestThread(scheduler, process_index,
                          OS_TEST_THREAD_SCHEDULER_FIRST_INDEX,
                          first_thread_index, first_thread_id) ||
        !CreateTestThread(scheduler, process_index,
                          OS_TEST_THREAD_SCHEDULER_SECOND_INDEX,
                          second_thread_index, second_thread_id)) {
        return false;
    }

    os::kernel::ThreadSchedulingDecision decision{};
    if (scheduler.Start(decision) !=
            os::kernel::ThreadSchedulerStatus::Succeeded ||
    ...
    if (scheduler.ReadProcess(process_index, zombie_process) !=
        ...
        zombie_process.live_thread_count !=
            OS_TEST_THREAD_SCHEDULER_EXPECTED_ZERO_COUNT ||
        zombie_process.exited_thread_count !=
    ...
    return CreateTestProcess(
           ...
           statistics.reaped_process_count ==
               OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT &&
           statistics.zombie_transition_count ==
               OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT &&
           scheduler.Validate() ==
               os::kernel::ThreadSchedulerStatus::Succeeded;
}
...
[[nodiscard]] bool ValidateWaitQueueFifo() noexcept {
    ...
    if (!InitializeTestScheduler(scheduler, processes, threads) ||
        wait_queue.Initialize(os::kernel::WaitQueueId{
            .value = OS_TEST_THREAD_SCHEDULER_WAIT_QUEUE_ID}) !=
    ...
    for (uint64_t thread_ordinal = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         thread_ordinal < OS_TEST_THREAD_SCHEDULER_WAKE_REASON_COUNT;
         ++thread_ordinal) {
        os::kernel::ThreadId thread_id{};
        if (!CreateTestThread(scheduler, process_index, thread_ordinal,
    ...
    for (uint64_t waiter_index = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         waiter_index < OS_TEST_THREAD_SCHEDULER_WAKE_REASON_COUNT;
         ++waiter_index) {
        if (scheduler.BlockCurrentThread(
    ...
    constexpr os::kernel::WakeReason OS_TEST_THREAD_SCHEDULER_WAKE_REASONS
        [OS_TEST_THREAD_SCHEDULER_WAKE_REASON_COUNT] = {
    ...
    for (uint64_t wake_index = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         wake_index < OS_TEST_THREAD_SCHEDULER_WAKE_REASON_COUNT;
         ++wake_index) {
        uint64_t woken_thread_index =
            os::kernel::OS_KERNEL_THREAD_INVALID_INDEX;
        bool wake_won = false;
        if (scheduler.WakeOne(wait_queue,
                              OS_TEST_THREAD_SCHEDULER_WAKE_REASONS[wake_index],
                              woken_thread_index, wake_won) !=
                os::kernel::ThreadSchedulerStatus::Succeeded ||
            !wake_won ||
            woken_thread_index != thread_indices[wake_index]) {
            return false;
        }
        bool duplicate_wake_won = true;
        if (scheduler.WakeThread(
                wait_queue, woken_thread_index,
                OS_TEST_THREAD_SCHEDULER_WAKE_REASONS[wake_index],
                duplicate_wake_won) !=
    ...
    return wait_statistics.enqueue_count ==
               OS_TEST_THREAD_SCHEDULER_WAKE_REASON_COUNT &&
...
[[nodiscard]] bool ValidateWaitQueueClose() noexcept {
    ...
    if (!InitializeTestScheduler(scheduler, processes, threads) ||
        wait_queue.Initialize(os::kernel::WaitQueueId{
            .value = OS_TEST_THREAD_SCHEDULER_CLOSE_QUEUE_ID}) !=
            os::kernel::WaitQueueStatus::Succeeded ||
        !CreateTestProcess(scheduler, OS_TEST_THREAD_SCHEDULER_FIRST_INDEX,
                           process_index, process_id)) {
        return false;
    ...
    for (uint64_t thread_ordinal = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         thread_ordinal < OS_TEST_THREAD_SCHEDULER_CLOSE_WAITER_COUNT;
         ++thread_ordinal) {
        uint64_t thread_index = os::kernel::OS_KERNEL_THREAD_INVALID_INDEX;
        os::kernel::ThreadId thread_id{};
        if (!CreateTestThread(scheduler, process_index, thread_ordinal,
    ...
    for (uint64_t waiter_index = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         waiter_index < OS_TEST_THREAD_SCHEDULER_CLOSE_WAITER_COUNT;
         ++waiter_index) {
        if (scheduler.BlockCurrentThread(
                wait_queue, os::kernel::WaitCondition::TestCondition,
                decision) != os::kernel::ThreadSchedulerStatus::Succeeded) {
    ...
    if (scheduler.CloseWaitQueue(wait_queue, woken_thread_count) !=
            os::kernel::ThreadSchedulerStatus::Succeeded ||
        woken_thread_count !=
            OS_TEST_THREAD_SCHEDULER_CLOSE_WAITER_COUNT ||
        ...
        scheduler.BlockCurrentThread(
            wait_queue, os::kernel::WaitCondition::TestCondition,
            decision) != os::kernel::ThreadSchedulerStatus::WaitQueueClosed) {
        return false;
    }
    for (uint64_t thread_index = OS_TEST_THREAD_SCHEDULER_FIRST_INDEX;
         thread_index < OS_TEST_THREAD_SCHEDULER_CLOSE_WAITER_COUNT;
         ++thread_index) {
    ...
    const os::kernel::WaitQueueStatistics statistics =
        wait_queue.Statistics();
    return statistics.closed &&
           statistics.close_count ==
               OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT &&
...
[[nodiscard]] bool ValidateMutexHandoff() noexcept {
    ...
    os::kernel::ThreadId second_thread_id{};
    if (!InitializeTestScheduler(scheduler, processes, threads) ||
        mutex.Initialize(os::kernel::WaitQueueId{
            .value = OS_TEST_THREAD_SCHEDULER_MUTEX_QUEUE_ID}) !=
    ...
    if (scheduler.Start(decision) !=
        ...
        mutex.Lock(scheduler, first_thread_id, decision) !=
            os::kernel::MutexStatus::Succeeded ||
        scheduler.YieldCurrentThread(decision) !=
            os::kernel::ThreadSchedulerStatus::Succeeded ||
        decision.current_thread_index != second_thread_index ||
        mutex.Lock(scheduler, second_thread_id, decision) !=
            os::kernel::MutexStatus::Blocked ||
        decision.current_thread_index != first_thread_index ||
        mutex.Unlock(scheduler, first_thread_id, woken_thread_index) !=
            os::kernel::MutexStatus::Succeeded ||
        woken_thread_index != second_thread_index ||
        !mutex.IsOwnedBy(second_thread_id) ||
        scheduler.YieldCurrentThread(decision) !=
            os::kernel::ThreadSchedulerStatus::Succeeded ||
        decision.current_thread_index != second_thread_index ||
        mutex.TryLock(second_thread_id) !=
            os::kernel::MutexStatus::Succeeded ||
    ...
    return statistics.acquisition_count ==
               OS_TEST_THREAD_SCHEDULER_SECOND_IDENTIFIER &&
           statistics.contention_count ==
               OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT &&
           statistics.handoff_count ==
               OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT &&
           statistics.unlock_count ==
               OS_TEST_THREAD_SCHEDULER_SECOND_IDENTIFIER &&
           statistics.waiting_thread_count ==
               OS_TEST_THREAD_SCHEDULER_EXPECTED_ZERO_COUNT &&
           !statistics.owned &&
           mutex.Reset() == os::kernel::MutexStatus::Succeeded &&
           scheduler.Validate() ==
               os::kernel::ThreadSchedulerStatus::Succeeded;
}
...
[[nodiscard]] bool ValidateSpinLockProtocol() noexcept {
    ...
    if (os::kernel::CurrentSpinLockDepth() !=
            OS_TEST_THREAD_SCHEDULER_EXPECTED_ZERO_COUNT ||
        !spin_lock.TryLock() || !spin_lock.IsLocked() ||
        os::kernel::CurrentSpinLockDepth() !=
            OS_TEST_THREAD_SCHEDULER_EXPECTED_SINGLE_COUNT) {
        return false;
    }
    spin_lock.Unlock();
    os::kernel::IrqSaveSpinLock irq_lock{DisableTestInterrupts,
                                         RestoreTestInterrupts};
    bool interrupts_were_enabled = false;
    if (!irq_lock.TryLock(interrupts_were_enabled) ||
        !interrupts_were_enabled || test_interrupts_enabled ||
        !irq_lock.IsLocked()) {
        return false;
    }
...
[[nodiscard]] bool ValidateSpinLockBlockRejected() noexcept {
    ...
    spin_lock.Lock();
    const os::kernel::MutexStatus lock_status =
        mutex.Lock(scheduler, second_thread_id, decision);
    spin_lock.Unlock();
    const os::kernel::ThreadSchedulerStatistics statistics =
        scheduler.Statistics();
...
int main() {
    os::test::TestContext test_context{
        OS_TEST_THREAD_SCHEDULER_SUITE_NAME};
    test_context.Expect(
        ValidateInitializationBoundaries(),
        OS_TEST_THREAD_SCHEDULER_INITIALIZATION_BOUNDARIES);
    test_context.Expect(
        ValidateInvalidKernelStackRejected(),
        OS_TEST_THREAD_SCHEDULER_INVALID_KERNEL_STACK);
    test_context.Expect(
        ValidateCapacityAndIdentifiers(),
        OS_TEST_THREAD_SCHEDULER_CAPACITY_AND_IDENTIFIERS);
    test_context.Expect(
        ValidateProcessThreadLimit(),
        OS_TEST_THREAD_SCHEDULER_PROCESS_THREAD_LIMIT);
    test_context.Expect(
        ValidateStateLifecycle(),
        OS_TEST_THREAD_SCHEDULER_STATE_LIFECYCLE);
    test_context.Expect(
        ValidateWaitQueueFifo(),
        OS_TEST_THREAD_SCHEDULER_WAIT_QUEUE_FIFO);
    test_context.Expect(
        ValidateWaitQueueClose(),
        OS_TEST_THREAD_SCHEDULER_WAIT_QUEUE_CLOSE);
    test_context.Expect(
        ValidateMutexHandoff(),
        OS_TEST_THREAD_SCHEDULER_MUTEX_HANDOFF);
    test_context.Expect(
        ValidateSpinLockProtocol(),
        OS_TEST_THREAD_SCHEDULER_SPIN_LOCK_PROTOCOL);
    test_context.Expect(
        ValidateSpinLockBlockRejected(),
