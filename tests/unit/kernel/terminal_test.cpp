...
constexpr std::string_view OS_TEST_TERMINAL_OUTPUT =
    "终端输出队列必须完整排空到设备并保持统计守恒";
...
constexpr uint64_t OS_TEST_TERMINAL_EXPECTED_SUBMITTED_BYTE_COUNT = 9ULL;
...
int main() {
    ...
    static_cast<void>(
        terminal.SubmitCharacter(OS_TEST_TERMINAL_FIRST_CHARACTER, action));
    static_cast<void>(
        terminal.SubmitCharacter(OS_TEST_TERMINAL_SECOND_CHARACTER, action));
    static_cast<void>(
        terminal.SubmitCharacter(OS_TEST_TERMINAL_THIRD_CHARACTER, action));
    static_cast<void>(terminal.SubmitCharacter(
        os::kernel::OS_KERNEL_TERMINAL_BACKSPACE_CHARACTER, action));
    static_cast<void>(
        terminal.SubmitCharacter(OS_TEST_TERMINAL_REPLACEMENT_CHARACTER, action));
    ...
    const bool canonical_line =
        terminal.TryRead(line, sizeof(line), read_bytes) ==
            os::kernel::TerminalStatus::Succeeded &&
    ...
    const bool stop =
        terminal.SubmitCharacter(os::kernel::OS_KERNEL_TERMINAL_STOP_CHARACTER, action) ==
            os::kernel::TerminalStatus::Succeeded &&
    ...
    const bool end_of_file =
        ...
        terminal.TryRead(line, sizeof(line), read_bytes) ==
            os::kernel::TerminalStatus::EndOfFile;
    test_context.Expect(interrupt && stop && end_of_file, OS_TEST_TERMINAL_CONTROL);

    const bool acquired =
        terminal.AcquireControllingSession(OS_TEST_TERMINAL_SESSION_ID,
                                           OS_TEST_TERMINAL_SESSION_ID,
                                           OS_TEST_TERMINAL_FOREGROUND_GROUP_ID) ==
        os::kernel::TerminalStatus::Succeeded;
    const bool foreground_allowed =
        terminal.CanRead(OS_TEST_TERMINAL_SESSION_ID,
                         OS_TEST_TERMINAL_FOREGROUND_GROUP_ID);
    ...
    test_context.Expect(
        output_written &&
            statistics.submitted_byte_count ==
                OS_TEST_TERMINAL_EXPECTED_SUBMITTED_BYTE_COUNT &&
            statistics.read_byte_count == OS_TEST_TERMINAL_EXPECTED_LINE_SIZE_BYTES &&
            statistics.consumed_byte_count ==
                OS_TEST_TERMINAL_EXPECTED_CONSUMED_BYTE_COUNT &&
