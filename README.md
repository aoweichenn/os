...
当前状态：第二周期 `v1.17 ordered metadata journal 与崩溃恢复`
已完整完成，下一阶段是 v1.18 ABI 冻结与系统加固。v1.1 已
落地动态物理内存、
...
只能是完整旧状态或完整新状态。最后由 v1.18 冻结 ABI、加固边界并建立
发布溯源。
...
ordered metadata journal 深入章，以及一份 v1.1–v1.17
...
当前 v1.17 的精确统计由本阶段发布门禁生成；测试、工具、书籍、构建文件和
网站均不计入。
