...
NASM Intel 汇编 & \OsProductionAssemblyCodeLineCount & 复位与串口有效行 \\
\bottomrule
\end{longtable}

当前阶段汇编占比较高，因为 CPU 复位后还没有 C++ ABI、64 位栈和运行时。
随着模式切换完成，高层机制会主要进入 C++20，汇编只保留入口、异常桩、上下文
切换和端口边界。这个变化反映系统层次演进，不是追求某种语言比例。

\section{三、代码行不是完成度}

\OsProductionCodeLineCount 行有效目标代码已经完成真实复位和串口，但尚未读取一个磁盘扇区，也没有
页表、异常入口或内核。相反，短小的复位向量只有三个有效字节，却承担整个系统
是否获得执行权的关键边界。完成度必须由机制和验收项表示。
...
\section{五、从 v0.1 到 v1.0 的规模增长来源}

后续真实代码增长主要来自磁盘状态机、模式转换、ELF 验证、异常入口、页分配、
页表、驱动、调度、同步、文件系统和用户程序。每个模块都先建立类型与失败语义，
再添加正常路径。测试和教材会同步增长，但继续从真实代码量中排除。
