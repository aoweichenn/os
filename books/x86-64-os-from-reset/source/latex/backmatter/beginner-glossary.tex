...
\markboth{初学者术语表与链路索引}{初学者术语表与链路索引}
...
\begin{description}[style=nextline,leftmargin=2.8em,labelindent=0em]
  ...
  \item[ABI]
  Application Binary Interface，已经编译的机器码之间交换参数、结果和结构
  的二进制约定。调用号、寄存器、字段 offset、对齐、错误值和 ELF 身份都可
  成为 ABI；修改头文件后重新编译成功，不能证明旧二进制仍然兼容。
  ...
  \item[VMA / 虚拟内存区域]
  Virtual Memory Area 是 Kernel 对一段半开虚拟区间的意图记录，保存来源、
  软件权限和用途。VMA 存在只表示进程有权使用该范围，不表示每页已经有 PTE
  或物理 frame；这正是 reservation 与 resident 的区别。

  \item[按需分页 Demand Paging]
  先登记 VMA，等真实访问产生 not-present 页故障时再提交页面的策略。匿名页
  提交时必须完整清零；Kernel 修复 PTE 并返回后，CPU 重试原用户指令。
  权限 fault、保留位错误和没有 VMA 的地址不能借此自动变成合法映射。

  \item[Program Break]
  传统连续数据区的字节级逻辑末端。用户分配器可以请求前移或后退 break，
  Kernel 以整页 VMA 覆盖它；扩大只建立 reservation，首次访问才产生
  resident page。它适合连续 heap，但不能代替多个独立匿名映射。

  \item[栈 Stack]
  按后进先出习惯保存调用返回地址、局部状态和中断现场的一段 RAM。CPU 复位后
  不会替裸机固件建立可用栈，早期代码必须先选择并设置 RSP。v1.8 用户栈预留
  8 MiB，但只允许紧邻 committed bottom 的下一页按保存的用户 RSP 连续提交；
  最低 guard 完全没有 VMA。

  \item[堆 Heap]
  供动态分配器管理的内存区域。它不是磁盘，也不是“所有剩余 RAM”；分配器
  仍需建立范围、对齐、所有权和释放规则。v1.8 Ring 3 UserHeap 在 program
  break 上用 boundary tag、first-fit、split 与双向 coalesce 管理对象。
  ...
  \item[devfs]
  把设备名称组织为 VFS 节点的伪文件系统。它负责
  \filepath{/dev/console} 这样的路径和 vnode，不等于终端驱动本身；字符
  读写、队列、中断和作业控制仍由对应设备对象负责。

  \item[procfs]
  把动态进程或系统状态呈现为只读文件的伪文件系统。内容通常在读取时生成，
  不占用普通磁盘数据块；一次 read 的快照与下一次 read 可以不同，用户程序
  必须处理 short read 和 EOF。
