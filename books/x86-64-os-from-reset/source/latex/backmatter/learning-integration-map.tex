\chapter*{配套学习资料索引}
\addcontentsline{toc}{chapter}{配套学习资料索引}
\markboth{配套学习资料索引}{配套学习资料索引}

本索引列出配套学习资料在正文中的落点。读者可以按目录顺序阅读，也可以从
实验记录、硬件图或背景材料反查对应章节。
...
第 7 章工具链；第 8 章 ELF loader；第 20 章修改与验证方法 \\ \hline
...
第 9--11 章异常与内存；第 12 章用户边界；第 15 章虚拟内存；术语表 \\ \hline
...
\section*{六、同一概念应该从哪里开始阅读}

配套实验记录保留开发时的先后顺序，正文则按照概念依赖组织。
...
这样阅读可以避免相互矛盾的重复解释。若从开发记录反查，上面的表给出主要
位置；若按目录顺序阅读，前置概念会在第一次需要它时出现。

\section*{七、怎样从正文返回原始资料}

\begin{enumerate}[leftmargin=2.2em]
  \item 在本索引找原 learning 文件，定位对应章节；
  \item 在章节先找术语定义，再找状态表、图、失败框和实验结果；
  \item 对图中任一箭头回答源、目的、载荷、初始化者、完成条件和失败所有权；
  \item 对实体网络回到十页原理图和 KiCad，不把学习图当制造数据；
  \item 对实现数字回到当前源码/CTest，不把历史阶段值当当前版本；
  \item 在书末术语表回查跨层同名概念，再用检查清单确认能够独立复述完整链。
\end{enumerate}

生成 PDF 前，自动检查会确认全部章节和图片存在，并检查七张整机图的卡片净距
以及三张实体电路图的显式引脚和导线连通。随后通过 XeLaTeX 构建和页面渲染
检查横置页、图注、字体与边界。概念和数值仍需同源码、处理器/设备规范以及
固定版本的上游硬件资料交叉核对。
