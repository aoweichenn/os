\section{v1.7：从磁盘 PID1 到进程树与原子 exec}
...
\subsection{历史背景：为什么 Unix 把 fork、exec 与 wait 分开}
...
这不是绕过 Unix 模型，而是把每一项机制放到其依赖已经存在的阶段。

\subsection{PID1 为什么不是“恰好编号为一的普通程序”}
...
\begin{longtable}{L{0.18\textwidth}L{0.23\textwidth}L{0.48\textwidth}}
\toprule
概念 & 生命周期 & 责任\\
\midrule
Process & 程序资源容器 & AddressSpace、FileTable、FsContext、父子身份和退出记录\\
Thread & 可调度执行流 & UserContext、KernelStack、FXSAVE、run/wait queue 关系\\
PID & 单调 64 位公开身份 & 系统调用、日志和父进程保存的稳定名字\\
process index & 可复用内部槽位 & 有界数组定位；回收后可用于另一 PID\\
\bottomrule
\end{longtable}
...
\subsection{ProcessTree 是调度器之外的第二个状态机}
...
\subsection{退出、孤儿收养与 wait 的精确顺序}
...
\subsection{为什么 ELF reader 是随机访问接口}
...
\subsection{第一遍 ELF：分配之前拒绝不可信元数据}
...
\subsection{第二遍 ELF：物理页、BSS 与 PTE 权限}
...
\subsection{x86-64 在 exec 成功时真正改变什么}
...
状态 & 新值来源 & 意义\\
\midrule
CR3 & 候选 AddressSpace 根物理地址 & 改变用户虚拟地址翻译；Kernel 高半分支保持共享\\
RIP & ELF \code{e\_entry} & 新程序第一条指令，旧 syscall 后继不再执行\\
RSP & ProgramArgumentPlan & 指向 16 字节对齐的新用户栈\\
RDI/RSI/RDX & argc/argv/envp 布局 & freestanding 入口直接取得参数\\
CS/SS/RFLAGS & 安全用户初始常量 & 恢复 CPL3 段和允许的标志集合\\
\bottomrule
\end{longtable}
...
\subsection{close-on-exec 为什么属于 fd 表项}
...
\subsection{系统调用 ABI 如何保持固定宽度}

v1.7 在既有编号之后追加 36--38，不重排历史接口：

\begin{longtable}{L{0.11\textwidth}L{0.20\textwidth}L{0.31\textwidth}L{0.27\textwidth}}
\toprule
编号 & 接口 & 输入 & 成功结果\\
\midrule
36 & SpawnProcess & 48 B launch request & 新 64 位 PID\\
37 & ExecProcess & 48 B launch request & 不回旧入口\\
38 & WaitProcess & PID/ANY + 40 B result & 被回收 PID\\
\bottomrule
\end{longtable}
...
\begin{pdfdiagram}[node distance=7mm and 8mm]
  ...
  \draw[os arrow,dashed] (child) to[bend left=34]
      node[right,font=\scriptsize]{父退出后收养} (init);
...
容量不是用小配置冒充压力测试，而是保证最小兼容配置也运行同一完整功能链。

\subsection{构建期离线安装为什么不是启动后门}
...
\subsection{失败矩阵：哪一层失败，什么必须保持}

\begin{longtable}{L{0.25\textwidth}L{0.29\textwidth}L{0.35\textwidth}}
\toprule
失败位置 & 对外结果 & 必须保持\\
\midrule
请求尺寸错误 & InvalidArgument & 不读取未知布局，不改变 Process\\
请求/字符串页错误 & InvalidUserMemory & 旧映像、fd、树和候选资源不变\\
项目数或 128 KiB 越界 & ArgumentListTooLarge & 旧 RIP/CR3/RSP 可继续执行\\
路径/文件读取失败 & executable read error & VFS 临时打开引用关闭\\
ELF 截断/W+X/重叠 & InvalidExecutable & 不提交任何候选页\\
frame/页表不足 & image/address-space failure & 候选根递归销毁，旧根权威\\
候选 CR3 激活失败 & page-table activation failure & 恢复旧 CR3，销毁候选\\
Process 容量耗尽 & ProcessLimitExceeded & 父 Process 与现有孩子不变\\
wait 结果页错误 & InvalidUserMemory & Zombie 尚未被消费\\
wait 无直接孩子 & NoChildProcess & 树与调度器不变\\
提交后内部不变量失败 & Kernel 停机 & 不伪装为可恢复用户错误\\
\bottomrule
\end{longtable}

\begin{failurebox}
exec 的“原子性”不是 CPU 提供的一条原子指令，而是软件定义的发布边界。
提交前所有新对象都可丢弃，提交后只有新映像权威。若某项资源既在提交前修改
旧对象、又无法回滚，就说明它放错了事务位置。
\end{failurebox}

\subsection{五层测试分别证明什么}
...
\begin{contractbox}
v1.7 的完成判定不是“屏幕出现 init”。必须同时证明：程序字节来自 rootfs，
PID1 收养并回收孤儿，精确 128 KiB 参数到达新用户栈，失败 exec 保留旧
CR3/RIP/fd，成功 exec 不返回旧入口，八个 Process/Thread 与全部页、栈、
fd、FsContext 最终恢复基线，宿主模型与真实 QEMU 对同一不变量给出一致
结论。
\end{contractbox}
