\section{实体 x86-64 载板全链路案例：LattePanda Mu}
...
\subsection{二、案例的硬件参数先完整列出}

\begin{longtable}{L{0.23\linewidth}L{0.25\linewidth}L{0.42\linewidth}}
\toprule
项目 & 参数 & 这项参数告诉我们什么 \\
\midrule
计算模块 & LattePanda Mu，N100/N305 版本 & 两种处理器版本共用载板接口；CPU、DRAM 和平台固件在模块内 \\
模块接口 & 260-pin DDR4 SODIMM 外形边缘连接器 & 借用成熟机械形态，不表示接口电气协议是 DDR4 内存条 \\
载板 & DFR1142 Lite Carrier V2 & 本节所有位号和网络均以 V2 原理图为准 \\
原理图 & 10 个层次页 & 总图、模块、PCIe、HDMI、USB、GbE、低速、M.2、电源、风扇 \\
PCB 外形 & \(146\ \mathrm{mm}\times102\ \mathrm{mm}\) & 机械外壳、安装孔、连接器伸出和散热都受此边界约束 \\
PCB 层数 & 四层，约 \(1.6062\ \mathrm{mm}\) 厚 & 高速回流和电源分配依赖叠层，不是只看顶层连线 \\
铜厚 & 外层 \(35\ \mu\mathrm{m}\)，内层 \(15.2\ \mu\mathrm{m}\) & 电源压降、温升和阻抗计算都需要实际铜厚 \\
器件封装 & 305 个已放置 footprint & 原理图符号已经对应 PCB 焊盘和机械占位 \\
电气网络 & 407 个 net & 同名网络把跨页符号和 PCB 铜连接成同一电气节点 \\
布线规模 & 3961 段走线、1307 个过孔 & 它是已布局布线的板级工程，不只是概念连接表 \\
主输入 & DC、15 V USB-PD、Smart UPS 三路 & 三路经保护和肖特基汇流到 \code{VDC} \\
模块输入 & \code{VIN} 允许 9--20 V，pins 250--260 & 11 个并联触点降低单触点电流和阻抗 \\
板上电源 & \code{+12VA}、\code{+5V}、\code{+3V3}、\code{+3V3\_ROM} & 分别服务 PCIe、USB/HDMI/风扇、低压外设和可选 SPI ROM \\
高速接口 & HDMI、USB 3.x、PCIe x4、M.2 M/E、GbE & 每类接口都包含数据对及必要的时钟、复位、电源或管理信号 \\
低速接口 & I2C、UART、SPI、RTC、按键、UPS、风扇 & 用于控制、调试、状态保持和散热，不是“无关紧要的慢线” \\
\bottomrule
\end{longtable}
...
层级 & 它拥有的职责 & 本案例怎样体现 \\
\midrule
处理器/SoC & 执行 x86-64、内存控制、根端口和平台状态 & N100/N305 位于 Mu 模块内部 \\
计算模块 & 封装 SoC、DRAM、核心电源与固件接口 & 通过 260-pin 边缘触点发布信号 \\
载板 & 输入保护、降压、连接器、外设与 PCB 互连 & DFR1142 V2 十页电路 \\
扩展设备 & 实现具体存储、网络、显示或用户输入功能 & NVMe、Wi-Fi、PCIe 卡、USB 设备等 \\
固件 & 初始化模块内部平台并描述硬件 & 模块 UEFI/BIOS，受 HSIO 配置约束 \\
操作系统 & 枚举接口、驱动设备、管理资源 & STUOS 实机适配仍需新增相应路径 \\
\bottomrule
\end{longtable}
...
前缀 & 器件类别 & 案例与职责 \\
\midrule
\code{J} & 连接器、插座 & J17 DC 输入、J18 USB-C、J2 PCIe x4 \\
\code{U} & 集成电路或阵列 & U14 CH224K 协商 PD，U6 RTL8111H 实现网卡 \\
\code{Q} & MOSFET 或晶体管 & Q7 控制 PCIe 插槽的 \code{+12VA} \\
\code{D} & 普通二极管、TVS、ESD & D29 钳位输入瞬态，D15--D20 参与电源汇流 \\
\code{F} & 保险丝或可恢复保险丝 & F6 保护 DC 输入，F2--F5 保护 USB VBUS \\
\code{L} & 电感 & L1/L2 是两路降压转换器的储能元件 \\
\code{R} & 电阻 & 上拉、下拉、反馈、配置、限流或阻尼 \\
\code{C} & 电容 & 去耦、储能、补偿或高速 AC 耦合 \\
\code{SW} & 人工开关 & SW1 电源键，SW2 复位键 \\
\code{TP} & 测试点 & 允许探针稳定接触关键网络，便于上电测量 \\
\bottomrule
\end{longtable}
...
\subsubsection{5.1 DC Jack：从 J17 到 VDC}
...
F6 的型号是 \code{BSMD1206C-2100T}。它位于能量入口，目的不是让正常工作
时产生明显压降，而是在严重过流或短路持续存在时限制能量。保险丝不能代替正确
...
\subsubsection{5.2 USB-C PD：协商电压不等于转换电压}
...
CH224K 也不是降压芯片；它没有把 20 V“变成”15 V，而是请求支持的适配器直接
提供 15 V。若适配器不支持、CC 连接错误或配置不对，预期 VBUS 就不会出现。

\subsubsection{5.3 Smart UPS：一只连接器同时承载四类职责}
...
\subsubsection{5.4 三路为什么能够汇合而不直接互灌}
...
\subsubsection{5.5 VDC 的五条去路}

\begin{longtable}{L{0.18\linewidth}L{0.25\linewidth}L{0.24\linewidth}L{0.23\linewidth}}
\toprule
分支 & 核心器件 & 输出/去向 & 为什么需要 \\
\midrule
模块主输入 & 直接连接 & Mu pins 250--260 \code{VIN} & 模块接受 9--20 V，并联 11 个触点承载电流 \\
PCIe 12 V & Q7 \code{NCE30P28Q}、Q5/Q6/U13 & \code{+12VA} 到 J2 & 受控向扩展卡提供 12 V，而非把任意 VDC 直接送出 \\
5 V 降压 & U15 \code{SY8253ADC}、L1 & \code{+5V} & USB VBUS、HDMI 和风扇需要 5 V \\
3.3 V 降压 & U16 \code{SY8253ADC}、L2 & \code{+3V3} & M.2、RTL8111H 和低速接口需要 3.3 V \\
ROM 电源 & U17 \code{HM7533HBPR} & \code{+3V3\_ROM} & 为可选载板 SPI Flash 提供独立 3.3 V \\
\bottomrule
\end{longtable}
...
\subsubsection{5.6 降压转换器为什么需要电感、电容和反馈}
...
\subsubsection{5.7 PWR\_EN 与“插电并不等于所有电源立即打开”}
...
\subsubsection{5.8 USB 端口为什么各有自己的保险丝}
...
\subsection{六、260-pin 接口：载板与计算模块的总边界}
...
\subsubsection{6.1 最早期电源和控制脚}

\begin{longtable}{L{0.13\linewidth}L{0.21\linewidth}L{0.27\linewidth}L{0.29\linewidth}}
\toprule
物理 pin & 信号 & 电气/方向 & 载板中的完整作用 \\
\midrule
1 & \code{PWR\_SW\#} & 3.3 V 输入，内部约 10 k\(\Omega\) 上拉 & SW1 或外接按键拉低，请求模块改变电源状态 \\
3 & \code{RST\_SW\#} & 3.3 V 输入，内部约 10 k\(\Omega\) 上拉 & SW2 拉低，请求复位而非切断全部输入 \\
5 & \code{PSON} & 3.3 V 输出 & 表示活动电源状态，参与外设降压和状态灯控制 \\
7 & \code{SLP\_S4} & 3.3 V 输出 & 表示睡眠/软关机相关状态 \\
9 & \code{TSENSE} & 3.3 V 输入 & 可接 NTC 温度检测网络 \\
115 & \code{VBAT} & 约 3.0 V 后备电源 & CR1220 在主电源断开时维持 RTC/配置域 \\
131 & \code{SUSCLK} & 挂起时钟输出 & 向 M.2 等设备提供低功耗状态参考 \\
133 & \code{BIOS\_SEL} & 3.3 V 输入 & 选择模块集成固件或可选载板 SPI ROM 路径 \\
250--260 & \code{VIN} & 9--20 V 输入 & 11 个并联触点向模块传输主功率 \\
\bottomrule
\end{longtable}
...
\begin{longtable}{L{0.12\linewidth}L{0.32\linewidth}L{0.46\linewidth}}
\toprule
HSIO & Mu 物理 pin & DFR1142 V2 实际用途 \\
\midrule
0 & TX 13/15，RX 16/18 & USB 3.x 端口 1 的 SuperSpeed TX/RX \\
1 & TX 19/21，RX 22/24 & USB 3.x 端口 2 的 SuperSpeed TX/RX \\
2 & TX 25/27，RX 28/30 & M.2 M Key 的 PCIe x1 \\
3 & TX 31/33，RX 34/36 & M.2 E Key 的 PCIe x1 \\
6 & TX 61/63，RX 64/66 & RTL8111H 千兆网控制器的 PCIe x1 \\
8 & TX 37/39，RX 40/42 & PCIe x4 插槽 lane 0 \\
9 & TX 43/45，RX 46/48 & PCIe x4 插槽 lane 1 \\
10 & TX 49/51，RX 52/54 & PCIe x4 插槽 lane 2 \\
11 & TX 55/57，RX 58/60 & PCIe x4 插槽 lane 3 \\
\bottomrule
\end{longtable}

\subsection{七、高速线路为什么一定要画 P 和 N 两根}
...
\subsection{八、HDMI：显示数据之外还有发现与管理}
...
\begin{longtable}{L{0.24\linewidth}L{0.15\linewidth}L{0.23\linewidth}L{0.28\linewidth}}
\toprule
Mu 信号 & pin & HDMI pin & 含义 \\
\midrule
\code{DDI\_B\_TX0\_P/N} & 217/215 & 1/3 & TMDS Data2 P/N \\
\code{DDI\_B\_TX1\_P/N} & 211/209 & 4/6 & TMDS Data1 P/N \\
\code{DDI\_B\_TX2\_P/N} & 205/203 & 7/9 & TMDS Data0 P/N \\
\code{DDI\_B\_TX3\_P/N} & 199/197 & 10/12 & TMDS Clock P/N \\
\code{DDI\_B\_DDC\_SCL} & 171 & 15 & 读取 EDID 的时钟 \\
\code{DDI\_B\_DDC\_SDA} & 169 & 16 & 读取 EDID 的双向数据 \\
\code{DDI\_B\_HPD} & 183 & 19 & 显示器热插拔检测 \\
\code{HDMI\_5V} & 板上 5 V & 18 & HDMI 辅助 5 V \\
GND/Shield & 地网/机壳策略 & 2/5/8/11/17/外壳 & 差分回流、低速参考和屏蔽 \\
\bottomrule
\end{longtable}
...
\subsection{九、USB：一个 USB 3.x 插口其实同时有两套数据通道}
...
\begin{longtable}{L{0.13\linewidth}L{0.31\linewidth}L{0.27\linewidth}L{0.19\linewidth}}
\toprule
端口 & SuperSpeed TX & SuperSpeed RX & USB 2.0 \\
\midrule
J4A & HSIO0 pins 13/15，经 C27/C26 & HSIO0 pins 16/18 & Port 2 pins 75/73 \\
J4B & HSIO1 pins 19/21，经 C29/C28 & HSIO1 pins 22/24 & Port 1 pins 69/67 \\
J5 上层 & 无 & 无 & Port 3 D+ 81、D- 79 \\
J5 下层 & 无 & 无 & Port 5 D+ 111、D- 109 \\
\bottomrule
\end{longtable}
...
\subsection{十、PCIe x4：四条 lane、时钟、复位、唤醒和电源}
...
\begin{longtable}{L{0.09\linewidth}L{0.29\linewidth}L{0.23\linewidth}L{0.29\linewidth}}
\toprule
lane & Mu TX P/N 到插槽 & TX 耦合 & 插槽 TX 到 Mu RX P/N \\
\midrule
0 & HSIO8 pins 37/39 到 \code{PETp0/PETn0} & C1/C2 & \code{PERp0/PERn0} 到 pins 40/42 \\
1 & HSIO9 pins 43/45 到 \code{PETp1/PETn1} & C3/C4 & \code{PERp1/PERn1} 到 pins 46/48 \\
2 & HSIO10 pins 49/51 到 \code{PETp2/PETn2} & C5/C6 & \code{PERp2/PERn2} 到 pins 52/54 \\
3 & HSIO11 pins 55/57 到 \code{PETp3/PETn3} & C7/C8 & \code{PERp3/PERn3} 到 pins 58/60 \\
\bottomrule
\end{longtable}
...
附加网络 & Mu pin/来源 & 完整作用 \\
\midrule
\code{REFCLK2\_P/N} & 97/99 & 100 MHz 差分参考时钟，帮助链路两端建立共同频率基准 \\
\code{PLT\_RST\#} & 105 & 接插槽 \code{PERST\#}，平台未就绪时保持设备复位 \\
\code{PEWAKE\#} & 103 & 设备可请求从低功耗状态唤醒平台 \\
\code{+12VA} & Q7 受控输出 & 为需要 12 V 的扩展卡负载供电 \\
\code{+3V3} & U16 输出 & 提供 3.3 V 和辅助电源相关触点 \\
GND & 地平面 & 覆盖标准规定的所有返回触点和屏蔽参考 \\
\bottomrule
\end{longtable}
...
\subsection{十一、M.2 M Key 与 E Key 为什么不能只说“都是 M.2”}
...
\subsubsection{11.1 M Key：本设计是一条 PCIe x1}

\begin{longtable}{L{0.25\linewidth}L{0.35\linewidth}L{0.30\linewidth}}
\toprule
功能 & Mu 来源 & J15 去向/状态 \\
\midrule
PCIe TX P/N & HSIO2 pins 25/27，经 C49/C50 220 nF/16 V & \code{PETp0/PETn0} \\
PCIe RX P/N & HSIO2 pins 28/30 & \code{PERp0/PERn0} \\
参考时钟 & REFCLK0 pins 85/87，原图明确交换 P/N & \code{REFCLKp/n} \\
复位 & \code{PLT\_RST\#} pin 105 & \code{PERST\#} \\
唤醒 & \code{PEWAKE\#} pin 103 & \code{PEWAKE\#} \\
挂起时钟 & \code{SUSCLK} pin 131 & \code{SUSCLK} \\
供电 & \code{+3V3}/GND & 标准规定的全部供电与地触点 \\
未用 & 未从模块接出 & SATA 与 \code{CLKREQ\#} 明确 NC \\
\bottomrule
\end{longtable}
...
\subsubsection{11.2 E Key：PCIe 常给 Wi-Fi，USB 常给蓝牙}

\begin{longtable}{L{0.25\linewidth}L{0.35\linewidth}L{0.30\linewidth}}
\toprule
功能 & Mu 来源 & J16 去向/状态 \\
\midrule
PCIe TX P/N & HSIO3 pins 31/33，经 C55/C56 220 nF/16 V & \code{PETp0/PETn0} \\
PCIe RX P/N & HSIO3 pins 34/36 & \code{PERp0/PERn0} \\
参考时钟 & REFCLK3 pins 88/90 & \code{REFCLKp/n} \\
USB 2.0 & Port 7 pins 76/78 & \code{USB\_D+/USB\_D-} \\
时钟请求 & \code{CLKREQ3\#} pin 100 & \code{CLKREQ\#} \\
复位/唤醒 & pins 105/103 & \code{PERST\#}/\code{PEWAKE\#} \\
挂起时钟 & \code{SUSCLK} pin 131 & \code{SUSCLK} \\
供电 & \code{+3V3}/GND & 标准规定的全部供电与地触点 \\
未用 & 未从模块接出 & SDIO、UART 明确 NC \\
\bottomrule
\end{longtable}
...
\subsection{十二、千兆以太网：PCIe 到控制器，再经磁性器件到网线}
...
\begin{longtable}{L{0.28\linewidth}L{0.15\linewidth}L{0.47\linewidth}}
\toprule
Mu 网络 & pin & RTL8111H 端及职责 \\
\midrule
HSIO6 TX P/N & 61/63 & \code{HSIP/HSIN}，主机向网卡的 PCIe 高速数据 \\
HSIO6 RX P/N & 64/66 & \code{HSOP/HSON}，网卡向主机的 PCIe 高速数据 \\
\code{REFCLK4\_P/N} & 94/96 & PCIe 参考时钟 \\
\code{PLT\_RST\#} & 105 & \code{PERSTB}，保持网卡复位直到平台放行 \\
\code{CLKREQ4\#} & 102 & \code{CLKREQB}，电源管理时请求参考时钟 \\
\bottomrule
\end{longtable}
...
\subsection{十三、I2C、UART、SPI 和控制线从零解释}
...
\subsubsection{13.1 I2C：两根共享线为什么需要上拉}
...
\begin{longtable}{L{0.15\linewidth}L{0.32\linewidth}L{0.33\linewidth}}
\toprule
总线 & SCL 来源/排针 & SDA 来源/排针 \\
\midrule
I2C2 & Mu pin 154 到相应 J9 pin 1 & Mu pin 156 到相应 J7 pin 1 \\
I2C3 & Mu pin 150 到相应 J9 pin 2 & Mu pin 152 到相应 J7 pin 2 \\
I2C4 & Mu pin 146 到相应 J9 pin 3 & Mu pin 148 到相应 J7 pin 3 \\
I2C5 & Mu pin 142 到相应 J9 pin 4 & Mu pin 144 到相应 J7 pin 4 \\
\bottomrule
\end{longtable}
...
\subsubsection{13.2 UART：异步串行不等于 RS-232 电平}
...
\begin{longtable}{L{0.15\linewidth}L{0.31\linewidth}L{0.31\linewidth}}
\toprule
UART & RX 到 J8 & TX 到 J10 \\
\midrule
UART0 & Mu pin 137 到 pin 1 & Mu pin 139 到 pin 1 \\
UART1 & Mu pin 141 到 pin 2 & Mu pin 143 到 pin 2 \\
UART2 & Mu pin 140 到 pin 3 & Mu pin 138 到 pin 3 \\
\bottomrule
\end{longtable}
...
\subsubsection{13.3 SPI 与可选载板固件 Flash}
...
Mu pin & 信号 & 作用 \\
\midrule
153 & \code{SPI\_IO3} & 四线模式数据位 3 \\
155 & \code{SPI\_CLK} & 串行时钟 \\
159 & \code{SPI\_MOSI/IO0} & 主机输出/数据位 0 \\
161 & \code{SPI\_IO2} & 四线模式数据位 2 \\
163 & \code{SPI\_MISO/IO1} & 主机输入/数据位 1 \\
165 & \code{SPI\_CS\#} & 低有效片选 \\
133 & \code{BIOS\_SEL} & 选择集成或载板 ROM 路径 \\
电源 & \code{+3V3\_ROM}/GND & U17 提供的专用供电和返回 \\
\bottomrule
\end{longtable}
...
\subsection{十四、RTC、按键、状态和风扇不是边角功能}
...
\begin{longtable}{L{0.24\linewidth}L{0.15\linewidth}L{0.51\linewidth}}
\toprule
功能 & Mu pin & 外部连接与意义 \\
\midrule
CPU 风扇 PWM & 2 & 到 M1 控制输入，只允许匹配的 3.3 V 逻辑 \\
CPU 风扇 TACH & 4 & M1 转速反馈到模块输入 \\
SYS 风扇 PWM & 6 & 到可选 M2 控制输入 \\
SYS 风扇 TACH & 8 & 可选 M2 转速反馈 \\
温度输入 & 9 & JP2/可选 NTC，用阻值随温度变化形成模拟测量 \\
风扇电源 & \code{+5V}/GND & 提供电机能量与返回，不能从 PWM 信号脚取电 \\
\bottomrule
\end{longtable}

\subsection{十五、从原理图进入四层 PCB}
...
从顶到底 & 铜/介质 & 主要学习职责 \\
\midrule
F.Cu & \(35\ \mu\mathrm{m}\) 铜 & 顶层器件和短连接 \\
Prepreg & \(0.2104\ \mathrm{mm}\) & 决定顶层到参考平面的几何关系 \\
In1.Cu & \(15.2\ \mu\mathrm{m}\) 铜 & 主要连续 GND 参考与回流 \\
Core & \(1.065\ \mathrm{mm}\) & 主介质厚度 \\
In2.Cu & \(15.2\ \mu\mathrm{m}\) 铜 & 电源分配和辅助参考 \\
Prepreg & \(0.2104\ \mathrm{mm}\) & 决定底层到内层的几何关系 \\
B.Cu & \(35\ \mu\mathrm{m}\) 铜 & 底层器件、走线和铜皮 \\
\bottomrule
\end{tabular}
\end{center}
...
\topic{原理图、PCB 和成品是三种不同证据}%

\begin{longtable}{L{0.17\linewidth}L{0.32\linewidth}L{0.41\linewidth}}
\toprule
证据层 & 能证明什么 & 不能单独证明什么 \\
\midrule
原理图 & 网络意图、器件值、方向和接口 & footprint 映射、实际阻抗、机械干涉 \\
PCB 文件 & 焊盘、铜线、叠层、布局与设计规则 & 制造厂是否按数据生产、装配是否正确 \\
成品测量 & 电压、波形、温升、枚举和真实行为 & 未测工况的长期可靠性与全部规范裕量 \\
\bottomrule
\end{longtable}
...
\subsection{十六、十页图的阅读路线与故障问题}

\begin{longtable}{L{0.08\linewidth}L{0.23\linewidth}L{0.31\linewidth}L{0.28\linewidth}}
\toprule
页 & 先追哪条链 & 正常条件 & 失败时先问 \\
\midrule
1 & 模块到每个功能页 & 所有跨页标签有唯一去向 & 网络名是否拼写一致、方向是否清楚 \\
2 & VIN/控制/HSIO 分组 & 模块 pin 与引脚表一致 & BIOS 复用是否与铜线方案一致 \\
3 & PCIe lane 0 再复制到 1--3 & TX/RX、时钟、复位、电源齐全 & 哪一 lane 未训练，PERST 是否释放 \\
4 & HPD、DDC、TMDS & 先发现显示器再传像素 & 5 V、HPD、EDID 还是高速数据失败 \\
5 & 单个 USB 3.x 端口两套通道 & VBUS、D+/D-、TX/RX 都存在 & 完全不枚举还是只降到 USB2 \\
6 & PCIe 到 RTL8111H 再到 RJ45 & 主机侧与网线侧分别成立 & PCIe 不枚举、PHY 无 link 或软件无驱动 \\
7 & 一组 I2C/UART 连电源地 & 电平、方向、上拉和 ESD 正确 & 是否误接 RS-232、是否缺共同地 \\
8 & M Key 和 E Key 分开追 & 实际接出协议匹配所插卡 & 卡要求 SATA/USB 而载板没有接出吗 \\
9 & 输入到 VDC 再到各电源轨 & 保护、汇流、使能、反馈依次成立 & 故障停在入口、EN、稳压还是负载短路 \\
10 & 5 V/PWM/TACH 三类线 & 能量、控制、反馈方向各自正确 & 风扇不转还是转速反馈不读 \\
\bottomrule
\end{longtable}

\subsection{十七、当前 STUOS 与实体载板的共同点和断层}

两台机器都执行 x86-64，都有物理地址、分页、异常、中断和设备，也都必须在
内核之前建立固件环境。不同之处不是“虚拟机没有硬件知识，实体机才有”，而是
平台契约和设备集合不同。

\begin{longtable}{L{0.25\linewidth}L{0.31\linewidth}L{0.34\linewidth}}
\toprule
层次 & 当前 STUOS/QEMU & Mu N100/N305 实体平台 \\
\midrule
机器定义 & \code{pc,accel=tcg} & SoC、模块固件、ACPI 和真实载板拓扑 \\
复位固件 & 自研 128 KiB ROM 映射到 4 GiB 顶端 & 模块厂商 UEFI/BIOS 完成硅与 DRAM 初始化 \\
主存说明 & QEMU 建立 RAM，项目另取 \code{fw\_cfg} E820 & UEFI Memory Map 与固件/ACPI 表 \\
磁盘 & legacy IDE/ATA PIO，端口 \code{0x1F0} & 主要可能是 NVMe、eMMC 或 USB \\
早期控制台 & 16550A COM1，端口 \code{0x3F8} & 需确认 UART 路径或使用 UEFI GOP framebuffer \\
定时与中断 & 8259A、PIT、i8042，加本地 APIC 路径 & APIC、ACPI、HPET/SoC 定时器和 USB HID 等 \\
设备发现 & 固定教学地址和少量约定 & PCI/PCIe 枚举、ACPI 命名设备和固件资源 \\
输入 & QEMU 注入 PS/2，经 i8042 IRQ1 & 实际常用 USB 键盘，需要 xHCI 与 HID 栈 \\
启动装入 & Stage 1 自行 ATA 读取容器/ELF & 初期更适合 UEFI 文件/块协议装入 PE/COFF 入口 \\
\bottomrule
\end{longtable}
...
\subsection{十八、把实体铜线重新接回软件全链路}

\subsubsection{18.1 从按下电源键到内核入口}
...
\subsubsection{18.2 从 USB 键盘到 Shell 字符}
...
\subsubsection{18.3 从 NVMe 文件到可运行进程}
...
\subsection{十九、从不上电到分阶段 bring-up}
...
\subsection{二十、学习验收：必须能完整说出“为什么”}

读完本节后，不以“看过图”为验收，而以能否回答下面问题为准：
