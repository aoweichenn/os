\section{rootfs v2 怎样保存大文件和目录}

文件系统不是“把一个 C++ 对象写进文件”。它要把断电后仍有意义的名字、
对象身份、逻辑长度、块所有权和提交状态编码到扇区中，再让另一个全新启动
实例能够独立检查这些事实。rootfs v2 使用 256 MiB 容量、三级间接树、稀疏文件、
完整命名空间修改和只读 fsck，把这条链从 Ring 3 一直闭合到 ATA LBA。

\topic{为什么先分离 VFS，随后才替换根格式}
VFS 已经把路径遍历、挂载、每 Process 的 root/cwd、一次打开和底层后端
...
ELF。因此新盘面使用新的 magic 和格式版本，不让同一个 magic 在新旧内核中
具有两种含义。

\topic{历史因果链：从连续分配到 inode 块树}
...
沿用过这套模型；现代 ext4、XFS 常用 extent 描述连续范围。这里选择块树，
不是声称它比 extent 更新，而是因为每个逻辑块、每个元数据块和每次回收都能
画成一棵确定所有权树，适合作为 extent 与 page cache 之前的学习基础。

\topic{四种“块”不要混为一个词}\par
\begin{osgridlongtable}{L{0.22\textwidth}L{0.24\textwidth}L{0.43\textwidth}}
名称 & 当前粒度 & 职责\\ \hline
ATA sector & 512 字节 & IDE 设备一次 PIO 命令传输的硬件单位\\ \hline
rootfs block & 512 字节 & 文件系统分配、缓存、CRC 和相对寻址单位\\ \hline
file logical block & 512 字节 & 文件 offset 除以块大小后的文件内部索引\\ \hline
host sparse extent & 宿主决定 & raw image 实际占用宿主存储的非空范围\\ \hline
\end{osgridlongtable}
...
\topic{逻辑 1 GiB 为什么不等于每次构建写 1 GiB}
...
派生故障镜像和持久化临时盘不能用一个只复制已知前缀的旧工具。稀疏
复制先建立精确逻辑长度，再复制非零 extent，并测试靠近 1 GiB 尾部的
非零内容；\code{pwrite} 还要循环处理短写。否则“看起来很快的复制”可能
悄悄截掉未来分区或把全部 hole 物化。

\section{rootfs v2 的盘面布局}
当前 rootfs 从 LBA 32768 开始，固定包含 524288 个 512 字节块，即
\(2^{28}\) 字节。区域如下：

\begin{osgridlongtable}{L{0.20\textwidth}L{0.13\textwidth}L{0.24\textwidth}L{0.31\textwidth}}
相对块 & 数量 & 字节容量 & 内容\\ \hline
0 & 1 & 512 B & superblock、事务、布局、feature 与 CRC\\ \hline
1--2 & 2 & 1 KiB & 8192 位 inode bitmap\\ \hline
3--4098 & 4096 & 2 MiB & 8192 个 256 B inode\\ \hline
4099--4226 & 128 & 64 KiB & data bitmap\\ \hline
4227--524287 & 520061 & 约 254 MiB & 文件数据与 pointer block\\ \hline
\end{osgridlongtable}
...
\end{pdfdiagram}
\diagramnote{图中块号从 rootfs 起点重新计数；真正 ATA LBA 还要统一加 32768。}

superblock 会把每段位置和数量写盘，但 Kernel 仍要求它们精确等于当前布局。
“自描述”让 inspector 能解释镜像、迁移器能识别版本；“严格匹配”则防止一项
损坏字段把 inode table 指向启动链、data 区或盘外。

\topic{为什么所有盘面整数都显式 little-endian}
...
\topic{四类盘面对象分别保存什么}
盘面布局由超级块、inode、目录项和指针块协同表达：一个给出全局边界，一个
描述文件身份，一个建立名称关系，最后一个扩展数据块寻址。
...
\section{大文件怎样沿多级索引找到数据块}
令 direct 数 \(D=8\)，每个 pointer block 的指针数 \(P=63\)。逻辑块号
\(L\) 的区间为：

\begin{osgridlongtable}{L{0.23\textwidth}L{0.23\textwidth}L{0.42\textwidth}}
层级 & 逻辑块数量 & 索引\\ \hline
direct & \(D\) & inode 的 \(L\) 项\\ \hline
single & \(P\) & 一个索引 \(i=L-D\)\\ \hline
double & \(P^2\) & \(i=\lfloor x/P\rfloor,\ j=x\bmod P\)\\ \hline
triple & \(P^3\) &
\(i=\lfloor x/P^2\rfloor,\ j=\lfloor x/P\rfloor\bmod P,\ k=x\bmod P\)\\ \hline
\end{osgridlongtable}
...
乘以 512 后大于 64 MiB；当前单文件上限为
\(67108864=2^{26}\) 字节。内部结构的理论余量不自动成为稳定规格，所有
\code{offset + length}、乘法与索引换算都先检查 64 位溢出和公开上限。

\begin{pdfdiagram}
  \node[os block,minimum height=32mm] (inode) at (0,1.8) {inode};

  \node[os state] (direct) at (3.0,3.6) {direct slot};
  \node[os hardware] (directdata) at (10.0,3.6) {data block};

  \node[os state] (single) at (3.0,2.4) {pointer block};
  \node[os hardware] (singledata) at (10.0,2.4) {data block};

  \node[os state] (double1) at (3.0,1.2) {pointer L1};
  \node[os state] (double2) at (6.5,1.2) {pointer L2};
  \node[os hardware] (doubledata) at (10.0,1.2) {data block};

  \node[os state] (triple1) at (3.0,0.0) {pointer L1};
  \node[os state] (triple2) at (5.3,0.0) {pointer L2};
  \node[os state] (triple3) at (7.6,0.0) {pointer L3};
  \node[os hardware] (tripledata) at (10.0,0.0) {data block};

  \draw[os arrow] (inode) -- node[above]{direct} (direct);
  \draw[os arrow] (direct) -- (directdata);

  \draw[os arrow] (inode) -- node[above]{single} (single);
  \draw[os arrow] (single) -- (singledata);

  \draw[os arrow] (inode) -- node[above]{double} (double1);
  \draw[os arrow] (double1) -- (double2);
  \draw[os arrow] (double2) -- (doubledata);

  \draw[os arrow] (inode) -- node[below]{triple} (triple1);
  \draw[os arrow] (triple1) -- (triple2);
  \draw[os arrow] (triple2) -- (triple3);
  \draw[os arrow] (triple3) -- (tripledata);
\end{pdfdiagram}
\diagramnote{direct 直接到数据；每增加一级 indirect，就增加一个由 bitmap 拥有且带 CRC 的元数据块。}

\topic{按需建立间接路径必须先知道需要几块}
...
当前尚无 journal 和 orphan list。RootFileSystem 为
...
\topic{truncate、stat 与系统调用 31--35}
共享 ABI 在末尾追加五个编号，不重排已有系统调用：

\begin{osgridlongtable}{L{0.09\textwidth}L{0.22\textwidth}L{0.56\textwidth}}
编号 & 名称 & 语义\\ \hline
31 & \code{UnlinkFile} & 删除普通文件\\ \hline
32 & \code{RemoveDirectory} & 删除空目录\\ \hline
33 & \code{Rename} & source、destination 与 replace 边界\\ \hline
34 & \code{TruncateFile} & 设置 64 位逻辑 size\\ \hline
35 & \code{StatFile} & 返回固定 64 字节 \code{FileInformation}\\ \hline
\end{osgridlongtable}
...
\topic{从 Shell 的 mv 到 ATA 的完整走读}\par
...
\begin{osgridlongtable}{L{0.17\textwidth}L{0.29\textwidth}L{0.41\textwidth}}
阶段 & 硬件事实 & 文件系统可承诺的事实\\ \hline
写 command & ATA 接受命令 & 尚不能声称扇区已持久化\\ \hline
DRQ data & 控制器接收 512 B & 仅完成数据传输阶段\\ \hline
BSY 清零 & 当前命令结束 & 设备 cache 可能仍未落盘\\ \hline
FLUSH 完成 & 之前写入越过设备屏障 & 可以进入下一持久顺序阶段\\ \hline
\end{osgridlongtable}

\topic{Dirty/Data/Clean 能检测什么}
每个修改事务具有一条不可交换的时间线：
...
\topic{独立 Python mkfs 与 fsck 为什么不能调用 Kernel}
宿主提供四个具名入口：
...
\begin{lstlisting}[caption={rootfs v2 的一次启动日志}]
...
\topic{沿一次文件读写走读源码}\par
\begin{enumerate}
  \item \filepath{root\_file\_system\_format.hpp/.cpp}：先读盘面字段与偏移；
