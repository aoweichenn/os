...
真正形成一个可从磁盘启动用户环境的系统。这里使用磁盘
\filepath{/sbin/init}、独立
ProcessTree、reader 驱动 ELF、\code{argc/argv/envp} 和
\code{spawn/exec/wait} 建立这条闭环。

\topic{上一阶段留下的最后一条“编译期脐带”}
rootfs v2 已经可以挂载 256 MiB 文件系统。名字可以经过 VFS 找到 inode，
...
这里不删除内嵌机制本身，而是缩小它的责任。正常路径只从 rootfs 加载
...
\section{为什么 Unix 把 fork、exec 与 wait 分开}
...
空间的接口，会让名称掩盖语义。因此先提供 \term{spawn}{直接从文件
...
\section{ProcessTree 怎样处理退出、wait 和孤儿}
...
失败都会递归销毁整个候选根；空 PT/PD 继续沿用页表所有权回收规则。
...
Thread；用户程序不得依赖 exec 前的 x87/SSE2 值。以后扩展 POSIX
...
\code{CloseOnExec}。前面建立的描述符模型由此第一次进入真实程序替换路径。

\topic{系统调用 ABI 如何保持固定宽度}
系统调用表在既有编号之后追加 36--38，不重排历史接口：
...
\begin{itemize}
  ...
  \item 参数全局工作区依赖不可抢占 Kernel 边界，当前只适用于单核。
\end{itemize}
...
半个 layout。调度器测试检查单 Thread exec 的提交前置条件和失败不修改旧值。
...
\begin{lstlisting}[caption={进程生命周期日志}]
...
\begin{enumerate}
  \item \filepath{source/abi/include/os/abi/system\_call.hpp}：先确定 36--38
        号、结构宽度、上限与错误值；
