...
最小环境，也接过继续初始化处理器的责任。交接点必须明确输入状态：当前模式、
...
IA-32e 兼容子模式 & 当前 CS.L=0，仍按 32 位默认方式解码 & PAE 页表、CR3、LME、PG 与当前地址映射 & LMA=1，当前 32 位检查代码仍可取指 \\ \hline
64 位子模式 & 64 位寄存器、四级页表和 L=1 代码段 & 有效 64 位代码描述符与目标地址映射 & far jump 后命中 64 位入口 \\ \hline
\end{osgridlongtable}
...
\topic{四、direct map 为什么不能代替页帧分配器}
高半区 direct map 建立
\[
