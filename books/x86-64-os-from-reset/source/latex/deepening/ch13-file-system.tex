...
校验、延迟分配和复杂恢复。这里采用固定布局和十个直接块，让一个 inode 的
盘面字节可以逐项写出，也让损坏测试能够指出究竟是哪一个字段出错。
...
\code{[2048,3072)} & 文件系统 & 1024 个固定大小逻辑块\\ \hline
...
\topic{文件系统只向块设备请求三个动作}
文件系统需要读取一个 512 字节块、写入一个 512 字节块，并在提交边界要求
此前写入稳定下来。它通过 \code{FileSystemBlockDevice} 调用
\code{ReadBlock}、\code{WriteBlock} 和 \code{Flush}，只接收成功、超时或
设备失败，不读取 ATA 状态寄存器。

宿主测试把这三个动作接到 4096 扇区的内存数组，因此可以精确翻转某个盘面
字节。目标内核则把同一接口接到真实块请求层。文件系统中的相对块先加起始
LBA，再交给接口；至于请求怎样写入 ATA 端口、怎样由 IRQ14 完成以及超时怎样
与中断竞争，都在第 26 章沿一次请求逐步展开。

\section{超级块、inode 和目录项怎样编码}
C++ 结构体适合内存中的类型检查，不适合直接成为持久格式。编译器可能插入
padding，枚举宽度和对齐规则也属于 ABI。编码器对每个多字节字段显式执行
...
创建目录、创建/截断文件和写文件会同时改变多个块。超级块中的
...
这还不是日志。它只能检测未完成事务并停止挂载，不能自动恢复。第 27 章会在
独立区域保存 redo 记录和 commit record，再根据代次重放；届时 Dirty 状态
才有确定的恢复路径。
...
路径可以到达。局部校验完成后，代码以 inode 1 为起点执行最多 32 项的
有界广度优先遍历。
...
\topic{这一盘面格式暂时没有哪些字段}
当前格式没有删除、rename、硬链接、权限、时间戳、间接块、稀疏文件、当前目录、
通用 VFS 对象、可睡眠 I/O 或日志恢复。全局文件系统锁使每次操作串行，ATA
每次只传一个扇区并轮询完成。Dirty 事务只检测，不重放。

这些限制都被类型、常量、状态和测试明确表达。下一步若加入日志，应先规定日志
