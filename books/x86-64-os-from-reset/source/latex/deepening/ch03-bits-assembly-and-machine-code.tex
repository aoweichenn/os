...
\begin{longtable}{L{0.15\textwidth}L{0.17\textwidth}L{0.23\textwidth}L{0.33\textwidth}}
\toprule
\textbf{字段} & \textbf{是否总有} & \textbf{典型内容} & \textbf{作用}\\
\midrule
\endfirsthead
\toprule
\textbf{字段} & \textbf{是否总有} & \textbf{典型内容} & \textbf{作用}\\
\midrule
\endhead
前缀 & 否 & 段覆盖、宽度、重复 & 修改默认译码或内存来源\\
操作码 & 是 & 一至数个字节 & 选择指令语义\\
ModR/M & 否 & mod、reg、r/m & 选择寄存器和寻址形态\\
SIB & 否 & scale、index、base & 表达比例索引地址\\
位移 & 否 & 8/16/32 位 & 加到有效地址或相对目标\\
立即数 & 否 & 指令内嵌常量 & 作为源操作数或掩码\\
\bottomrule
\end{longtable}
...
\section{ModR/M 把寄存器与内存形式压入一个字节}
...
\begin{longtable}{L{0.18\textwidth}L{0.23\textwidth}L{0.24\textwidth}L{0.23\textwidth}}
\toprule
\textbf{场景} & \textbf{关键标志} & \textbf{条件跳转} & \textbf{解释域}\\
\midrule
\endfirsthead
\toprule
\textbf{场景} & \textbf{关键标志} & \textbf{条件跳转} & \textbf{解释域}\\
\midrule
\endhead
相等 & ZF & \code{je/jne} & 位模式相等\\
无符号低于 & CF & \code{jb/jae} & 地址、容量、端口值\\
有符号小于 & SF 与 OF & \code{jl/jge} & 有符号整数\\
加法溢出 & OF & \code{jo/jno} & 补码范围\\
过程状态 & 项目约定 CF & \code{jc/jnc} & 汇编内部 ABI\\
\bottomrule
\end{longtable}
...
\section{汇编过程也需要清晰 ABI}

每个过程都要记录输入寄存器、输出寄存器、破坏集合、标志返回、栈使用和可重入
条件。寄存器数量有限不意味着可以隐式共享所有状态。下面的最小约定适合当前
单线程固件：

\begin{longtable}{L{0.19\textwidth}L{0.31\textwidth}L{0.38\textwidth}}
\toprule
\textbf{过程} & \textbf{输入与输出} & \textbf{主要破坏状态}\\
\midrule
\endfirsthead
\toprule
\textbf{过程} & \textbf{输入与输出} & \textbf{主要破坏状态}\\
\midrule
\endhead
UART 初始化 & 无输入；CF 返回结果 & AX、DX、计数寄存器\\
发送字节 & AL 为字节；CF 返回结果 & DX、轮询计数、标志\\
发送字符串 & CS:SI 为零结尾文本 & AX、SI、DX、计数、标志\\
\bottomrule
\end{longtable}
