...
磁盘第一个自研阶段至少包含 magic、格式版本、头长度、负载长度、加载物理地址、
入口偏移和校验。所有字段采用固定端序和宽度，解析器逐字段读取，不能把磁盘
字节直接重解释为可能带填充的 C++ 结构体。
...
\midrule
\endhead
magic & 64 位 & 精确匹配项目格式常量\\
version & 32 位 & 属于装载器支持集合\\
header size & 32 位 & 不小于必需字段且不越过镜像\\
payload size & 64 位 & 扇区换算、加法和目标区间不溢出\\
load address & 64 位 & 位于 RAM 且不覆盖固件数据\\
entry offset & 64 位 & 小于 payload 并落入可执行范围\\
checksum & 64 位或更强摘要 & 覆盖规定的头与负载字节\\
\bottomrule
\end{longtable}
