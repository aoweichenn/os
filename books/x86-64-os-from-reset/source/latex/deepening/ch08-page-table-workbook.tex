...
\topic{最小页表遍历器先返回状态，再返回物理地址}

下面的遍历器把“读取物理 64-bit 表项”作为外部函数。它只覆盖四级、4 KiB 和
2 MiB 页，不尝试支持 1 GiB 页：

\begin{lstlisting}[language=C++]
namespace os::book::paging {

constexpr std::uint64_t OS_BOOK_PAGING_ENTRY_COUNT = 512U;
...
using ReadPhysicalEntry = std::uint64_t (*)(
...
[[nodiscard]] WalkResult WalkFourLevel(
    ...
    const ReadPhysicalEntry read_entry) noexcept {
    const std::uint64_t sign =
        (virtual_address >> 47U) & 1U;
    const std::uint64_t high = virtual_address >> 48U;
    const std::uint64_t expected_high = sign == 0U ? 0U : 0xFFFFU;
    if (high != expected_high) {
        return {WalkStatus::NonCanonical, 0U};
    }

    const std::array<std::uint64_t, 4U> shifts{39U, 30U, 21U, 12U};
    std::uint64_t table =
        cr3 & OS_BOOK_PAGING_TABLE_ADDRESS_MASK;
    for (std::uint64_t level = 0U; level < shifts.size(); ++level) {
        const std::uint64_t index =
            (virtual_address >> shifts[level]) & OS_BOOK_PAGING_INDEX_MASK;
        const std::uint64_t entry = read_entry(table + index * 8U);
        if ((entry & OS_BOOK_PAGING_PRESENT) == 0U) {
            return {WalkStatus::NotPresent, 0U};
        }
        if ((entry & OS_BOOK_PAGING_PAGE_SIZE) != 0U) {
            if (level != 2U) {
                return {WalkStatus::UnsupportedLargePage, 0U};
            }
            return {
                WalkStatus::Translated,
                (entry & OS_BOOK_PAGING_LARGE_ADDRESS_MASK)
                    + (virtual_address & OS_BOOK_PAGING_2M_OFFSET_MASK)};
        }
        table = entry & OS_BOOK_PAGING_TABLE_ADDRESS_MASK;
    }
    return {
        WalkStatus::Translated,
        table + (virtual_address & OS_BOOK_PAGING_4K_OFFSET_MASK)};
}

}  // namespace os::book::paging
\end{lstlisting}

这个最小版本只计算地址。正式内核还要沿路径累计 RW、US、NX，检查保留位和
物理地址宽度，并区分“查询现有映射”和“创建缺失中间表”。
...
\begin{enumerate}[leftmargin=2.2em]
  \item 在 GDB 中执行 \code{p/x \$cr3}，记录根表物理地址；
  \item 选取日志中已知的虚拟地址，手算四级索引；
  \item 使用 QEMU monitor 的物理内存查看命令
    \code{xp/4gx physical\_address} 读取根表项；
  \item 清除表项低 12 位得到下一级表物理基址，重复到叶项；
  \item 把页帧基址与偏移相加，再用 monitor 查看该物理地址的 byte；
  \item 用 \code{monitor info tlb} 或 \code{monitor info mem} 辅助观察，
    但最终索引与地址仍要在纸上独立计算。
\end{enumerate}
