...
v0.9 的首个实现让每个槽位拥有 20 KiB 编译期连续存储：底部 4 KiB 保持
...
\begin{pdfdiagram}[node distance=11mm and 15mm]
  \node[os state] (unused) {Unused};
  \node[os block,right=of unused] (ready) {Ready};
  \node[os state,right=of ready] (running) {Running};
  \node[os hardware,right=of running] (terminated) {Terminated};
  ...
  \draw[os arrow] (unused) -- node[above]{创建} (ready);
  \draw[os arrow] (ready) -- node[above]{派发} (running);
  \draw[os arrow] (running) to[bend left=32] node[above]{抢占} (ready);
  \draw[os arrow] (running) -- node[right]{条件不满足} (blocked);
  \draw[os arrow] (blocked.west) to[out=180,in=-90]
    node[below left]{条件变化} (ready.south);
  \draw[os arrow] (running) -- node[above]{退出/异常} (terminated);
\end{pdfdiagram}
...
\topic{Try 与 Wait 分离使挂起的系统调用现场没有隐式副作用}
...
\begin{osgridlongtable}{L{0.25\textwidth}L{0.24\textwidth}L{0.39\textwidth}}
状态 & 操作结果 & 条件语义\\ \hline
\(B>0\) & 读部分或全部字节 & 消费数据并可能释放写空间\\ \hline
\(B=0\)，写端开放 & WouldBlock & 未来仍可能写入\\ \hline
\(B=0\)，写端关闭 & EOF，返回 0 & 未来不再有字节\\ \hline
\(B<64\)，读端开放 & 写部分或全部字节 & 提交数据并可能满足读取\\ \hline
\(B=64\)，读端开放 & WouldBlock & 等待消费者释放空间\\ \hline
读端关闭 & BrokenPipe & 未来无人消费写入\\ \hline
\end{osgridlongtable}
...
成功写入后运行时累计当前 PCB 的 \code{pipe_bytes_written}，成功读出后累计
\code{pipe_bytes_read}。这两项和管道全局统计从不同所有权视角证明同一事实：
生产者只能写 256、消费者只能读 256，两个调度 worker 两项都为零。

\topic{端点关闭改变等待谓词，所以关闭本身也必须唤醒}
...
\topic{三层测试把互斥、状态机与真实 Ring 3 行为分开证明}
...
\begin{pdfdiagram}[node distance=11mm and 13mm]
  ...
  \node[os state,right=of running] (zombie) {zombie};
  \node[os hardware,right=20mm of zombie] (reaped) {reaped};
  \node[os block,below=of running] (blocked) {blocked};
  \draw[os arrow] (created) -- (runnable);
  \draw[os arrow] (runnable) -- node[above]{调度} (running);
  \draw[os arrow] (running) to[bend left=32] node[above]{抢占} (runnable);
  \draw[os arrow] (running) -- node[right]{等待} (blocked);
  \draw[os arrow] (blocked.west) to[out=180,in=-90]
    node[below left]{唤醒} (runnable.south);
  \draw[os arrow] (running) -- node[above]{退出} (zombie);
  \draw[os arrow] (zombie) -- node[above]{父进程回收} (reaped);
\end{pdfdiagram}
...
单次运行看不出每轮泄漏一个对象的问题。重复运行以后，三个计数仍应回到开始
时的数值；只要其中一个逐轮增长，回收路径就还漏了一步。
