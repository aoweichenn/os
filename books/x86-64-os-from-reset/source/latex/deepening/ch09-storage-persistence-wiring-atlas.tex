...
\subsection{一、先区分“可见”“已写回”和“持久”}

\begin{longtable}{L{0.20\linewidth}L{0.29\linewidth}L{0.31\linewidth}L{0.10\linewidth}}
\toprule
结论 & 已经证明 & 还没有证明 & 强度 \\
\midrule
write 返回成功 & 内核接受请求并按接口完成当前提交点 & 数据已离开全部 volatile cache & 低 \\
同次启动读回相同 & 当前内核路径能从缓存或磁盘返回字节 & 重启后仍存在 & 较低 \\
ATA WRITE 完成 & 控制器接受/处理数据传输 & 设备内部易失缓存已稳定落介质 & 中 \\
FLUSH 成功 & 设备按当前 ATA 协议完成缓存刷新 & 新启动的 mount/读取逻辑也正确 & 较高 \\
新 QEMU 进程读回 & 旧进程和所有 Guest RAM 已消失，镜像重新挂载后字节一致 & 任意断电点都有日志级原子恢复 & 当前最强 \\
\bottomrule
\end{longtable}

\term{持久性}{persistence}表示状态在产生它的执行实例结束后仍可恢复；
\term{耐久性}{durability}通常强调已经承诺成功的数据在规定故障模型下不会
丢失。当前项目证明正常 FLUSH 后跨 QEMU 进程保存，不声称覆盖写入任意一半时
突然断电的所有崩溃一致性；后者需要日志、写序和恢复协议继续约束。

\subsection{二、步骤 1--3：用户地址必须在边界上重新验证}
...
\subsection{三、步骤 4：fd 不是 inode，也不是磁盘地址}
...
\subsection{四、步骤 5：文件系统把名字和 offset 变成磁盘块}
...
\subsection{五、步骤 6：block cache 为什么需要 dirty/clean 状态}
...
\subsection{六、步骤 7--8：ATA PIO 把一个扇区变成 256 个 word}
...
\subsection{七、步骤 9：raw disk 是镜像容器，不等于文件系统}
...
\begin{longtable}{L{0.22\linewidth}L{0.22\linewidth}L{0.25\linewidth}L{0.21\linewidth}}
\toprule
LBA 半开区间 & 大小 & 所有者 & 内容 \\
\midrule
\([0,2048)\) & 1 MiB & 启动镜像布局 & Stage 1、Kernel 容器/ELF 与保留结构 \\
\([2048,3072)\) & 512 KiB & 文件系统 & 1024 个当前文件系统块 \\
\([3072,4096)\) & 512 KiB & 明确保留 & 不允许 Kernel 镜像或文件系统越界占用 \\
\bottomrule
\end{longtable}
...
自 v1.6 起镜像已扩为逻辑 1 GiB 稀疏文件，并从 LBA 2048 起分配固定
256 MiB rootfs；本表保留为第一版持久格式的历史基线，详细新布局见本章
后续 rootfs v2 节。

\subsection{八、步骤 10：为什么必须终止旧 QEMU}
...
\subsection{九、把 QEMU ATA 换成实体 NVMe 时哪些层保持}
...
\subsection{十、失败现象怎样对应到十步链}

\begin{longtable}{L{0.25\linewidth}L{0.31\linewidth}L{0.34\linewidth}}
\toprule
现象 & 最可能边界 & 应保存的证据 \\
\midrule
write 立即返回参数错误 & 用户复制、fd 类型/权限或范围检查 & syscall 号、fd、长度、精确错误码 \\
同次读回正确、重启丢失 & dirty cache、ATA WRITE/FLUSH 或镜像绑定 & dirty/clean 计数、状态位、FLUSH 标记 \\
重启后自动格式化 & mount 的全零/损坏分类错误 & superblock 原始字节、CRC、禁止标记 \\
小文件正常、大文件破坏启动 & block/LBA 边界或长度溢出 & 目标半开区间、镜像审计、受影响 LBA \\
第二次 QEMU 读到旧内容 & 旧进程未终止、磁盘文件错误或 flush 未完成 & 进程边界、镜像路径/hash、完成标记 \\
随机卡在 ATA & BSY/DRQ/ERR/DF 轮询与预算 & 最后状态、命令、LBA、剩余 timeout \\
\bottomrule
\end{longtable}
