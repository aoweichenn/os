...
LAPIC LVT，因此内核显式建立 virtual wire。内存管理器先从当前
...
\begin{pdfdiagram}
  ...
  \node[os state,below=of runtime] (main) {kernel\_main\\有界日志与检查};
...
模拟时序。内核在 IF 开放前执行 \code{INT 0x27}：软件中断进入同一 IDT
...
无条件开启会扩大可重入区域。\code{DisableInterrupts} 返回旧 IF，
...
\topic{第一次 ATA 读取为什么仍然使用轮询}
固件和 Stage 1 已有 ATA PIO，但内核不能调用它们的私有例程或依赖遗留寄存器
状态。C++ 驱动独立选择 primary master LBA28，向 device control
...
捕获生命周期不能把某一台宿主机的速度写成来宾条件。最初固定等待两秒再检查，
...
组合。完整回归还会运行 ROM、ATA、长模式、ELF、异常、内存与失败路径，
确认新增异步状态没有破坏旧的确定性行为。
...
装载器只接受最多八个 \code{PT\_LOAD}，对每个 64 位字段执行以下检查：
...
U/S=1 而上级任一项 U/S=0，用户访问仍会故障。页表管理器在建立用户
...
内核为 RSP0 分配独立 16 KiB 特权转换栈，底端另有 4 KiB not-present
...
此时内核已经拥有经过测试的 IDT、统一寄存器帧与 TSS，所以把 IDT
...
缓冲也可能跨越 user 页和 supervisor/not-present 页。
...
另一线程并发解除映射，所以当前检查与复制之间不存在可触发的 TOCTOU；
...
它表示“用户读 not-present”，与内核早期自检使用的 \code{0x3}
...
经过自研 ROM、Stage 1、长模式、Kernel ELF 与内核初始化。调度、锁、
阻塞/唤醒和管道测试加入后，这些早期启动路径仍作为回归保留。

\topic{单一用户程序怎样演化为调度运行时}
初次进入用户态时只有一个同步执行单元，全局保存一个原内核 RSP，所有用户页位于
当前内核页表根。用户结束后页面不回收，也没有 PID、进程状态或上下文切换。
加入多进程以后，这些限制分别落到具体对象：
...
下一章沿这份清单加入独立 PML4、每进程 Ring 0 栈、PIT
