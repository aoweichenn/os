...
收尾并主动回收 QEMU；未到达才等待五秒总截止。QMP 线程等待 \code{READY}
...
并继续运行之前的 ROM、ATA、长模式、ELF、异常、内存与失败路径。阶段完成
...
\begin{longtable}{L{0.10\textwidth}L{0.15\textwidth}L{0.28\textwidth}L{0.33\textwidth}}
\toprule
槽 & 选择子 & 类型 & 关键属性\\
\midrule
0 & --- & null & 永不引用\\
1 & \code{0x08} & Ring 0 code & DPL0、execute/read、L=1\\
2 & \code{0x10} & Ring 0 data & DPL0、read/write\\
3 & \code{0x1B} & Ring 3 data & DPL3、read/write、RPL3\\
4 & \code{0x23} & Ring 3 code & DPL3、execute/read、L=1、RPL3\\
5--6 & \code{0x28} & 64-bit TSS & 104 字节基址与界限\\
\bottomrule
\end{longtable}
...
\section{严格用户 ELF 把文件声明转换为可执行布局}
...
\begin{longtable}{L{0.31\textwidth}L{0.17\textwidth}L{0.38\textwidth}}
\toprule
虚拟范围 & 权限 & 用途\\
\midrule
\code{0..0xFFFF} & 非用户范围 & 捕获空指针与低地址错误\\
\code{0x40000000...} & ELF 决定 RX、R/NX、RW/NX & 用户代码、常量和数据\\
\code{0x00007FFFFFFEA000} & not-present & 用户栈 guard\\
\code{0x00007FFFFFFEB000..EFFFF} & user RW/NX & 四个用户栈页\\
\code{0x00007FFFFFFF0000} & 初始端点 & RSP 指向其下方已映射字节\\
\bottomrule
\end{longtable}
...
\section{五项 IRETQ 帧让处理器真正进入 CPL3}
...
\section{TSS.RSP0 必须与等待返回的内核栈分离}
...
\section{选择 INT 0x80 是控制学习变量，不是忽略现代指令}
...
\begin{longtable}{L{0.10\textwidth}L{0.22\textwidth}L{0.30\textwidth}L{0.22\textwidth}}
\toprule
编号 & 名称 & 参数 & 返回\\
\midrule
1 & \code{WriteLog} & RDI=地址，RSI=长度 & 写入字节数或负错误\\
2 & \code{ExitProcess} & RDI=有符号退出码 & 不回到用户态\\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  ...
  \item \filepath{source/kernel/src/user\_memory.cpp} 映射段、清零 BSS、
        建栈并负责失败回滚；
  \item \filepath{source/kernel/src/architecture.asm} 保存原内核调用链，
        执行降权和终止恢复；
  \item \filepath{source/user/src/system\_call.asm} 执行
        \code{INT 0x80}；
  \item \filepath{source/kernel/src/system\_calls.cpp} 校验帧、复制用户数据
        并分发调用；
  \item \filepath{source/kernel/src/exceptions.cpp} 按 CPL 选择用户终止或
        内核 panic；
...
\section{四类用户路径构成可反驳的验收证据}

\begin{longtable}{L{0.18\textwidth}L{0.34\textwidth}L{0.34\textwidth}}
\toprule
路径 & 必须观察 & 必须禁止\\
\midrule
正常程序 & 非法指针拒绝、未知号拒绝、Ring 3 文本、exit 0、六次调用、
恢复内核、IRQ 与 READY & 用户结果非法、panic\\
用户 \#UD & vector 6、error 0、零次调用、恢复内核、READY &
内核 EXCEPTION/PANIC\\
用户 \#PF & vector 14、error 4、CR2=\code{0x30000000}、恢复内核、
READY & 内核 EXCEPTION/PANIC\\
截断 ELF & validation status 2，降权前停止 & 设备启动、Ring 3、READY\\
\bottomrule
\end{longtable}
...
经过自研 ROM、Stage 1、长模式、Kernel ELF 与内核初始化。完整集合为
55 项 CTest，并保留 v0.0--v0.7 的所有回归。

\section{v0.8 的边界为调度阶段保留真实问题}

当前只有一个同步用户执行单元，全局只保存一个原内核 RSP，所有用户页仍位于
当前内核页表根。用户结束后页面尚未回收，也没有 PID、进程状态、等待队列或
上下文切换。这些不是隐藏的“以后优化”，而是 v0.9 必须新增并验证的所有权：

\begin{itemize}
  \item 每进程地址空间与用户页回收；
  \item 每线程独立内核栈和可重入的保存现场；
  \item ready/running/blocked/zombie 状态机；
  \item PIT 触发的抢占与寄存器上下文切换；
  \item 系统调用、异常、退出与调度器之间的锁和生命周期。
\end{itemize}
