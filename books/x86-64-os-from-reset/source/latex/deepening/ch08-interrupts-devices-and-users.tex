...
慢速 CI runner 曾在 \code{INTERRUPTS\_ENABLED} 后结束进程；这只能说明宿主
墙钟预算耗尽，不能单独说明 PIC 或 PIT 错误。当前捕获器观察每个用例最后一个
必需里程碑：到达后短暂收尾并主动回收 QEMU；未到达才等待五秒总截止。QMP
线程等待 \code{READY} 也使用同一上界。结束后仍完整检查所有必需标记的顺序和
禁止标记，因此放宽的是宿主调度假设，不是来宾正确性。
