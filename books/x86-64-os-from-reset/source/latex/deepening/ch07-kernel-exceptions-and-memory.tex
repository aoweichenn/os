...
迁移后诊断为零。加入 buddy 与类型缓存测试后，当前门禁扫描 145 个文件，并在每次
...
空中间页表回收仍是 v1.1 后续闭环，而不是 heap 接口暗中承担的能力。固定
尺寸 type cache 已经作为下一层独立机制建立，下面沿其历史、布局和状态机
继续展开。
...
本项目不能因为最终方向类似 SLUB，就一次复制其全部复杂度。当前 v1.1 尚未
完成 KVA，也尚未冻结 Thread/IRQ 锁语义；若现在引入多 slab 增长和 per-CPU
缓存，页后备、地址空间、并发与对象析构的错误会混在一起。本增量因此选择
一个固定容量、一个 heap 后备块、可以整体销毁的最小闭环。
...
v1.1 后续先实现 KVA：从高半区分配虚拟区间，再由 buddy 提供物理页并建立
映射。届时一个类型缓存可以拥有多个页后备 slab；空 slab 能撤销映射、归还
物理页和 KVA。动态内核栈使用同一 KVA/页后备事务，但额外保留 not-present
guard。页表回收再保证撤销映射不会永久泄漏中间表。
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 固定种子 \code{0x6D656D6F72793634} 完成 8192 组叶表项往返，
        ...
        分配并立即回收；独立 heap 与 type cache 种子分别再执行 100000 步
        字节块和固定槽申请/释放；
