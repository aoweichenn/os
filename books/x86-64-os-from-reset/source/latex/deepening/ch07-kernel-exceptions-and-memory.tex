...
当前 \code{UnmapPage} 清除叶项并执行 \code{INVLPG}，尚不回收变空的 PT、
PD 和 PDPT。KVA 自检先用一个虚拟页和一个 order-0 物理页预热首条页表路径，
撤销叶映射并归还两种可回收资源；三页中间表作为已知基础设施基线保留。主事务
随后复用这条路径，因而能够严格要求自身前后的物理页、buddy 和 KVA 统计完全
相等，而不会把已知页表生命周期边界误报成数据页泄漏。
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 比较三层基线，并运行 buddy 与 KVA 的完整校验。
\end{enumerate}
...
当前边界仍然明确：最多 256 个有主区间、操作为 \(O(N)\)、没有内部锁，
中间页表尚不回收。动态内核栈已经成为第一个长期持有 KVA、物理帧和叶映射的
真实调度资源；下一资源闭环是释放随最后一个叶映射消失而变空的 PT、PD 和
PDPT。性能树、per-CPU KVA cache 和延迟 shootdown 必须等并发契约与测量
证据出现后再加入。
...
管理器有 256 个长期槽，容量与 KVA 描述符容量显式分开。槽索引当前等于 PCB
索引只是调用方策略；栈管理器不理解 PID、调度状态或用户地址空间。统计记录：
...
回滚本身也可能失败，因此接口区分原始
\code{FrameAllocationFailed}/\code{PageMappingFailed} 与
...
\code{KernelStackManager::Validate} 不只比较计数。它遍历全部 256 槽并
重新证明：
...
KVA 页数恢复暖机基线；创建、销毁均为 4，峰值活动栈为 4，峰值映射页为 16。
...
这些日志在准备阶段、每进程创建结果和最终聚合点输出，不在页申请、映射、
时钟中断或回收循环中逐步输出。宿主 QEMU runner 继续给每行添加单调时间；
来宾使用 PIT 单调 tick 表达自己的时间域。丰富日志在这里意味着关键状态可
关联、计数可守恒，而不是把高频事件冲进 115200 波特串口。

\subsection{当前完成边界与下一资源问题}

动态栈已经消除了编译期四份 20 KiB 进程栈存储，四个 PCB 只保留管理器槽索引
与保存帧引用。容量仍由 256 槽固定存储限定，管理器仍是单 BSP、无内部锁；
栈大小固定 16 KiB，不支持按需增长，也没有每 CPU emergency stack 或
stack canary。下一个资源闭环不是立刻增加这些策略，而是回收空 PT、PD 和
PDPT，并用页表帧统计证明动态栈反复创建/销毁不会永久增长中间表基础设施。
...
NMI 或 panic 中调用。buddy 与 KVA 已由前文的独立页层和虚拟区间层承担；
动态 guard stack、按页增长 heap 和空中间页表回收仍是后续闭环，而不是 heap
接口暗中承担的能力。固定尺寸 type cache 已经作为下一层独立机制建立，下面
沿其历史、布局和状态机继续展开。
...
当前缓存容量初始化后不变。它没有空/部分/满 slab 列表，也不能只归还一部分
后备页；后备仍受 64 KiB heap 总容量约束。这足以验证 v1.2 动态对象所需的
基本槽所有权，却不是大规模内核的最终分配策略。

KVA 已经能够从高半区分配虚拟区间，并与 buddy、页表组成经过整机验证的跨层
事务。下一步让类型缓存拥有多个按页后备 slab；空 slab 才能撤销映射、归还
物理页和 KVA。动态内核栈使用同一 KVA/页后备事务，但额外长期保留
not-present guard。页表回收再保证撤销映射不会永久泄漏中间表。

v1.2 的 Process/Thread 对等路径通过后，固定 PCB 才迁入动态缓存；v1.4
进一步把引用计数 KernelObject 和动态 FileDescription 建立在类型缓存之上。
这一顺序保留一条重要工程原则：基础设施可以先存在，旧生产路径必须在新对象
模型拥有对等测试之前继续工作，不能为了“看起来已经动态化”制造不可启动的
中间版本。
