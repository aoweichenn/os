...
内核当前管理低 64 MiB：
\[
 \frac{64\ \mathrm{MiB}}{4\ \mathrm{KiB}}=16384\ \text{frames}.
\]
每帧不用一位，而用两位表示四种状态：
...
状态存储大小为：
\[
  16384\times2/8=4096\ \mathrm{bytes}.
\]
这里使用 \code{uint8\_t} 数组并不是违背内部大位宽规范；密集状态的每字节
恰好保存四帧，扩大为 64 位会把元数据增加八倍。对外地址、计数、容量和索引
仍全部使用 64 位。

初始化先把所有状态字节清零，对应 unavailable，再把可用区间按 4 KiB 向内
收缩。若区域起点不是页边界，向上对齐；终点向下对齐。只有整个页都被平台声明
为可用时才进入 free。全部扫描后仍没有 free 帧，分配器把 managed count
恢复为零再返回 \code{NoUsableFrames}，不会留下“看似初始化”的对象。
...
\begin{itemize}
  ...
  \item 链接器符号 \code{osKernelImageStart..osKernelImageEnd}：精确覆盖
        text、rodata、data、BSS、IDT、IST 和页帧状态；
...
并清零。这个临时便利必须写进前置条件：一旦未来取消大范围身份映射，
\code{tableAtPhysicalAddress} 就要改用物理直映区或临时映射窗口。

内核先分配根 PML4，再对低 64 MiB 的每个 4 KiB 页执行映射。中间表缺失时，
从同一个物理分配器取得一帧，清零 512 个 64 位项，再写入 parent。整个
64 MiB 恒等映射需要若干 PT，而不是预先写死表数量；高半区 heap 会共享
同一个 PML4 却走另一组索引。
...
中间表不存在时分配并清零；存在时验证 PS 大页位没有意外置一，再用
\code{0x000FFFFFFFFFF000} 取下一级物理地址。叶项写入前拒绝重复映射。
物理和虚拟地址都必须 4 KiB 对齐，物理地址还不能越过表项可编码范围。
...
\begin{enumerate}[label=\arabic*.]
  \item 单元测试验证内存图失败枚举、2-bit 状态迁移、heap 原子失败、
        canonical 地址、四级索引和表项权限编码；
  \item 集成测试按 QEMU 的低端 RAM/高端保留形状建立内存图，检查低 1 MiB、
        Kernel、栈保留后第一个可分配帧，并检查交接物理区间不重叠；
  \item 固定种子 \code{0x6D656D6F72793634} 完成 8192 组叶表项往返，
        再执行 4096 步随机 allocate/release，与独立布尔模型逐步比较；
  \item QEMU 正常路径真实加载 CR3、访问 heap；失败路径分别证明
        \code{MEMORY\_MAP\_INVALID} 和错误码 3 的写保护 \#PF。
\end{enumerate}
