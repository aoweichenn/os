...
迁移后诊断为零。第 77 项 CTest 会在每次完整回归中重新执行同一检查，使命名
规范从说明文字变成可失败的工程契约。
...
表现为耗尽。v1.1 因而先建立通用字节块的申请、释放和守恒，再引入 buddy、
KVA 与类型缓存；每层只回答一种所有权问题。
...
NMI 或 panic 中调用。buddy、KVA、动态 guard stack、空中间页表回收和
type cache 仍是 v1.1 后续独立闭环，而不是本次接口暗中承担的能力。
