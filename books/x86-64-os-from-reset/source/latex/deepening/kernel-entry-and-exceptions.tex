...
\begin{itemize}
  ...
  \item Ring 3 到 Ring 0 的控制转移依赖特权级和 RSP0。
\end{itemize}

Stage 1 的 GDT 只负责模式切换。内核随后用 BSS 中的新表覆盖
...
旧属性。装载新表后用 CS、SS 和 \code{STR} 回读分别得到
\code{0x08}、\code{0x10}、\code{0x18}。

这套表先只服务 Ring 0。进入用户态前，还要在 Ring 0 数据段之后插入用户数据段与用户
...
每个 IST 存储块占 20 KiB：低端 4 KiB 保持 guard，随后 16 KiB
...
\textbf{向量} & \textbf{名称} & \textbf{类别} & \textbf{错误码} & \textbf{当前处理}\\ \hline
\endfirsthead
\textbf{向量} & \textbf{名称} & \textbf{类别} & \textbf{错误码} & \textbf{当前处理}\\ \hline
\endhead
...
表中未列出的保留向量也有独立桩并进入 panic。这里先只允许 breakpoint 返回，
因为其他 fault 尚无足够策略保证恢复安全。例如
页不存在可以在内存管理器分配并映射后重试；非法指令通常意味着代码或 CPU 能力
契约错误，跳过一个字节既不知道指令长度，也会掩盖破坏。

\topic{分别触发 INT3、UD2 和页故障}
单一正常启动不能覆盖异常系统。测试执行三条具有不同语义的真实指令路径：
...
字符串或获取日志锁，会扩大它必须信任的系统。panic 路径只保留：
