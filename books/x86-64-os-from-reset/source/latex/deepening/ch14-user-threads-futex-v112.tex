...
\subsection{历史动机：共享资源为什么不等于共享执行现场}
...
\begin{center}
\begin{tabular}{L{0.27\linewidth}L{0.27\linewidth}L{0.34\linewidth}}
\toprule
事实 & 所有者 & 不共享的原因或共享收益 \\
\midrule
页表根、VMA 与后备 & Process & 所有 Thread 观察同一组用户对象 \\
FileTable 与 FsContext & Process & fd、共享 offset 和 cwd 形成同一环境 \\
PID 与父子关系 & Process & wait 消费的是程序实例的终止记录 \\
RIP/RSP/RFLAGS & Thread & 每个执行流停在不同指令和调用深度 \\
通用/FXSAVE 现场 & Thread & 整数、x87、MMX、XMM 状态不能串用 \\
内核栈与用户栈 & Thread & 特权入口和普通调用都需要独立生存区间 \\
TID、等待节点与退出值 & Thread & 调度、join 和同步以执行流为单位 \\
FS-base TLS & Thread & 同一条机器指令要定位不同私有实例 \\
\bottomrule
\end{tabular}
\end{center}

Linux 后来用可组合的共享标志构造任务；POSIX pthread 又为应用提供可移植
接口。本项目不先复制两套庞大兼容层，而是在 v1.2 已分离的 Process/Thread
内核模型上冻结最小公开 ABI。这样每个新增字段都有硬件或所有权原因，而不是
为了让接口名字看起来像现成系统。

\subsection{一条 Thread 从用户请求到 Ready 发布}
...
\begin{center}
\begin{tabular}{L{0.30\linewidth}L{0.19\linewidth}L{0.39\linewidth}}
\toprule
字段 & 宽度 & Kernel 要证明的性质 \\
\midrule
entry address & 64 bit & 用户规范地址，位于可执行且不可写 VMA \\
argument & 64 bit & 原样放入初始 RDI，不由 Kernel 解引用 \\
stack base/size & 各 64 bit & 页对齐、至少 16 KiB、读写且 NX、无溢出 \\
stack pointer & 64 bit & 精确位于栈顶减 8，满足入口 ABI \\
TLS base & 64 bit & 16 字节对齐，当前地址空间内可写 \\
\bottomrule
\end{tabular}
\end{center}
...
一经 Ready 就可能先运行；子入口必须调用 \code{GetThreadId} 自行发布身份。
线程创建给出的是并发关系，不是“父返回必然先于子入口”的顺序保证。
\end{failurebox}

\subsection{FS-base：长模式为什么仍保留一小块分段}
...
自身不使用 FS，因此 v1.12 的 SYSCALL、IRET 和调度汇编路径不再重载 FS；
真正切换 Thread 时由架构包装写 \code{IA32\_FS\_BASE} 并读回验证。

\subsection{为什么仅靠自旋不能形成通用用户 Mutex}
...
路径 & 操作 & 成本与语义 \\
\midrule
无竞争 Lock & CAS \(0\rightarrow1\), acquire & 不进入 Kernel \\
竞争 Lock & wait(word, expected \(=1\)) & Thread 进入 WaitQueue \\
Unlock & release store 0，wake one & 发布临界区写入并允许一个竞争者重试 \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{compare-and-block 封闭丢失唤醒窗口}
...
\subsection{private futex 的 key 为什么不是虚拟地址或物理页}
...
\subsection{ConditionVariable 保存变化序列而不是业务条件}
...
等待者 & 通知者 \\
\midrule
持 Mutex 读取 sequence & 修改受保护谓词 \\
释放 Mutex & sequence 加一，release \\
futex wait(sequence, old) & wake one 或 wake all \\
重新取得 Mutex & 返回继续工作 \\
循环复查谓词 & 不替等待者判定谓词 \\
\bottomrule
\end{tabular}
...
v1.12 明确冻结、等待以后扩展的失败语义。

\subsection{ExitThread、Join 与 ProcessExit 是三种回收边界}
...
join 为目标冻结唯一 join owner。目标退出后，Kernel 在安全点释放其
...
\subsection{多线程 exec 为什么先验证候选、再停止 sibling}
...
\subsection{munmap 必须认识活动 Thread 和 futex waiter}
...
\subsection{容量不是一个孤立数字}

三档 QEMU 运行同一实现，只改变资源配置和探针目标：

\begin{center}
\begin{tabular}{L{0.25\linewidth}L{0.20\linewidth}L{0.23\linewidth}L{0.22\linewidth}}
\toprule
档位 & RAM & Thread 规格 & thread probe \\
\midrule
bootstrap & 64 MiB & 每 Process 1 & 明确验证不能再创建 \\
functional & 256 MiB & 系统 128、每 Process 32 & 总计 32 条同时 Ready \\
capacity & 64 GiB & 系统 512、每 Process 64 & 总计 64 条并验证上限 \\
\bottomrule
\end{tabular}
\end{center}
...
容量验收既检查“能创建目标数量”，也检查 join 后活动 Thread、栈槽、futex
entry、VMA、页表和 frame 回到基线。

\subsection{测试怎样分别证明局部算法与真实硬件路径}
...
ABI 集成测试冻结系统调用 47--53、48 字节创建请求、16 字节 join 结果和
...
147 项 CTest 是 v1.12 构建图的当前结果，不是以后不得变化的魔法规格。冻结的
是测试层次、随机可复现、QEMU 总截止/静默截止、64 GiB 发布门禁，以及成功
摘要中创建、退出、等待、唤醒和回收必须守恒。
...
\begin{contractbox}
不看源码回答：为什么 Ready 必须最后发布；为什么子 Thread 可以在父
\code{CreateThread} 返回前运行；为什么 private futex key 不能只用 VA 或
frame；为什么 ConditionVariable 保存 sequence 仍要求循环检查谓词；为什么
exec 验证失败不能终止 sibling；以及怎样用 Thread、WaitQueue、futex entry、
VMA、KernelStack 和 frame 六组统计证明 join 没有泄漏。
\end{contractbox}
