\section{系统调用怎样找到当前 CPU 和 Thread}

第一次进入 Ring 3 时，项目选择 \code{INT 0x80}。此时
...
\begin{pdfdiagram}
  \node[os state] (user) {Ring 3\\RIP/RFLAGS/RSP};
  \node[os block,right=of user] (cpu) {SYSCALL 硬件动作\\
  RCX\(\leftarrow\)RIP\\R11\(\leftarrow\)RFLAGS};
  \node[os block,right=of cpu] (target) {STAR/LSTAR/FMASK\\
  改 CS、SS、RIP、RFLAGS\\RSP 不变};
  \node[os block,below=of target] (prefix) {入口汇编\\SWAPGS\\保存用户 RSP};
  \node[os state,left=of prefix] (stack) {可信内核栈\\规范化 UserContext};
  \node[os block,left=of stack] (cpp) {C++ 分发器};
  \draw[os arrow] (user) -- (cpu);
  \draw[os arrow] (cpu) -- (target);
  \draw[os arrow] (target) -- (prefix);
  \draw[os arrow] (prefix) -- (stack);
  \draw[os arrow] (stack) -- (cpp);
\end{pdfdiagram}
\diagramnote{依据 Intel SDM Vol. 3A 6.8.8 重绘。手册明确指出 SYSCALL
不会保存或切换 RSP；下半段的 SWAPGS、用户 RSP 暂存和内核栈选择来自
\filepath{source/kernel/src/arch/architecture.asm}。}

\code{syscall} 不是“更快的 \code{int 0x80}”。它把换栈、每 CPU 状态、统一
保存现场和安全返回从硬件门描述符移交给内核。Thread 的动态内核栈、当前栈顶
和回收时刻确定以后，才能安全地把换栈动作交给这条新入口。
...
\begin{osgridlongtable}{L{0.15\textwidth}L{0.29\textwidth}L{0.19\textwidth}L{0.23\textwidth}}
...
建立的第五级。因此 profile 当前要求 linear width 等于 48。未来开放
...
\code{CpuLocal} 按缓存线大小 64 字节对齐。前四个字段是 C++ 与汇编
共同遵守的 ABI：
...
\code{CpuPreemptionGuard} 构造时增加深度，析构时减少；增加失败时守卫保持
未取得状态。
它没有让内核自动变成 SMP-safe，只把当前单 BSP 的合法调度点变成可检查状态。

\section{SYSCALL 入口怎样建立内核现场}
...
\begin{lstlisting}[caption={原生入口的一次代表性统计}]
...
\topic{沿目录走读 CPU-local 与系统调用}\par
...
\code{INT 0x80} 继续作为兼容入口，何时移除由公共 ABI 的兼容要求决定。

此时系统调用背后的描述符仍是固定槽。后面的类型化 KernelObject、动态
FileTable 和共享 FileDescription 只修改 C++ 对象层，没有改写汇编入口、
UserContext 和返回验证。对象表示可以继续变化，跨特权边界仍沿同一条路径。

运行 QEMU 时应当看到兼容与原生入口都被使用，至少一次 IRQ 在系统调用期间
到达，SYSRET 和 IRET fallback 计数都非零，非法用户返回计数为零。关闭
SYSCALL 能力的 CPU 配置则必须在写 MSR 以前停止。两组结果共同检查能力探测、
可信换栈、统一现场和返回白名单。
