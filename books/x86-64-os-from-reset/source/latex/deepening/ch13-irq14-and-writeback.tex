...
其他 Ready Thread 无法获得本应可用的 CPU。因此这里保留两种适配器：
...
PIC 初始化后全屏蔽。接入磁盘中断后的最终掩码是 \code{0xBFF8}：
...
必须在复制终态结果后显式 Reap，槽位才重新成为 Unused。生产 ATA 适配器在
...
取消、设备撤回和 restart ABI 尚未定义，当前实现不返回这类结果。
...
x86 没有“每次 store 都调用 Kernel”的页模式。当前使用 write-protect
notification：
...
shared alias 便不再共享。rootfs 集成测试专门检查这一条。

FLUSH 完成只说明当前设备模型报告先前写入已越过其 cache 边界；下一章还需
定义数据、metadata、journal descriptor 和 commit record 的顺序。没有
journal 时，文件系统仍只能在 Dirty superblock 后拒绝未完成事务，不能自动恢复。
...
\begin{itemize}
  ...
  \item rootfs 尚无 journal/replay；下一章才加入 credits、commit 与断电矩阵；
  \item 单 BSP 下完成状态正确，不宣称已经处理跨 CPU completion 与 cache coherence。
\end{itemize}

这里先让请求只能完成一次，让脏页在写回失败后仍然可见，并通过 FLUSH 等待
设备报告稳定。下一章再讨论多块元数据事务怎样在断电后只
恢复为完整旧状态或完整新状态。
