...
完整回归共 92 项 CTest，其中可回收堆、buddy、type cache、KVA、动态
内核栈和页表空分支都拥有单元、集成和固定种子生命周期证据，资源随机状态机
各执行 100000 步；页表模型额外推导三级表数并验证映射失败回滚，动态栈
集成测试还从独立进程 CR3 查询共享高半区映射。三条用户 QEMU 路径都要求
最终创建数等于销毁数且活动栈归零；
