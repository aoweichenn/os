...
\topic{没有 Ready Thread 时 CPU 为什么执行 HLT}
...
\end{osgridtabularx}
\caption{单调时间与 deadline 的固定宽度用户 ABI}
\end{table}

超时结果使用 \code{-51}。接口没有 \code{time\_t}、\code{long} 或平台
...
一千赫兹 IRQ 若每次写串口，会把 PIO 延迟注入被测调度时序。内核只在用户
事务完成和系统回收边界打印：
...
后面的 signal 会使 condition、deadline 与异步通知成为三方竞争。signal
