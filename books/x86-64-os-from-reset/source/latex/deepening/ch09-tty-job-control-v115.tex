...
关系 & 公开身份 & 解决的问题\\
\midrule
进程树 & PID 与 parent PID & 谁 wait、谁收养 Zombie\\
进程组 & PGID & 哪些兄弟进程组成一个作业和信号目标\\
会话 & SID & 哪些进程组共享一台控制终端\\
\bottomrule
\end{tabularx}
...
v1.15 的 \code{Terminal} 固定拥有 512 字节编辑区、1024 字节可读输入环与
...
\toprule
输入 & 行规程动作 & 用户是否读取该控制字节\\
\midrule
普通字符 & 追加编辑区 & 换行提交后读取\\
CR/LF & 提交编辑区并统一附加 LF & LF 可见\\
Backspace/Delete & 删除最后一个编辑字节 & 否\\
Ctrl-D & 提交当前内容，或登记一次 EOF & 否\\
Ctrl-C & 清除未提交行，产生 InterruptForeground & 否\\
Ctrl-Z & 清除未提交行，产生 StopForeground & 否\\
\bottomrule
\end{tabularx}
...
\subsection{输入守恒为何必须增加 consumed}

v1.0 ConsoleInput 可以要求 submitted 等于 read 加 buffered，因为每个接受
...
\subsection{单核为什么仍需要 irq-save 锁}
...
\subsection{JobControlManager 的状态和不变量}
...
完整 Unix 可发送 SIGTTIN；本阶段先冻结专用错误。

输出目前允许后台写。这符合“后台构建仍可输出日志”的基础需求，但可能与 Shell
提示符交错。终端显示重绘、SIGTTOU 和 termios \code{TOSTOP} 不在 v1.15。

\subsection{/dev/console 为何进入 VFS}

标准 fd 曾以 ConsoleInput/ConsoleOutput 特殊 kind 直接指向全局对象，用户
不能通过路径观察设备。v1.15 增加最小只读设备后端：
...
权限或热插拔。保留 VFS 契约后，v1.18 可以替换后端而不重写 Shell I/O。

\subsection{Ctrl-C 和 Ctrl-Z 如何选择目标}
...
\subsection{Stopped 为什么必须是独立生命周期状态}
...
状态 & 等待什么 & 资源语义\\
\midrule
Blocked & I/O、condition、deadline 等具体条件 & 条件满足后自动 Ready\\
Stopped & SIGCONT 或作业控制动作 & 全部地址空间、fd 与 CPU 现场保留\\
Zombie & parent wait 收集 & 执行资源已释放，只保留身份和退出事实\\
\bottomrule
\end{tabularx}
...
\subsection{为何 Zombie 仍需保留进程组身份}
...
\subsection{Ctrl-Z、fg 与 Ctrl-C 的逐步时序}
...
\subsection{日志为何选择事务 marker 与终态账本}
...
buffered bytes 和 pending output 必须全为零。用户 marker 正确而资源账本
错误时，整机仍输出 \code{USER\_RESULT\_INVALID}。

\subsection{四层测试怎样冻结机制而非演示}
...
层次 & v1.15 证据\\
\midrule
单元 & Terminal 行规程/守恒、JobControl 身份、字符 vnode、Ctrl 扫描码\\
集成 & 同一 child 的 Stopped→Continued→Exited 与 scheduler 状态\\
随机 & 64 槽、固定种子 100000 步 PGID 迁移，每步完整 Validate\\
系统 & QMP 真按键、IRQ1、Ring 3 cat、jobs/fg/bg/\&、终态资源守恒\\
\bottomrule
\end{tabularx}
\caption{四层证据分别隔离局部算法、组合事务、长序列性质和真实硬件路径}
\end{table}
...
\subsection{当前边界与 v1.16 的连接}

当前只有一个控制终端和 canonical 基线，没有 termios/raw mode、多个虚拟
终端或 pty、窗口尺寸、SIGWINCH、SIGHUP、SIGTTIN/SIGTTOU、孤儿进程组规则
与完整 POSIX job spec。Shell 作业表固定 16 项；这些限制均明确返回或记录，
不伪装成完整 Linux。

v1.16 将把 ATA 同步轮询升级为 IRQ14 完成的 BlockRequest，并为 page cache
建立 clean/dirty/writeback/error。TTY 阶段已经验证的 irq-save 临界区、
WaitQueue 单赢家、deadline、signal 中断、超时进度门禁和终态账本会直接复用：
设备 IRQ 只提交有界完成，Thread 在安全边界恢复，错误不能伪装成成功。
