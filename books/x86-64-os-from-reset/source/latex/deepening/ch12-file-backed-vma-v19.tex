\section{v1.9：文件后备、按需 ELF 与 clean page cache}
...
\subsection{从主动装入到 fault-driven execution}
...
入口无效，候选映像的原子提交就被破坏。v1.9 只延迟物理装入，不延迟结构
合法性判断。

\subsection{四类状态不能压进一个对象}

\begin{longtable}{L{0.20\textwidth}L{0.31\textwidth}L{0.33\textwidth}}
\toprule
\textbf{对象} & \textbf{保存的事实} & \textbf{不应保存的事实}\\
\midrule
VMA & 半开地址区间、R/W/X、kind、后备与文件区间 & 当前 frame 和硬件
accessed 位\\
PTE & present、frame、U/S、R/W、NX & fd、inode 生命周期与文件长度\\
...
\subsection{文件 VMA 的 split 必须平移 offset}
...
\subsection{x86-64 \#PF 提供的是原因，不是政策}
...
\subsection{有界 clean cache 的状态与所有权}
...
\subsection{MAP\_SHARED 与 MAP\_PRIVATE}

本阶段接受只读 shared 和可写 private：

\begin{longtable}{L{0.27\textwidth}L{0.25\textwidth}L{0.30\textwidth}}
\toprule
\textbf{映射} & \textbf{frame} & \textbf{写入语义}\\
\midrule
只读 shared & 完整页引用 clean cache & 用户写产生 protection fault\\
只读 private & 可引用 clean cache & 无写入\\
可写 private & fault 时读取到独立 frame & 只修改进程副本，不回写\\
可写 shared & 不支持 & 明确返回错误，不能静默降级\\
\bottomrule
\end{longtable}

完整 COW 留给 v1.10。v1.9 的 writable private 在第一次 fault 就复制文件
内容，内存效率较低，但不需要提前引入匿名页引用计数和只读 COW PTE 软件位。

\subsection{文件修改需要同时处理 PTE 与 cache}
...
\subsection{启动布局也是文件装入系统的一部分}

代码增长使 Kernel payload 超过历史 1 MiB 暂存区。Stage 1 的 ATA reader
必须先把容器放到不会覆盖 Kernel segment、临时页表和栈的物理窗口。v1.9
冻结 8 MiB \code{0x03600000..0x03E00000} staging，并把 rootfs v2 移到
LBA 32768，即 16 MiB。
...
v1.9 不包含 dirty page、writeback、writable shared、\code{msync}、
readahead、swap、SMP fault I/O 或动态链接。所有 cache 页都能从 rootfs
重建。v1.10 才为私有可写页增加共享引用和 COW，并要求 Kernel
\code{CopyToUser} 与用户写 fault 使用同一 break-COW 路径。
这个顺序使两个问题保持正交：v1.9 先回答“相同文件页怎样成为唯一 clean
对象”，v1.10 再回答“相同私有页怎样在第一次写时分裂”。
