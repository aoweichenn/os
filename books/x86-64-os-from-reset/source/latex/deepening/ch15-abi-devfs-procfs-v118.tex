\section{v1.18：把接口、设备名和观察面冻结成可执行契约}

\begin{keyidea}
操作系统接近一个正式版本时，最重要的能力不再是继续增加机制，而是准确回答：
哪些边界已经承诺、由谁消费、怎样证明它们不会在一次“无害重构”中漂移。
v1.18 把系统调用、公共结构与 ELF 假设收束为 ABI v2；把设备名称从终端驱动
中分离为 devfs；把动态系统状态接入只读 procfs；再让 32 个用户工具和整机
测试共同消费这些边界。冻结不是停止写代码，而是把隐含知识变成编译、测试和
发布都能拒绝的错误。
\end{keyidea}

\subsection{为什么最终开发阶段要主动停止扩张}

到 v1.17，系统已经拥有从复位到 Ring 3、从页故障到 COW、从键盘中断到
作业控制、从文件写入到 journal replay 的完整闭环。此时再加入一个看似很小
的系统调用，实际会同时改变调用号、dispatcher、用户封装、错误语义、测试、
教材和网站。若新调用还携带结构体，字段宽度、对齐、指针合法性和未来兼容都
会成为长期责任。

这与早期阶段的最优策略不同。早期要尽快建立一条最小垂直链，以真实反馈替代
纸面设计；临近发布则要减少边界数量，扩大每条边界的证明强度。v1.18 因此只
允许三类改动：
...
冻结仍然可能发现实现缺陷。例如 \code{cp} 若先截断目标、再发现源和目标是
同一 inode，源数据已经丢失；procfs 若持自己的锁调用 VFS 快照，锁方向可能
形成环；多个 TCG 实例并行运行，则正确的故障路径也可能撞上墙钟超时。所谓
“收尾”并不比开发简单，它只是把注意力从功能宽度转向契约完整度。

\subsection{ABI 的历史：源代码兼容与二进制兼容不是一回事}
...
\code{ET_EXEC}。这是一份项目自身的 64 位协议，不声称 syscall number、
errno、结构布局或调用约定与 Linux 兼容。
\end{contractbox}

\subsection{固定宽度类型只解决位数，不自动解决序列化}
...
\begin{longtable}{L{0.28\linewidth}L{0.22\linewidth}L{0.38\linewidth}}
\toprule
对象 & 关键冻结点 & 为什么必须冻结 \\
\midrule
\endfirsthead
...
\subsection{为什么设备节点需要单独的文件系统}

v1.15 为了让 fd 0/1/2 指向终端，曾用专用后端提供
\filepath{/dev/console}。它完成了垂直闭环，却把两个责任粘在一起：
...
\subsection{devfs 的注册、发布与 vnode 生命周期}
...
\midrule
\endhead
\filepath{/proc/version} & ABI major/minor 与 \code{x86_64} 目标架构 \\
\filepath{/proc/uptime} & PIT 驱动的单调纳秒，不是宿主墙钟 \\
\filepath{/proc/meminfo} & managed、free、allocated 物理内存字节 \\
\filepath{/proc/processes} & 活动 Process/Thread、容量与当前 PID \\
\filepath{/proc/resources} & heap、FileDescription、Pipe、vnode 与 journal commit \\
\filepath{/proc/mounts} & 当前 VFS mount 数 \\
\bottomrule
\end{longtable}

没有每 PID 目录、\filepath{/proc/self}、符号链接、权限、命令行和可写调优。
这些不是“漏做”，而是冻结时明确排除的新机制。

\subsection{为什么每次 read 都要重新生成快照}

进程数、空闲页和单调时间会持续变化。若挂载时生成文本，内容在第一次读取前
就可能过时；若每个 open 长期保存完整快照，则需要动态分配、新的关闭生命
周期和更复杂的偏移状态。v1.18 选择在每次 read 或 stat 时采集一次
...
\subsection{锁顺序：最容易被文本文件隐藏的内核死锁}
...
VFS 汇总也不能在全局锁内递归回调后端。当前 mount 数组在初始化完成后冻结，
...
\subsection{32 个工具：用户空间组合比新增 syscall 更有价值}

v1.18 的 rootfs 冻结 32 个独立工具路径：

\begin{lstlisting}[caption={冻结的用户工具集合}]
...
\subsection{tool probe 为什么不能被普通编译替代}
...
\subsection{测试矩阵怎样证明冻结边界}

\begin{longtable}{L{0.23\linewidth}L{0.62\linewidth}}
\toprule
证据层 & 拒绝的错误 \\
\midrule
\endfirsthead
...
\midrule
\endhead
ABI 单元测试 & 版本、号值、枚举、大小、对齐、字段 offset 或 ELF 身份漂移 \\
devfs 单元测试 & 重名、容量、半发布、readdir、只读语义、open/close 账本错误 \\
procfs 单元测试 & 六文件格式、short read、EOF、stat、provider 失败与统计错误 \\
VFS 集成测试 & rootfs、memfs、devfs、procfs 同时挂载后的路径穿越错误 \\
随机测试 & 十万组变化数值、offset 与 capacity 相对独立 oracle 的偏差 \\
产物审计 & Kernel/用户 ELF 段、权限、符号或 freestanding 依赖异常 \\
64 MiB QEMU & 启动、ABI/devfs/procfs 标记和 32 个独立 ELF 路径 \\
256 MiB QEMU & 新工具、16 级流水线、TTY、signal 与资源守恒 \\
64 GiB QEMU & 主规格地址宽度、4 GiB 以上真实页和容量模型 \\
\bottomrule
\end{longtable}

构建图共有 169 项测试，其中 145 项宿主测试允许开满 CPU，24 项
\code{os_qemu_*} 共享同一个 CTest \code{RESOURCE_LOCK}。这条锁不是功能
依赖，而是机器资源约束：多个 TCG 实例争抢 CPU 和内存，会让正确的 2--20 秒
用例撞上有界超时。把串行要求写进测试图后，执行者即使使用
\code{ctest -j20} 也不会制造虚假失败。

\subsection{日志中的时间与来宾中的时间}
...
这些限制必须与已完成能力写在同一份发布记录中。冻结的意义不是把未来想象
成已经实现，而是给下一代设计一个可靠起点。v2.0 只整合并发布已冻结机制；
若以后引入 SMP、权限、动态链接或设备热插拔，需要新的 ADR、所有权图、锁
顺序、失败模型和独立验收，而不能悄悄扩大 v1.18 的承诺。

\begin{contractbox}
v1.18 的完成条件是同一份源码同时通过 ABI 编译门禁、单元/集成/随机测试、
169 项完整构建图、三档真实 QEMU、教材构建、手机同哈希导出，以及主仓和
网站的可追溯发布。任何一项缺失，都只能称为局部实现完成，不能称为冻结完成。
\end{contractbox}
