...
\subsection{历史问题：为什么 Unix 没有把路径直接交给每次读写}
...
v1.0 的八槽 \code{IoDescriptor} 已经让控制台、管道和文件使用同一组系统
...
因此 v1.4 没有只把数组从 8 改成 4096。那只会扩大旧表示，同时掩盖所有权
仍未分层的事实。本阶段先建立通用 \code{KernelObject} 生命周期，再用
\code{FileDescription} 适配已有 I/O，最后让每个 Process 持有动态
\code{FileTable}。

\subsection{四层对象图与所有权方向}
...
\code{FileSystemHandle}，还没有独立 Vnode。这个边界是有意的：v1.4 先冻结
打开实例与 fd 的语义，v1.5 再在它下面接入 Path、Mount 与 Vnode。若同一阶段
同时替换描述符表、命名空间和磁盘格式，失败时将无法判断错误属于哪层。

\subsection{KernelObject 的内存布局}

每个动态对象由一个 64 字节对齐的私有头和紧随其后的类型载荷组成：

\begin{longtable}{L{0.29\textwidth}L{0.17\textwidth}L{0.42\textwidth}}
\toprule
字段 & 宽度 & 作用\\
\midrule
\code{previous/next} & 各 64 位 & 活动对象侵入链，不另行分配节点\\
\code{type/state} & 各 64 位 & 类型门禁与 Active/Finalizing 状态机\\
\code{variant} & 64 位 & 记录 FileDescription 的具体种类\\
\code{generation} & 64 位 & 区分同址堆内存的不同生命代次\\
\code{strong_reference_count} & 64 位 & 当前强引用总数\\
\code{allocation_size_bytes} & 64 位 & 精确归还整个堆块\\
\code{payload_size_bytes} & 64 位 & 检查载荷边界\\
\code{finalize_operation/context} & 各 64 位 & 最后释放时的类型析构回调\\
\code{operation_lock} & 实现定义 & 串行化该对象的可变语义状态\\
\bottomrule
\end{longtable}
...
\subsection{地址加 generation：处理内存复用造成的 ABA}
...
\subsection{强引用状态机与最后释放}

\code{ReferenceCounter} 在 v1.1 已冻结三条性质：非零启动、零后禁止复活、
上溢拒绝。v1.4 把它改为可用于原始 \code{uint64\_t} 状态的基础操作，
...
\subsection{Reference 与 Handle 为什么是两种类型}
...
\subsection{FileDescription：一次打开，而不是一个文件名}
...
Directory、PipeReader 或 PipeWriter\\
\code{file_status_flags} & 可读、可写等共享打开状态\\
\code{console_input} & 指向 PS/2 字符进入的有界控制台 FIFO\\
\code{device_write_operation/context} & 注入串口或控制台写操作，不把设备
实现硬编码进对象层\\
\code{pipe} & 指向现有 64 字节有界 Pipe 及相应端点\\
\code{file_system} & 指向已挂载的 legacy 文件系统实例\\
\code{file_system_handle} & inode、类型与当前共享偏移\\
\bottomrule
\end{longtable}
...
\subsection{共享状态标志与局部描述符标志}

两组名字相似的 flags 必须分开：

\begin{longtable}{L{0.25\textwidth}L{0.29\textwidth}L{0.34\textwidth}}
\toprule
类别 & 存储位置 & 复制后的关系\\
\midrule
file status flags & FileDescription & 共享；描述一次打开的读写能力\\
descriptor flags & FileTableEntry & 独立；当前只有 close-on-exec\\
\bottomrule
\end{longtable}

若把 close-on-exec 放入 FileDescription，那么给复制出来的 fd 设置该位会
连带修改源 fd；下一次 exec 将同时关闭两者。这与 fd 局部语义冲突。v1.4 的
\code{Duplicate} 接受目标 descriptor flags，源表项完全不变。

当前系统还没有完整 \code{execve}，但 \code{CloseOnExec} 已经实现并通过
宿主单元测试。提前冻结位置和批量关闭语义，能让 v1.7 接入真正 exec 时不再
重塑对象所有权。

\subsection{FileTable 的分块表示}
...
三档 hard limit 对应不同验收目标：

\begin{longtable}{L{0.24\textwidth}L{0.20\textwidth}L{0.44\textwidth}}
\toprule
配置 & hard limit & 用途\\
\midrule
最小调度事务 & 64 & 验证小容量拒绝与失败原子性\\
functional QEMU & 256 & 完整 Shell、文件、管道与 v1.4 用户证明\\
capacity QEMU & 4096 & 在 64 GiB 主规格上证明表结构不被教学小值锁死\\
\bottomrule
\end{longtable}

4096 是当前冻结的 v1.x 能力上限，不是假装已经复制 Linux 的任意动态上限。
数据结构、计数、参数和系统调用均使用 64 位固定宽度类型，未来提高上限无需
改变 ABI；真正提高前仍要重新评估 heap、每进程内存预算与遍历成本。

\subsection{soft limit 与 hard limit 的不同职责}

hard limit 在 FileTable 初始化时冻结，且不能超过 4096。soft limit 可由
当前进程调整，但必须满足：
...
切换 Thread。锁因此不是装饰：它们冻结抢占边界下的对象状态，并为 v2 多核
路线写下可审计的顺序。另一方面，本阶段没有宣称实现跨核 RCU、无锁 fd 查找
或远端 close；这些需要 AP、IPI 与内存回收协议共同验收。

\subsection{既有 I/O 如何无旁路迁移}
...
\begin{longtable}{L{0.29\textwidth}L{0.29\textwidth}L{0.30\textwidth}}
\toprule
端点 & 读路径 & 写路径\\
\midrule
ConsoleInput & 读取 256 字节 FIFO，可返回 WouldBlock & 拒绝\\
ConsoleOutput/Error & 拒绝 & 调用注入的设备写函数\\
RegularFile & legacy-fs Read 并推进共享 offset & Write 并推进共享 offset\\
Directory & ReadDirectory 并推进目录位置 & 拒绝\\
PipeReader & Pipe TryRead、EOF 与唤醒语义 & 拒绝\\
PipeWriter & 拒绝 & Pipe TryWrite、背压与 broken pipe\\
\bottomrule
\end{longtable}
...
\subsection{系统调用 23 到 28 的 ABI}

v1.3 已冻结原生 \code{SYSCALL} 与 \code{INT 0x80} 共用的
\code{UserContext}，v1.4 只在分发层增加操作：

\begin{longtable}{L{0.10\textwidth}L{0.31\textwidth}L{0.35\textwidth}L{0.12\textwidth}}
\toprule
编号 & 名称 & 参数 & 成功结果\\
\midrule
23 & DuplicateDescriptor & RDI=源 fd，RSI=最低目标，RDX=目标 flags & 新 fd\\
24 & GetDescriptorFlags & RDI=fd & flags\\
25 & SetDescriptorFlags & RDI=fd，RSI=flags & 0\\
26 & SetDescriptorSoftLimit & RDI=新 soft limit & 0\\
27 & GetDescriptorSoftLimit & 无 & soft limit\\
28 & GetDescriptorHardLimit & 无 & hard limit\\
\bottomrule
\end{longtable}
...
\subsection{汇编边界为什么不需要为 v1.4 重写}
...
没有增加新的寄存器，也没有改变汇编帧偏移。这不是“没有汇编工作”，而是
v1.3 分层成功的证据：硬件特权转换与对象语义在 UserContext 边界解耦。
若新增一个 fd 操作就必须修改 SWAPGS 或返回帧，说明 ABI 层次没有真正稳定。

\subsection{PID 4 的整机语义证明}
...
对象与字节，验收不把某次局部事务误当成全系统总量。

\subsection{统计为何在阶段结束时集中打印}

fd 查找、字符读写和引用增减都是热路径。逐次串口输出会改变 PIT 调度时序，
淹没真正异常，还可能让 QEMU 超时。v1.4 只在阶段边界打印汇总：

\begin{lstlisting}[caption={v1.4 functional 运行的代表性对象证据}]
...
v1.4 功能里程碑把回归从 102 项推进到 106 项；当前仓库又加入实体学习图
连通性门禁，完整 CTest 总数为 107。v1.4 新增的四个测试目标并不替代原有
102 个证据；历史固定槽实现的专用测试被移除，因为生产类型也已经删除。保留
针对已不存在结构的测试只会制造虚假安全感。

\subsection{增量构建为什么必须依赖生成文件}

用户 worker 不是由 QEMU 从宿主目录直接读取，而是先链接成 User ELF，再由
...
v1.4 没有 Vnode、Path、Mount、memfs 或新磁盘格式；legacy-fs handle 仍直接
存在于 FileDescription。没有 fork，因此还没有跨 Process 继承表；没有完整
exec，因此 close-on-exec 只在对象层验收。没有 seek、append、非阻塞 flag
或 advisory lock。对象类型当前只有 FileDescription，通用 manager 的目的
是冻结生命周期协议，不是提前制造庞大类层次。

FileTable 的空 chunk 只在 Destroy 时释放；4096 hard limit 是 v1.x 规格，
不是 Linux 兼容声明。KernelObject finalizer 仍是同步调用，也没有弱引用、
循环引用检测、RCU 延迟释放或多核缓存。把这些边界明确写出，才能让 v1.5
聚焦 VFS，而不是把尚未实现的能力藏在“类似 Linux”的模糊表述中。

\begin{keyidea}
v1.4 的完成不是“fd 从 8 个增加到 4096 个”。完整结论是：进程局部名字与
共享打开实例已经分离；地址复用由 generation 防护；强引用从创建、安装、
查找、复制、关闭到最终析构具有单一所有权链；descriptor flags 与 file
status flags 位于正确层次；容量、随机、双系统调用入口和 QEMU 最终统计都能
重复证明失败原子性与零泄漏。
\end{keyidea}
