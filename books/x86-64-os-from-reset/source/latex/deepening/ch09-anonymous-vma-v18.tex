\section{v1.8：匿名 VMA、按需分页与用户堆}

\begin{keyidea}
页表只能回答“处理器此刻怎样翻译这个地址”，不能回答“一个尚未驻留的地址
为什么有权在未来出现”。v1.8 用 VMA 保存区间意图、来源和软件权限，用 PTE
保存当前驻留事实；再让 x86-64 页故障把真实用户访问转换成一次有边界、可
回滚的物理页提交。匿名映射、program break、受控用户栈和 Ring 3 用户堆由此
第一次形成完整生命周期。
\end{keyidea}

\subsection{上一阶段留下的虚拟内存缺口}

v1.7 已经能够从 rootfs 读取 ELF，建立候选地址空间，构造
...
v1.8 同时保留 brk 与匿名 mmap，目的不是一次复制完整 POSIX，而是先冻结
连续区间和独立区间两种所有权。

\subsection{x86-64 页故障提供的硬件事实}
...
v1.8 使用的 error-code 位如下：

\begin{longtable}{L{0.08\textwidth}L{0.12\textwidth}L{0.31\textwidth}L{0.31\textwidth}}
\toprule
位 & 名称 & 为零 & 为一\\
\midrule
0 & P & 页不在场 & 已在场但权限违反\\
...
区域 & 范围或上限 & 语义\\
\midrule
ELF 与 brk & 最高不越过 \code{0x60000000} & ELF eager，break 从最高段页尾开始\\
匿名窗口 & \code{[0x60000000,0x80000000)} & 512 MiB first-fit 或非覆盖 fixed\\
栈 guard & \code{0x00007FFFFF7EF000} & 永久无 VMA、无 PTE\\
用户栈 & \code{[0x00007FFFFF7F0000,0x00007FFFFFFF0000)} & 8 MiB 预留、向低地址提交\\
用户 heap & 最大 8 MiB & UserHeap 自行施加的运行时边界\\
\bottomrule
\end{longtable}

这个匿名窗口不等于处理器全部用户地址能力。CPU profile 仍冻结 48 位虚拟
地址；窄窗口只是 v1.8 的公开策略，使重叠、first-gap、容量耗尽和边界溢出
可以被完整测试。

\subsection{描述符池为什么独立于每个 map}
...
\subsection{Insert：先检查，再合并，最后申请}
...
\subsection{Remove：中段拆分为何必须预取资源}
...
\subsection{first-gap 是一项有界区间算法}
...
\subsection{系统调用 39 至 42}

共享 ABI 在旧编号之后追加：

\begin{longtable}{L{0.08\textwidth}L{0.29\textwidth}L{0.31\textwidth}L{0.20\textwidth}}
\toprule
号 & 名称 & 输入 & 成功结果\\
\midrule
39 & \code{MapAnonymousMemory} & 地址、长度、权限、flags & 映射起点\\
40 & \code{UnmapMemory} & 页对齐地址、长度 & 0\\
41 & \code{SetProgramBreak} & 0 查询或新地址 & 当前 break\\
...
成功，重叠返回 \code{AddressInUse}。这避免 v1.8 同时承担跨 kind 替换事务。
...
\subsection{Kernel 用户复制为什么只解析部分 VMA}
...
\subsection{unmap 如何同时回收数据页和页表页}
...
\subsection{UserHeap 为什么放在 Ring 3}
...
\subsection{first-fit、split 与双向 coalesce}
...
检查“至少出现且数值非零”，不冻结精确 multiplicity。最终再一次输出池容量、
active、peak、acquire 和 release。

\subsection{六层证据如何互补}

\begin{longtable}{L{0.17\textwidth}L{0.71\textwidth}}
\toprule
层次 & 直接证明\\
\midrule
VMA 单元 & 排序、重叠、三类合并、trim/split、kind guard、first-gap、耗尽失败原子性\\
Heap 单元 & 增长、对齐、split、复用、双向合并、耗尽、外部和重复释放\\
固定随机 & VMA 与 heap 各 100000 步，逐步对照独立参考模型与活动载荷\\
组合生命周期 & 128 轮 reservation、真实 PTE、空页表回收和 frame/descriptor 基线\\
Ring 3 & 32 MiB 稀疏触页、2 MiB brk、递归栈和 5000 步 UserHeap\\
保护与容量 & guard/只读写 vector 14；64 MiB、256 MiB、64 GiB 同一工作负载\\
\bottomrule
\end{longtable}
...
成立。因此 v1.9 先在本阶段 fault 边界上加入文件来源，v1.10 再加入 fork/COW。

\subsection{本阶段完成判定}

v1.8 的完成不是“出现一次用户页故障后继续运行”，而是同时满足：
