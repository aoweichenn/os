\section{v1.16：从轮询磁盘到 IRQ14 与文件页写回}

\begin{keyidea}
异步 I/O 的本质不是把轮询换成中断，而是把命令、缓冲区、完成结果和等待者
从调用栈中抽出来，交给一个在 IRQ、超时和取消竞争下仍只有唯一结论的请求
状态机。可写共享映射同样不是把 PTE 的 W 位打开；Kernel 必须先取得脏页
责任，写回时重新建立下一次写通知边界。
\end{keyidea}

\subsection{为什么启动链必须轮询，运行期才适合中断}
...
\subsection{从 ATA 引脚语义到 CPU vector 0x2E}
...
控制器 & mask & 开放输入\\
\midrule
master & \code{0xF8 = 11111000b} & IRQ0 PIT、IRQ1 keyboard、IRQ2 cascade\\
slave & \code{0xBF = 10111111b} & IR6，即系统 IRQ14\\
\bottomrule
\end{tabularx}
...
端口 & 宽度 & 名称 & 读写语义\\
\midrule
\code{0x1F0} & 16 & DATA & 一个扇区 256 个 word\\
\code{0x1F1} & 8 & ERROR/FEATURES & 读错误，写 feature\\
\code{0x1F2} & 8 & SECTOR COUNT & 当前命令写一扇区\\
\code{0x1F3..0x1F5} & 8 & LBA LOW/MID/HIGH & LBA 的低 24 位\\
\code{0x1F6} & 8 & DRIVE/HEAD & master、LBA mode 与 LBA[27:24]\\
\code{0x1F7} & 8 & STATUS/COMMAND & 读状态，写命令\\
\code{0x3F6} & 8 & ALT STATUS/CONTROL & 读状态不确认 IRQ；写 nIEN/SRST\\
\bottomrule
\end{tabularx}
...
位 & 名称 & v1.16 解释\\
\midrule
7 & BSY & 设备仍拥有内部状态，不能发下一命令\\
6 & DRDY & 设备就绪提示，不单独表示传输完成\\
5 & DF & device fault，本请求完成为 DeviceError\\
3 & DRQ & CPU 必须执行一个数据阶段\\
0 & ERR & 命令错误，读取 ERROR 并完成为 DeviceError\\
\bottomrule
\end{tabularx}
...
\subsection{RFLAGS.IF 与页故障 error code}
...
\subsection{IRQ 与 PIT 超时怎样只产生一个结果}
...
\subsection{为什么 clean/dirty 两态仍然不够}
...
\toprule
状态 & 责任 & 零引用时能否 clean LRU 淘汰\\
\midrule
Empty & 没有 frame 或文件身份 & 不适用\\
Clean & frame 与文件一致，无写回债务 & 可以\\
Dirty & 内存比文件新 & 不可以\\
Writeback & writer 正在提交这个快照 & 不可以\\
Error & 上次 writer 失败，债务仍存在 & 不可以\\
\bottomrule
\end{tabularx}
...
\subsection{用写保护捕获 MAP\_SHARED 修改}
...
\subsection{从页身份到 ATA FLUSH 稳定边界}
...
证据层 & v1.16 覆盖\\
\midrule
单元 & 参数、FIFO、单飞、容量、取消、完成、超时、Reap、dirty limit、Error 重试\\
集成 & completion/timeout 后下一请求、PIC cascade、rootfs identity 稳定\\
随机 & 两个固定种子模型各 100000 步，逐步对照请求与页状态参考模型\\
故障 & ATA busy/error/timeout、重复解析、writer 失败与重试\\
QEMU & 三档 RAM、真实 IRQ14、shared alias、落盘读回、private 隔离与 Thread 前进\\
\bottomrule
\end{tabularx}
