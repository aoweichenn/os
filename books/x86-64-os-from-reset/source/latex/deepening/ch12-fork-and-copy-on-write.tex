...
\begin{osgridlongtable}{L{0.24\textwidth}L{0.28\textwidth}L{0.32\textwidth}}
\textbf{执行者} & \textbf{返回值} & \textbf{后续语义}\\ \hline
parent Thread & child PID & 可继续工作或 wait\\ \hline
child Thread & 0 & 可修改继承资源或 exec\\ \hline
失败的 parent & 负状态 & 原 Process 保持可观察不变\\ \hline
\end{osgridlongtable}

Process 是 AddressSpace、FileTable、FsContext 与父子关系的容器；Thread
才拥有通用寄存器、内核栈和 FXSAVE 现场。因此 child Process
...
\topic{x86 页故障错误码是硬件事实，不是 COW 判决}
vector 14 把故障线性地址写入 CR2，并给出 error code：

\begin{osgridlongtable}{L{0.11\textwidth}L{0.16\textwidth}L{0.54\textwidth}}
\textbf{位} & \textbf{名称} & \textbf{为 1 时的硬件事实}\\ \hline
0 & P & 页存在，但访问违反保护；为 0 是 not-present\\ \hline
1 & W/R & 这次访问是写\\ \hline
2 & U/S & 访问发生于 CPL3\\ \hline
3 & RSVD & 页表保留位编码非法\\ \hline
4 & I/D & NX 开启时，这是取指访问\\ \hline
\end{osgridlongtable}
...
\topic{软件位 9 把三种只读状态分开}
项目使用 4 KiB leaf PTE 的软件位 9 表示 COW：

\begin{osgridlongtable}{L{0.29\textwidth}L{0.12\textwidth}L{0.12\textwidth}L{0.28\textwidth}}
\textbf{状态} & \textbf{RW} & \textbf{COW} & \textbf{写入结果}\\ \hline
普通可写 private & 1 & 0 & CPU 直接写\\ \hline
fork 后共享 private & 0 & 1 & \#PF 后 private break\\ \hline
真正只读 & 0 & 0 & protection fault\\ \hline
非法组合 & 1 & 1 & 页表 API 拒绝建立\\ \hline
\end{osgridlongtable}
...
生命周期。反向映射只登记“经历 fork 后尚未恢复独占”的 private 用户页：

\begin{center}
\begin{osgridtabular}{ll}
无表项 & 隐含一个映射独占 frame\\ \hline
第一次 fork & 建表项，reference count 从隐含 1 变为 2\\ \hline
后续 fork & retain，计数递增\\ \hline
退出或 private copy & release，计数递减\\ \hline
只剩一个映射 & 可删除表项并恢复独占\\ \hline
最后映射销毁 & 计数归零，释放 frame\\ \hline
\end{osgridtabular}
\end{center}
...
\topic{CopyToUser 不能依赖用户页故障}
系统调用经常把结果结构写到用户缓冲，例如长度为 200 字节的结果结构；
本项目对应类型是 \code{VirtualMemoryStatistics}。Kernel 用 direct-map 访问目标物理 frame
...
\begin{osgridlongtable}{L{0.32\textwidth}L{0.43\textwidth}}
\textbf{页来源} & \textbf{child 行为}\\ \hline
anonymous writable & 同 frame，父子 COW\\ \hline
writable file private / ELF data & 同 private frame，父子 COW\\ \hline
完整 readonly file/ELF & 继续共享 clean cache，不标 COW\\ \hline
readonly private tail & 继续只读并复制后备生命周期\\ \hline
not-present VMA & 只 clone VMA/FileBacking，以后各自 fault\\ \hline
\end{osgridlongtable}
...
\topic{父 PTE 最晚提交：fork 是跨资源事务}
若先把 parent 全部 leaf 降为只读，再尝试创建 child，任何中途失败都会让
parent 无故停留在 COW 状态。实现先准备不可运行的候选 child：
...
\topic{阶段边界与后续演化}
当前不支持用户多 Thread 的 fork 停世、\code{vfork}、Linux
...
后面的 pipe、dup、重定向和外部 Shell 复用了这里继承的
FileTable。它们没有重新定义 private 页所有权，I/O 组合仍建立在同一套 COW
事务上。
