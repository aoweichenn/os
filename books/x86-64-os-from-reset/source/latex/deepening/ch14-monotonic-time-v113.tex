\section{v1.13：从 PIT 周期到统一 deadline}
...
\subsection{8253/8254 为什么仍在 64 位启动路径上}
...
\subsection{clock source、clock event 与 wall clock}
...
字段 & 不变量与用途\\
\midrule
\code{input\_frequency\_hz} & 非零，来自 PIT 硬件规格而不是目标频率\\
\code{divisor} & 实际写入通道 0 的整数除数\\
\code{nanoseconds} & 已经提交的整纳秒，只增不减\\
\code{fraction\_numerator} & 小于输入频率，保存尚未成为一纳秒整数的部分\\
\code{saturated} & 达到表示上界后保持真，后续推进不再回绕\\
\bottomrule
\end{tabularx}
...
\subsection{为什么内核只接受绝对 deadline}
...
\subsection{DeadlineQueue 的身份与稳定顺序}
...
\subsection{IRQ0 到用户 RAX 的完整控制流}
...
\subsection{idle 是 sleep 正确性的硬件证据}
...
\subsection{timed futex 仍要关闭 compare-and-block 窗口}
...
\subsection{ConditionVariable 返回前为什么必须重新加锁}
...
接口 & 编号 & 返回 & 语义\\
\midrule
\code{GetMonotonicTime} & 54 & \code{uint64\_t} & 当前单调纳秒\\
\code{SleepUntil} & 55 & 状态 & 阻塞到绝对 deadline，已到立即成功\\
\code{WaitPrivateFutexUntil} & 56 & 状态 & compare-and-block 加绝对 deadline\\
\bottomrule
\end{tabularx}
