\section{路径怎样穿过 VFS 和挂载点}

路径不是文件，目录项不是 Vnode，Vnode 不是一次打开，文件描述符也不是这些
对象的地址。VFS 把“进程从哪里开始找”“名字怎样逐项解析”“对象属于哪个
...
时可以让一次 open/read 走通，却产生四个继续演进的障碍：
...
因此这里不修改 rootfs 盘面格式，而是先把命名层独立出来。旧格式成为一个
后端，新的 memfs 成为第二个后端；两者使用同一套路径测试以后，后续盘面格式才在 VFS
接口不变的条件下替换根磁盘格式。

\topic{历史来源：为什么名字不属于 inode}
...
同时保存 Mount identity 与 Vnode。即使尚未实现 bind mount，也不把
未来需要的位置语义丢掉。

\topic{从 Ring 3 字符串到 ATA 扇区的完整控制链}
以 Shell 在 \filepath{/demo/message} 上执行 \code{cat} 为例，真实路径跨越
用户 C++、Intel 语法汇编、CPU 特权转换、内核对象与 ATA：

\begin{lstlisting}[caption={文件读取经过 VFS 的路径}]
Ring 3 shell.cpp
...
\topic{Superblock 的操作表为什么比继承层次更合适}
...
\topic{4096 与 255：规格边界怎样进入代码}
VFS 公共规格固定为：

\begin{osgridlongtable}{L{0.33\textwidth}L{0.20\textwidth}L{0.35\textwidth}}
量 & 上限 & 越界结果\\ \hline
完整路径 & 4096 字节 & \code{PathTooLong}\\ \hline
公共组件 & 255 字节 & \code{NameTooLong}\\ \hline
默认 Mount 数 & 64 & \code{MountCapacityExhausted}\\ \hline
单次遍历步数 & 4096 & \code{LoopDetected}\\ \hline
\end{osgridlongtable}
...
路径参数以“用户地址 + 精确长度”传递，不依赖 NUL 结尾。内核先验证整个
用户区间可读，再复制到固定 4096 字节 Ring 0 缓冲。解析拒绝 C0 控制字符、
...
\topic{四类特殊路径的精确语义}
重复分隔符、点组件、父组件和尾斜杠看似只是字符串细节，实际上分别影响组件
折叠、父级遍历、根边界与最终对象类型，都必须有确定的处理规则。
...
\section{memfs 事务、兼容适配与验证}

当前 memfs Node 的逻辑内容为：

\begin{osgridlongtable}{L{0.30\textwidth}L{0.18\textwidth}L{0.40\textwidth}}
字段 & 宽度/容量 & 作用\\ \hline
\code{next} & 目标指针 & 全局侵入节点链\\ \hline
\code{identifier/generation} & 各 64 位 & 稳定身份与代次\\ \hline
\code{parent\_identifier} & 64 位 & 父目录关系，根指向自身\\ \hline
\code{type} & 64 位枚举 & Directory 或 RegularFile\\ \hline
\code{name\_length\_bytes} & 64 位 & 名称有效长度\\ \hline
\code{name} & 255 字节 & 不依赖终止符的组件\\ \hline
\code{data} & 目标指针 & 规则文件数据，目录必须为空\\ \hline
\code{size/capacity} & 各 64 位 & 逻辑长度和已申请容量\\ \hline
\end{osgridlongtable}

节点链采用线性查找。当前先让所有节点、父关系和
...
这样枚举顺序由单调身份决定，与链表插入位置无关。当前没有删除，因此游标
不会遇到中途消失的项；引入 unlink 后必须重新定义并发修改语义，不能
直接把当前性质外推。

\topic{memfs Validate 重新计算什么}
...
\topic{legacy 适配器如何保留旧磁盘}
legacy 后端不把旧 FileSystem 整体复制一份。它在持有原 FileSystem 锁时调用
已有 inode、目录、事务和块缓存函数：

\begin{osgridlongtable}{L{0.28\textwidth}L{0.60\textwidth}}
VFS 操作 & legacy 映射\\ \hline
lookup & 读取目录 inode，查单个 40 字节以内组件，再读子 inode\\ \hline
create & 在已解析父 inode 下执行原有事务创建\\ \hline
parent/get-name & 扫描已分配目录，寻找引用当前 inode 的目录项\\ \hline
read/write & 构造临时旧 handle，用 OpenFile offset 调用既有 I/O\\ \hline
truncate & 当前只支持归零并释放直接块\\ \hline
readdir & 复用旧目录项解码与 handle offset\\ \hline
sync/validate & 复用缓存 flush 与完整可达性检查\\ \hline
\end{osgridlongtable}

parent 扫描在 inode 上限内是有界的，但不是最终高性能实现。它的价值在于
不改变盘面格式就能提供 \code{..} 和 getcwd 语义；新的盘面格式可以直接保存
或高效查找父关系。
...
\topic{FileDescription 迁移为什么没有破坏 dup}
接入 VFS 以前的共享关系是：
...
\topic{每 Process FsContext 与系统调用 29、30}
...
\begin{osgridlongtable}{L{0.10\textwidth}L{0.29\textwidth}L{0.37\textwidth}L{0.12\textwidth}}
编号 & 名称 & 参数 & 成功结果\\ \hline
29 & ChangeDirectory & RDI=路径地址，RSI=字节数 & 0\\ \hline
30 & GetWorkingDirectory & RDI=目标地址，RSI=容量 & 路径字节数\\ \hline
\end{osgridlongtable}
...
\topic{目录项 ABI 为什么显式增加 reserved 字节}
...
\code{static\_assert(sizeof(DirectoryEntry)==280)} 检查大小。这个例子说明
...
\topic{持久 memfs 资源怎样进入 ResourceSnapshot}
...
\section{怎样验证路径、挂载和 memfs}
宿主侧先用局部契约、双后端协作和独立随机模型扩大覆盖面，随后再由 QEMU
证明同一套语义确实穿过磁盘镜像、内核对象和用户态调用链。
...
\topic{同一条路径测试怎样覆盖 rootfs 和 memfs}
...
\topic{沿目录走读 VFS 与 memfs}\par
\begin{osgridlongtable}{L{0.44\textwidth}L{0.44\textwidth}}
文件 & 阅读时要回答的问题\\ \hline
\filepath{kernel/fs/vfs.hpp/.cpp} &
Path 怎样组合 Mount 与 Vnode，逐组件解析和挂载退出的提交点在哪里？\\ \hline
\filepath{kernel/fs/memfs.hpp/.cpp} &
Node、数据增长、目录游标和 heap 资源统计怎样保持一致？\\ \hline
\filepath{kernel/fs/legacy\_file\_system.hpp/.cpp} &
旧 inode/事务怎样变成单组件后端，而不改变盘面格式？\\ \hline
\filepath{kernel/fs/file\_system.hpp/.cpp} &
新增的 parent 与 child helper 怎样复用旧锁和事务？\\ \hline
\filepath{kernel/io/file\_description.hpp/.cpp} &
legacy handle 怎样替换为 OpenFile，duplicate 和 finalizer 为何不变？\\ \hline
\filepath{kernel/process/process\_runtime.hpp/.cpp} &
FsContext 何时初始化，持久 VFS 资源怎样与 Process 快照分账？\\ \hline
\filepath{kernel/user/system\_calls.cpp} &
4096 字节用户复制、29/30 号调用和 280 字节目录项怎样校验？\\ \hline
\filepath{user/src/shell.cpp} &
cd、pwd、动态提示符和默认 \code{ls .} 怎样只经用户 ABI 工作？\\ \hline
\filepath{tests/unit/kernel/vfs\_test.cpp} &
边界、尾斜杠、Mount 环和资源归还如何构造？\\ \hline
\filepath{tests/integration/boot/vfs\_backend\_contract\_test.cpp} &
哪一组契约在两个后端完全相同？\\ \hline
\filepath{tests/randomized/kernel/vfs\_namespace\_randomized\_test.cpp} &
独立模型保存哪些事实，为什么不复用生产解析器？\\ \hline
\filepath{tools/os\_tools/qemu\_runner.py} &
Shell 输入、marker 精确次数、相对时间与超时回收怎样组合？\\ \hline
\end{osgridlongtable}
...
\topic{失败矩阵}\par
\begin{osgridlongtable}{L{0.27\textwidth}L{0.28\textwidth}L{0.33\textwidth}}
输入或状态 & 结果 & 必须保持不变的状态\\ \hline
空路径 & {\small\code{InvalidPath}} & cwd、Mount、节点\\ \hline
4097 字节路径 & {\small\code{PathTooLong}} & 输出 Path 为空\\ \hline
256 字节组件 & {\small\code{NameTooLong}} & 父目录和统计无半创建\\ \hline
普通文件后继续组件 & {\small\code{NotDirectory}} & offset 与后端状态\\ \hline
普通文件尾斜杠 & {\small\code{NotDirectory}} & 文件仍可由无斜杠路径访问\\ \hline
缺失文件尾斜杠 create & {\small\code{NotDirectory}} & 不建立同名普通文件\\ \hline
memfs 节点耗尽 & {\small\code{CapacityExhausted}} & 链、计数和输出 vnode\\ \hline
memfs 文件越界 & {\small\code{FileTooLarge}} & data、size、capacity\\ \hline
Mount 表已满 & {\small\code{MountCapacityExhausted}} & mount count 与旧项\\ \hline
Mount 父链环 & {\small\code{LoopDetected}} & 不无限遍历\\ \hline
legacy 损坏非零盘 & {\small\code{Corrupt / Incomplete}} & 不格式化、不进入用户态\\ \hline
用户目标不可写 & {\small\code{InvalidUserMemory}} & 不调用 getcwd 写回\\ \hline
\end{osgridlongtable}

\topic{当前实现范围}
当前还没有 rootfs v2、间接块、64 MiB 单文件、unlink、rename、非零
...
第 25 章回到磁盘后端，介绍 rootfs v2、稀疏大文件、命名空间修改以及独立的
mkfs、inspector 和 fsck。这里建立的 Path、Mount 与 FileDescription 语义
保持不变。

在 Shell 输入 \code{cd /tmp/session} 时，路径查找从当前 Process 的 root
和 cwd 开始，依次处理每个组件。Mount 与 Vnode 确定当前走到哪个对象；
FileDescription 保存已经打开的那一次访问。memfs 和旧磁盘后端都实现相同
操作，所以同一条系统调用路径可以访问二者。宿主测试检查路径状态机，QEMU
继续检查真实 SYSCALL 入口和跨启动磁盘读取。
