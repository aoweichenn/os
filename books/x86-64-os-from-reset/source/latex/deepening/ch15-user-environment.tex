...
\[
  ...
  \longrightarrow
  (\mathrm{type},\mathrm{object_index},\mathrm{rights}).
\]

v1.0 每个 PCB 固定八个槽。固定容量让所有状态都能在退出路径完整遍历，也
避免在尚无通用内核对象分配器时假装支持无限打开。表的初始布局如下：

\begin{osgridlongtable}{L{0.10\textwidth}L{0.22\textwidth}L{0.18\textwidth}L{0.38\textwidth}}
fd & 类型 & 方向 & 当前语义\\ \hline
\endfirsthead
fd & 类型 & 方向 & 当前语义\\ \hline
\endhead
0 & ConsoleInput & 只读 & 从全局控制台 FIFO 取字符；空时可以等待\\ \hline
1 & ConsoleOutput & 只写 & 经内核校验后写 COM1，作为用户标准输出\\ \hline
2 & ConsoleError & 只写 & 当前同样写 COM1，但保留独立语义通道\\ \hline
3--7 & 动态槽 & 由对象决定 & 普通文件、目录、管道读端或管道写端\\ \hline
\end{osgridlongtable}
...
\begin{osgridlongtable}{L{0.18\textwidth}L{0.18\textwidth}L{0.22\textwidth}L{0.30\textwidth}}
输入 & 类别 & 状态变化 & 字符结果\\ \hline
左/右 Shift make & 修饰键 & 对应 shift=true & 不提交字符\\ \hline
左/右 Shift break & 修饰键 & 对应 shift=false & 不提交字符\\ \hline
Caps Lock make & 锁定键 & caps 取反 & 不提交字符\\ \hline
字母 make & 可打印键 & 状态不变 & shift XOR caps 选择大小写\\ \hline
数字/标点 make & 可打印键 & 状态不变 & 只由 shift 选择符号\\ \hline
Enter make & 控制键 & 状态不变 & 提交换行\\ \hline
Backspace make & 控制键 & 状态不变 & 提交退格\\ \hline
任意 break & 释放事件 & 必要时更新修饰键 & 普通键不重复提交\\ \hline
\end{osgridlongtable}
...
\topic{Try 与 Wait 分离，避免把睡眠误当成 I/O 完成}
...
for (;;) {
    ...
    if (wait_result < OS_USER_SYSTEM_CALL_SUCCESS) {
        return wait_result;
    }
}
\end{lstlisting}
...
\begin{osgridlongtable}{L{0.12\textwidth}L{0.30\textwidth}L{0.46\textwidth}}
编号 & 调用 & 主要边界\\ \hline
16 & 尝试读描述符 & fd、写入用户范围、长度、对象读权限\\ \hline
17 & 尝试写描述符 & fd、读取用户范围、长度、对象写权限\\ \hline
18 & 等待描述符可读 & 当前 fd 权限、条件复查与等待原因\\ \hline
19 & 等待描述符可写 & 当前 fd 权限、条件复查与等待原因\\ \hline
20 & 关闭描述符 & 动态 fd、类型化资源释放、重复关闭\\ \hline
21 & 打开目录 & 绝对路径复制、目录类型、动态槽容量\\ \hline
22 & 读取目录项 & 64 字节目录项 ABI、游标、目录 EOF\\ \hline
\end{osgridlongtable}
...
\topic{目录迭代为何使用结构化 64 字节 ABI}
\code{ls} 不能只把目录原始字节当普通文件读出。磁盘目录项是文件系统内部
格式，未来可能改变 CRC、填充或块布局；用户真正需要的是 inode、对象类型和
名称。因此 \code{ReadDirectory} 返回独立的 ABI 结构：

\begin{osgridlongtable}{L{0.24\textwidth}L{0.13\textwidth}L{0.47\textwidth}}
字段 & 大小 & 语义\\ \hline
\code{inode_number} & 8 B & 当前文件系统中的稳定 inode 编号\\ \hline
\code{type} & 8 B & RegularFile 或 Directory\\ \hline
\code{name_length_bytes} & 8 B & 后续名称中的有效字节数\\ \hline
\code{name} & 40 B & 不依赖 NUL 的固定容量名称\\ \hline
\end{osgridlongtable}
...
\begin{pdfdiagram}
  \node[os state] (none-single) {无引号};
  \node[os block,right=35mm of none-single] (single) {单引号};
  \draw[os arrow] ([yshift=2mm]none-single.east) --
    node[above]{读到单引号} ([yshift=2mm]single.west);
  \draw[os arrow] ([yshift=-2mm]single.west) --
    node[below]{再次读到单引号} ([yshift=-2mm]none-single.east);

  \node[os state,below=9mm of none-single] (none-double) {无引号};
  \node[os block,right=35mm of none-double] (double) {双引号};
  \draw[os arrow] ([yshift=2mm]none-double.east) --
    node[above]{读到双引号} ([yshift=2mm]double.west);
  \draw[os arrow] ([yshift=-2mm]double.west) --
    node[below]{再次读到双引号} ([yshift=-2mm]none-double.east);

  \node[os state,below=9mm of none-double] (escape-source) {无引号或双引号};
  \node[os hardware,right=35mm of escape-source] (escape) {转义下一字节};
  \draw[os arrow] (escape-source) --
    node[above]{读到反斜杠} (escape);
\end{pdfdiagram}
...
命令 & 系统调用组合 & 学习重点\\ \hline
\code{help} & 标准输出 & 用户程序自己的静态数据与写包装\\ \hline
\code{echo} & 参数拼接、标准输出 & 引号、转义和有界输出\\ \hline
\code{pwd} & 标准输出 & 当前仅根目录的显式范围\\ \hline
\code{ls [path]} & 打开目录、逐项读取、关闭 & 类型化目录 EOF 与游标\\ \hline
\code{mkdir path} & CreateDirectory & 路径复制和已存在语义\\ \hline
\code{write path text} & Open/Write/Close & create、truncate、部分写循环\\ \hline
\code{cat path} & Open/Read/Close & 文件 EOF 和固定块读取\\ \hline
\code{sync} & SyncFileSystem & 缓存写回与 ATA flush 边界\\ \hline
\code{exit} & ExitProcess & 资源自动关闭和进程终止\\ \hline
\end{osgridlongtable}
...
\topic{从四个层次检查用户环境}\par
\begin{osgridlongtable}{L{0.19\textwidth}L{0.31\textwidth}L{0.36\textwidth}}
层级 & 主要输入 & 能证明的边界\\ \hline
单元 & FIFO、描述符表、扫描码、解析器 & 容量、顺序、权限、状态机和错误原子性\\ \hline
集成 & 调度器、目录生命周期、真实 ELF & 无 Ready 状态、类型化目录、ABI 与链接结构\\ \hline
随机 & 4096 轮任意解析字节 & 所有切片位于固定存储、失败不越界、结果可重复\\ \hline
整机 & QMP 十命令、同盘双启动、损坏盘 & IRQ 到 Ring 3、idle 唤醒、持久化与拒绝路径\\ \hline
\end{osgridlongtable}

最终目标内核汇总：
\[
  \mathrm{console_submitted}
  =
  \mathrm{console_read}
  =109,\qquad
  \mathrm{dropped}
...
v1.1 完成时回归共 97 项 CTest，其中可回收堆、buddy、type cache、KVA、动态
内核栈、页表空分支与通用资源生命周期都拥有单元、集成和固定种子证据，资源
随机状态机各执行 100000 步；页表模型额外推导三级表数并验证映射失败回滚，
动态栈集成测试还从独立进程 CR3 查询共享高半区映射。26 字段快照在启动期
真实栈事务和四进程退出后各要求零差异；256 MiB functional 与 64 GiB
capacity 都执行完整 Shell 路径。三条用户故障 QEMU 路径仍要求资源回收；
命名门禁组合 Clang AST 与 Python
词法扫描，锁定
变量蛇形、函数大驼峰与单词级小写命名空间。数字本身不是质量指标，它只说明
新增验证没有替换旧
