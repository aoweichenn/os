...
\begin{longtable}{L{0.24\textwidth}L{0.26\textwidth}L{0.36\textwidth}}
\toprule
\textbf{示意范围} & \textbf{传统用途} & \textbf{本项目使用原则}\\
\midrule
\endfirsthead
\toprule
\textbf{示意范围} & \textbf{传统用途} & \textbf{本项目使用原则}\\
\midrule
\endhead
\code{0x00000000} 起 & 低端 RAM、向量和固件数据 & 明确保留后再放栈与启动结构\\
\code{0x000A0000} 附近 & VGA 与适配器窗口 & 不当作普通可用 RAM\\
\code{0x00100000} 以上 & 扩展 RAM & 后续装载内核和页表\\
4 GiB 以下高端 & PCI 窗口与平台设备 & 依据机器模型排除\\
\code{0xFFFE0000} 起 & 本项目 128 KiB ROM & 只读执行与常量区域\\
\bottomrule
\end{longtable}
...
\toprule
\textbf{对象} & \textbf{物理地址} & \textbf{文件偏移} & \textbf{作用}\\
\midrule
\endfirsthead
\toprule
\textbf{对象} & \textbf{物理地址} & \textbf{文件偏移} & \textbf{作用}\\
\midrule
\endhead
ROM 起点 & \code{FFFE0000} & \code{00000} & 镜像映射基准\\
入口 & \code{FFFFF000} & \code{1F000} & 完整初始化代码\\
复位向量 & \code{FFFFFFF0} & \code{1FFF0} & 三字节近跳转\\
ROM 末端 & \code{FFFFFFFF} & \code{1FFFF} & 最后一个可寻址字节\\
\bottomrule
\end{longtable}

\section{相对跳转依赖下一指令地址}

x86 相对位移不是从当前指令起点计算，而是从完整指令之后计算。复位跳转长度
...
\begin{longtable}{L{0.20\textwidth}L{0.24\textwidth}L{0.24\textwidth}L{0.20\textwidth}}
\toprule
\textbf{指令} & \textbf{端口来源} & \textbf{数据宽度} & \textbf{方向}\\
\midrule
\endfirsthead
\toprule
\textbf{指令} & \textbf{端口来源} & \textbf{数据宽度} & \textbf{方向}\\
\midrule
\endhead
\code{in al,dx} & DX 低 16 位 & 8 位 & 设备到 CPU\\
\code{in ax,dx} & DX 低 16 位 & 16 位 & 设备到 CPU\\
\code{out dx,al} & DX 低 16 位 & 8 位 & CPU 到设备\\
\code{out dx,ax} & DX 低 16 位 & 16 位 & CPU 到设备\\
\bottomrule
\end{longtable}
