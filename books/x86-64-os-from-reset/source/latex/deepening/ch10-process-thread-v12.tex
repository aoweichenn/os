\section{v1.2 把资源所有者与处理器执行流彻底分开}
...
PID 不等于槽位；页表根非零；Thread 链计数与三类计数一致\\
Thread &
\code{ThreadId}、所属 Process、调度状态、内核栈槽、用户栈、TLS、signal mask &
TID 独立单调；每个活动 Thread 恰属于一个 Process\\
Thread 现场 &
176 B 通用/IRET 帧、512 B FXSAVE 区 &
帧完整位于本 Thread 动态内核栈；FXSAVE 区 16 B 对齐\\
Thread 队列 &
Process 链、双向 run queue 链、WaitQueue 链 &
一个 Thread 不能同时是 Running、Ready 和 waiter\\
统计 &
run tick、dispatch、block、wake &
当前量与累计 create/discard/reap 分开\\
\bottomrule
\end{longtable}
...
\subsection{为什么 Thread 与 Process 要分两次 reap}
...
\section{WaitQueue 解决的是竞争完成，不只是“睡一下”}
...
\begin{longtable}{L{0.27\textwidth}L{0.25\textwidth}L{0.36\textwidth}}
\toprule
WakeReason & 典型来源 & 调用者恢复后的含义\\
\midrule
ConditionSatisfied & 数据到达、空间释放、Mutex handoff & 重新检查条件并尝试操作\\
Timeout & 单调 deadline 到达 & 无进度时返回超时；本阶段仅冻结原因\\
Signal & 可中断等待收到信号 & 按系统调用重启规则处理；后续阶段实现\\
ObjectClosed & pipe/descriptor/队列关闭 & 重新检查 EOF、broken pipe 或关闭错误\\
Cancelled & futex 地址 unmap 等取消 & 资源身份已失效，不得继续使用旧 key\\
\bottomrule
\end{longtable}
...
\section{三类锁服务三种不同等待成本}

\begin{longtable}{L{0.22\textwidth}L{0.25\textwidth}L{0.18\textwidth}L{0.23\textwidth}}
\toprule
原语 & 使用位置 & 能否睡眠 & v1.2 关键语义\\
\midrule
SpinLock & Thread 上下文短提交区 & 否 & exchange-acquire、store-release、内层 PAUSE\\
IrqSaveSpinLock & 与本 BSP IRQ 共享状态 & 否 & 保存 IF、CLI、取锁、释放后恢复原 IF\\
Mutex & 可能较长的 Thread 临界区 & 是 & 竞争者进 WaitQueue，解锁直接 handoff\\
\bottomrule
\end{longtable}
...
\section{x87 到 SSE2 的历史解释了为什么现场不只是一组通用寄存器}
...
\subsection{CPUID 能力与 OS 控制位必须同时成立}

\code{CPUID.01H:EDX} 的三位是本阶段硬门槛：

\begin{longtable}{L{0.12\textwidth}L{0.17\textwidth}L{0.59\textwidth}}
\toprule
位 & 名称 & 意义\\
\midrule
24 & FXSR & 处理器支持 FXSAVE/FXRSTOR\\
25 & SSE & 存在 XMM 与 MXCSR 状态\\
26 & SSE2 & 满足当前 64 位用户程序的扩展指令基线\\
\bottomrule
\end{longtable}

能力存在仍不代表 OS 已准备好。v1.2 配置：

\begin{longtable}{L{0.14\textwidth}L{0.24\textwidth}L{0.14\textwidth}L{0.36\textwidth}}
\toprule
寄存器 & 位 & 当前值 & 原因\\
\midrule
CR0 & MP(1) & 1 & 让 WAIT/FWAIT 与 TS 规则正确配合\\
CR0 & EM(2) & 0 & 不使用软件仿真，允许真实 x87 指令\\
CR0 & TS(3) & 0 & 不使用基于 \#NM 的惰性切换\\
CR0 & NE(5) & 1 & x87 错误经原生异常，不走旧式 IRQ13\\
CR4 & OSFXSR(9) & 1 & 声明 OS 管理 FXSAVE 现场\\
CR4 & OSXMMEXCPT(10) & 1 & SIMD 浮点异常经 \#XM\\
CR4 & OSXSAVE(18) & 0 & 禁止尚未保存的 XSAVE/AVX 状态\\
\bottomrule
\end{longtable}
...
\section{512 字节 FXSAVE 区是一份硬件规定的二进制结构}

\code{FXSAVE64} 要求内存操作数按 16 字节对齐。v1.2 用
\code{alignas(16)} 定义恰好 512 字节的 \code{FxSaveArea}，并用两个
\code{static_assert} 锁定大小和对齐。主要布局为：

\begin{longtable}{L{0.14\textwidth}L{0.15\textwidth}L{0.48\textwidth}L{0.13\textwidth}}
\toprule
偏移 & 长度 & 内容 & 验收关注\\
\midrule
\code{0x000} & 8 B & FCW、FSW、abridged FTW、FOP & x87 控制/状态\\
\code{0x008} & 16 B & 64 位 FIP 与 FDP & 指令/数据指针\\
\code{0x018} & 8 B & MXCSR 与 MXCSR\_MASK & SIMD 控制/能力\\
\code{0x020} & 128 B & ST0--ST7/MMX，每槽 16 B & x87 数据\\
\code{0x0A0} & 256 B & XMM0--XMM15，每槽 16 B & SSE2 数据\\
\code{0x1A0} & 96 B & 保留或软件可用尾区 & 总长补足 512 B\\
\bottomrule
\end{longtable}
...
\subsection{为什么选择 eager 而不是 lazy FPU switching}
...
状态 & 模式 & 覆盖目的\\
\midrule
XMM0 & 每 PID 两个不同 64 位分量 & 覆盖常用低 XMM 数据\\
XMM15 & 另一组两个 64 位分量 & 覆盖 64 位模式高 XMM 区\\
MXCSR & 四个有效舍入/控制组合 & 证明控制状态也隔离\\
x87 control word & 四个不同舍入控制字 & 覆盖 x87 控制字段\\
ST0 & 1.0、2.0、3.0、4.0 & 覆盖 x87 数据栈且可精确比较\\
\bottomrule
\end{longtable}
...
\section{三档容量事务证明规格不是常量表}

\begin{longtable}{L{0.20\textwidth}L{0.13\textwidth}L{0.16\textwidth}L{0.15\textwidth}L{0.20\textwidth}}
\toprule
...
Process/Thread 状态、侵入式链和 Validate 如何彼此约束？\\
\filepath{process/wait_queue.hpp/.cpp} &
为什么队列只暴露生命周期与统计，实际链接由 scheduler 维护？\\
\filepath{sync/spin_lock.hpp/.cpp} &
acquire/release、PAUSE、IF 保存和持锁深度在哪里建立？\\
\filepath{sync/mutex.hpp/.cpp} &
直接 handoff 如何避免 owner 空窗与插队？\\
\filepath{arch/extended_state_layout.hpp/.cpp} &
哪些能力和布局可在宿主测试而不执行特权指令？\\
\filepath{arch/extended_state.hpp/.cpp} &
CPUID、CR0/CR4、初始模板和累计计数如何组合？\\
\filepath{arch/architecture.asm} &
FXSAVE64/FXRSTOR64 为什么保持为三个极小 Intel 语法边界？\\
\filepath{process/process_runtime.cpp} &
纯调度决定如何落实为 CR3、TSS.RSP0、动态栈和扩展现场？\\
\filepath{user/src/extended_state.asm} &
四组 XMM/MXCSR/x87 模式如何在不依赖 C++ 浮点代码的情况下验证？\\
\bottomrule
\end{longtable}
