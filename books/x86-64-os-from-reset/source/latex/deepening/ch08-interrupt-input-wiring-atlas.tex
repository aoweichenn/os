...
\subsection{一、IRQ 和 vector 为什么不是同一个编号}
...
\begin{longtable}{L{0.16\linewidth}L{0.24\linewidth}L{0.27\linewidth}L{0.23\linewidth}}
\toprule
名称 & 所在层 & 本图例子 & 谁配置或产生 \\
\midrule
IRQ line & 设备到中断控制器的请求 & IRQ0、IRQ1 & PIT/i8042 产生，PIC 接收 \\
PIC vector & 控制器交给 CPU 的编号 & \code{0x20}、\code{0x21} & Kernel 初始化 PIC offset \\
IDT entry & CPU 按 vector 找到的 16 B gate & entry 32、33 & Kernel 构造并 \code{lidt} \\
handler & 保存现场后运行的软件 & timer/keyboard C++ 分派 & 汇编 adapter 调用 \\
\bottomrule
\end{longtable}

\subsection{二、硬件泳道从左到右发生什么}

\subsubsection{2.1 PIT 和 i8042 先保存各自的事件事实}
...
\subsubsection{2.2 PIC 仲裁、记录并等待 EOI}
...
\subsubsection{2.3 Local APIC virtual-wire 是兼容桥}
...
\subsubsection{2.4 CPU 还要经过四道接收条件}
...
\subsection{三、CPU/IDT 如何安全改变控制流}
...
\subsection{四、Kernel 软件泳道的四个提交点}

\begin{longtable}{L{0.16\linewidth}L{0.27\linewidth}L{0.30\linewidth}L{0.17\linewidth}}
\toprule
阶段 & 必须完成 & 失败边界 & 提交证据 \\
\midrule
汇编入口 & 保存/规范化现场，建立 ABI 栈 & 任何调用前，不可破坏原现场 & 固定 frame 可审计 \\
C++ dispatcher & 检查 vector、来源与当前上下文 & 未知/非法状态走确定性失败 & 选中唯一 handler \\
设备处理 & 读取事实，更新最小有界状态 & 不在 IRQ 中无限等待或做阻塞用户工作 & tick/扫描码入队 \\
完成返回 & 按来源发送 EOI，恢复现场，\code{iretq} & 不得返回失效进程/栈 & 下一现场继续执行 \\
\bottomrule
\end{longtable}
...
\subsection{五、按键的十四步为什么每一步都不可省}

\subsubsection{5.1 步骤 1--2：宿主请求不等于 Guest 字符}
...
\subsubsection{5.2 步骤 3--4：键盘产生扫描码，i8042 保存字节}
...
\subsubsection{5.3 步骤 5--7：通知抵达，handler 才读取数据}
...
\subsubsection{5.4 步骤 8--10：扫描码成为有界字符队列}
...
\subsubsection{5.5 步骤 11--14：系统调用把字符交给 Shell}
...
\subsection{六、怎样按症状定位断在哪一层}

\begin{longtable}{L{0.25\linewidth}L{0.30\linewidth}L{0.35\linewidth}}
\toprule
现象 & 首先检查 & 原因 \\
\midrule
QMP 成功但没有 IRQ1 & QEMU 设备选择、i8042 初始化、PIC IMR、LAPIC、IF & 宿主命令只证明注入请求被接受 \\
IRQ1 计数增加但无字符 & \code{0x64} 状态、\code{0x60} 读取、扫描码 decoder & 通知路径正常不等于数据解释正常 \\
字符入 FIFO 但 Shell 卡住 & waiter 登记、唤醒条件、调度、fd 0 对象 & 硬件路径已完成，故障位于阻塞协议 \\
Shell 收到错字符 & make/break、扩展前缀、Shift/Caps 状态 & 不能只检查 IRQ 数量 \\
能输入但无回显 & fd 1/2、系统调用返回、UART THRE 与 serial capture & 输出链独立于输入链 \\
第一个键后再无中断 & i8042 数据是否取走、PIC EOI 是否发送 & 设备/PIC 可能仍保持未完成状态 \\
\bottomrule
\end{longtable}
