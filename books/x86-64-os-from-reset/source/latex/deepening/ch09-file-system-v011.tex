...
打包器证明 Kernel 负载结束位置不越过 2048\\
\code{[2048,3072)} & v0.11 文件系统 & 1024 个固定大小逻辑块\\
\code{[3072,4096)} & 保留 & 当前必须不被文件系统分配\\
\bottomrule
\end{longtable}
...
\section{ATA PIO 写入是一条可失败的硬件状态机}
...
\begin{longtable}{L{0.17\textwidth}L{0.18\textwidth}L{0.53\textwidth}}
\toprule
相对块 & 数量 & 内容\\
\midrule
0 & 1 & superblock、事务状态与 CRC32\\
1 & 1 & 32 位有效范围的 inode bitmap\\
2--9 & 8 & 32 个固定 128 字节 inode\\
10 & 1 & 1013 位有效范围的数据块 bitmap\\
11--1023 & 1013 & 目录项和普通文件数据\\
\bottomrule
\end{longtable}
...
目录本身也是 inode，其 \code{sizeBytes} 必须是 64 的整数倍。一个块容纳
8 项，十个直接块最多容纳 80 项。inode 不保存路径；路径来自从根目录开始
的一串目录项，这正是 rename 不需要搬动文件数据的原因。

\section{CRC 能发现变化，但不能证明所有权正确}
...
\section{bitmap 分配把空间所有权变成守恒关系}

inode 分配从编号 2 开始寻找第一个零位；数据块分配在 1013 个有效位中寻找
零位，再转换为相对块 \code{11+bitIndex}。释放文件时逐个验证块位于数据区，
清对应位，并把 inode 的直接块、块数和长度归零。

一致性检查维护一张 \code{seenDataBitmap}。遍历每个已分配 inode 时：

\begin{itemize}
  \item 前 \code{allocatedBlockCount} 个直接块必须位于数据区且 bitmap 为 1；
  \item 同一块在 \code{seenDataBitmap} 中不能已经出现；
  \item 块数之后的直接块必须为零；
...
失败。文件所需块数还必须等于
\[
  \left\lceil\frac{\mathrm{sizeBytes}}{512}\right\rceil,
\]
不接受隐藏的尾部已分配块。

\section{八项写回缓存具有明确状态转换}
...
\section{Dirty/Clean 协议先保证可检测，再讨论恢复}

创建目录、创建/截断文件和写文件会同时改变多个块。v0.11 使用超级块中的
\code{transactionState} 和 64 位代次建立最小提交协议：
...
\begin{longtable}{L{0.21\textwidth}L{0.28\textwidth}L{0.39\textwidth}}
\toprule
停止位置 & 磁盘可见状态 & 下次挂载\\
\midrule
步骤 1 以前 & 上一份 Clean 格式 & 正常挂载\\
步骤 2--4 & Dirty，元数据可能部分新旧混合 & 报未完成事务并停止\\
步骤 5 完成 & 新一代 Clean 格式 & 正常挂载\\
\bottomrule
\end{longtable}
...
\section{测试按失败层次分开，而不是只增加数量}

\begin{longtable}{L{0.19\textwidth}L{0.28\textwidth}L{0.41\textwidth}}
\toprule
层次 & 替身或环境 & 主要证据\\
\midrule
单元 & 编码缓冲 & superblock/inode/目录项往返、CRC 损坏\\
集成 & 4096 扇区内存设备 & 格式化、目录、跨块文件、重挂载、截断、孤儿与非法名称\\
随机 & 固定种子内存设备 & 128 次随机长度/内容、每轮新实例重挂载并对照参考数组\\
目标系统 & QEMU TCG + ATA PIO & Ring 3 文件 ABI、真实端口写、flush、统计与最终一致性\\
重启系统 & 同一 snapshot-off 临时磁盘 & 第一次格式化写入，第二次先恢复旧载荷\\
损坏系统 & 第二次启动后的磁盘 & 翻转超级块受 CRC 保护字节，第三次必须拒绝挂载\\
\bottomrule
\end{longtable}
