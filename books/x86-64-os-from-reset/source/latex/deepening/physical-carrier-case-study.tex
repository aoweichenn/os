...
\begin{sidewaysfigure}[p]
  ...
  \begin{tikzpicture}[
    ...
    \begin{scope}[shift={(4.50,6.30)}]
      \input{figures/physical_carrier/intel_n_series_ballout_points}
    \end{scope}
...
\begin{sidewaysfigure}[p]
  \centering
  \input{figures/physical_carrier/cpu_pin_to_spi}
  \caption{Intel N-Series SPI0 到 SOIC-8 启动 Flash 的逐引脚参考接线。
  四条协议线、两个配置上拉、供电、地和去耦电容均有明确端点。}
  \label{fig:carrier-cpu-spi-pin-to-pin}
\end{sidewaysfigure}

顺着第一根线检查：CPU ball \code{CC52} 输出低电平，经过
\code{SPI0\_CS0\#} 网络到达 U1 pin 1，Flash 随后才采样 pin 6 的时钟。
CPU 从 ball \code{CF52} 发出的命令位进入 U1 pin 5；U1 从 pin 2 返回的数据
进入 CPU ball \code{CH52}。把 pin 2 和 pin 5 交换，波形分析仪仍可能看到
时钟和片选，但 CPU 读回的厂商 ID 会是全 0、全 1 或不稳定值。
...
\begin{sidewaysfigure}[p]
  \centering
  \input{figures/physical_carrier/cpu_pin_to_usb}
...
故意交换 J1 pin 2 和 pin 3 后，VBUS 仍有 5 V，插入设备也可能亮灯，但接收器
看到的是反相的差分极性，主机无法正常完成 USB reset 和枚举。断开 U2 pin 2
的短地线时，普通万用表仍会显示 D+/D- 直流连通；ESD 电流却失去低电感回路，
保护器件在真正的静电事件中可能来不及把接口钳位到安全范围。
...
\begin{sidewaysfigure}[p]
  \centering
  \input{figures/physical_carrier/cpu_pin_to_i2c_uart}
  \caption{Intel N-Series I2C0、24C02 与 UART0 调试排针的逐引脚参考接线。
  开漏总线的上拉和串口的交叉方向都直接画在导线上。}
  \label{fig:carrier-cpu-i2c-uart-pin-to-pin}
\end{sidewaysfigure}

I2C 的两只上拉若完全删除，CPU 和 EEPROM 都只能主动把线路拉低，没有器件负责
把线路恢复为高电平。逻辑分析仪会看到 SCL/SDA 在第一次低电平以后不再上升。
UART 若把 TX 接 TX、RX 接 RX，两端都在各自的发送线上产生正常波形，却没有
任何波形进入接收脚；这就是为什么原理图必须同时写引脚号和信号方向。
