...
控制器/PHY”合在左侧功能端，右侧从公开的 Mu pin 开始画真实载板铜线。
...
\begin{sidewaysfigure}[p]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\Large\bfseries\color{BookNavy}] at (9.6,9.15)
      {从 SoC 功能端点经过 260-pin 边界追到载板器件};
    \node[font=\bfseries\color{BookBlue},align=center] at (2.4,8.45)
      {CPU cores/APIC 经 SoC fabric\\到达控制器/PHY};
    \node[font=\bfseries\color{BookTeal}] at (9.6,8.45)
      {公开的 260-pin 信号组};
    \node[font=\bfseries\color{BookAmber!70!black}] at (16.8,8.45)
      {DFR1142 V2 载板落点};
    ...
    \node[
      ...
      虚线表示模块内部的功能关系。公开载板资料没有给出 N100/N305
      封装球位、模块内层走线和电源时序细节，因此这里不冒充封装级原理图。
      实线从 260-pin 边界开始，对应载板原理图中的真实网络；红线表示功率从
      VDC 经 VIN 进入模块，方向与普通外设信号相反。
    };
  \end{tikzpicture}
  \caption{CPU/SoC 到载板的总接线索引。先按行确定 SoC 中负责该事务的控制器，
  再查中间列的 Mu 信号与物理 pin，最后追到右侧连接器或器件。}
