...
\code{command \&} 只允许 \code{\&} 位于整行最后。如果 \code{\&} 出现在中间，
解析器会在 fork/pipe 以前返回错误，错误码为
\code{BackgroundOperatorNotLast}。后台作业建立后立即返回提示符；
\code{jobs} 以 NoHang 收割事件；\code{bg} 发送 CONT 但不交出 TTY；
...
终端与第 17 章的磁盘请求复用 irq-save 临界区、WaitQueue 单赢家、deadline
