\section{Thread 怎样保存处理器现场}

前面的实现用一个 PCB 同时保存页表根、文件状态、动态内核栈和
...
Thread 队列 &
Process 链、双向 Ready queue 链 &
Running 不在 Ready queue；Ready 在队列中只出现一次\\ \hline
统计 &
run tick、dispatch、yield、preemption &
...
\section{Thread 怎样从创建走到退出}
Thread 状态机为：

\[
  \text{Unused}\rightarrow\text{Ready}
  \leftrightarrow\text{Running},\qquad
...
调度器校验时重新扫描对象，不直接相信累计计数。扫描完成后，下面的关系必须
成立：
...
等于全局 current 索引，且不得仍带 Ready queue 链接。Ready Thread 必须
恰好在队列中出现一次。扫描还限制遍历步数不超过 Thread
...
\topic{Ready queue 怎样在中断中移动 Thread}
Ready 队列把前后槽索引直接放进 Thread。入队写
\code{previous\_run\_thread\_index} 与
\code{next\_run\_thread\_index}，维护 head/tail；出队清除两个链接后才
把 Thread 变成 Running。Process 的 Thread 链只需单向遍历。队列结点直接
放在 Thread 中，不需要在 IRQ 或退出边界调用 heap。

\begin{contractbox}
先看实际动作：入队时同时写前驱、后继与 head/tail，出队时同时断开这些链接。
中途若只改一半，下一次遍历就会漏掉 Thread 或形成环。因此代码把“状态为
Ready”和“在 Ready queue 中恰好出现一次”作为同一临界区内必须维持的不变量。
\end{contractbox}
...
\code{FXSAVE64} 要求内存操作数按 16 字节对齐。代码用
...
敏感状态边界。本项目选择 eager：抢占、主动让出和退出保存 current，激活目标
Thread 恢复 next。多做一次固定 512 字节硬件事务的成本，换来没有隐藏 owner
的直接状态机。

\section{怎样验证上下文切换}
布局单元测试只能证明结构大小，累计计数只能证明函数被调用；二者都不能证明
保存区属于正确 Thread。四个 Ring 3 程序按 PID 安装互不相同的模式：
...
四个程序先安装不同的扩展状态，再执行长度不同的整数循环和系统调用。PIT
可以在任意一个循环中打断当前 Thread，调度器随后恢复另一个 Thread：

\begin{itemize}
  \item 每个程序写入不同的 XMM0、XMM15、MXCSR、x87 control word 与 ST0；
  \item 程序执行有界计算，使真实 1000 Hz PIT 至少发生一次抢占；
  \item 每次恢复后重新读取五组状态，任一字节属于其他 PID 都立即失败；
  \item 程序退出后，内核在永久栈安全点回收它的动态内核栈。
\end{itemize}
...
层次 & 输入 & 能证明的性质\\ \hline
单元 & 非法容量、256/512 对象、Ready queue 操作 &
创建失败后的清理、状态转换、FIFO、身份与计数\\ \hline
集成 & 三 Process、六 Thread、48 tick、主动让出与退出 &
多 Thread 公平性、Process/Thread 两级回收\\ \hline
随机 & 固定种子 100000 步操作序列 &
长序列槽复用、Ready queue、累计与当前量对照\\ \hline
QEMU & 真实 CPUID、CR0/CR4、PIT 和 Ring 3 &
硬件现场隔离、三档容量、失败停止与资源回收\\ \hline
\end{osgridlongtable}

随机模型的种子为 \code{0x5448524541445632}。每一步从 create process、
create thread、start、tick、yield、exit、reap 和 discard 中
选择一个动作，再与独立参考状态比较。测试不是在 100000 步结束时才
\code{Validate}；每一步都检查 current Thread、Ready queue、每个状态
集合、PID/TID 单调性和累计计数，因此首次破坏位置可以稳定复现。

回归加入扩展现场布局单元测试和 SSE2 缺失系统用例；旧
ProcessScheduler 测试由 ThreadScheduler 的单元、集成和十万步随机测试
替换。启动、损坏镜像、用户异常、文件持久化和命名门禁仍继续运行。

\topic{沿目录走读 Process 与 Thread}\par
...
\topic{当前实现范围}
本章只跟踪 Unused、Ready、Running 与 Exited。用户多线程接口以及等待、
唤醒和 futex 在第 28 章加入。Process/Thread 存储仍有 256/512 编译期上界；内核仍是单
BSP、内核不可抢占模型，没有 per-CPU Ready queue、远端 TLB shootdown
或 SMP 锁竞争。

\code{INT 0x80} 仍是唯一用户入口。后续章节再建立 CpuLocal、可信入口栈、
\code{swapgs} 与统一 UserContext，并加入 \code{syscall/sysret}。本章运行
结束时应看到四个程序各自的扩展状态标记、至少一次 PIT 抢占、四次正常退出，
以及动态内核栈和地址空间计数回到启动前的数值。
