\section{v1.14：从异步事件到受控用户 handler}
...
\subsection{处置、屏蔽和 pending 为什么属于不同所有者}
...
状态 & 所有者 & 不变量与设计原因\\
\midrule
disposition & Process & 同一程序映像共享 Default/Ignore/Handler 决策\\
process group & Process & 广播目标身份，不等同于父子进程树\\
Process pending & Process & 尚无合格 Thread 时仍保存已经发生的事实\\
mask & Thread & 一个执行流的临界区不应屏蔽全部兄弟\\
Thread pending & Thread & 普通信号已经完成唯一接收者选择\\
active frame & Thread & sigreturn 只能恢复本 Thread 的寄存器与栈\\
\bottomrule
\end{tabularx}
\caption{v1.14 信号状态的所有权分层}
\end{table}
...
\subsection{普通 pending 是位集合，不是次数}
...
\subsection{Default、Ignore 和 Handler 是三种提交}

\code{SignalAction} 固定为 40 字节，保存 disposition、handler、restorer、
additional mask 与 flags：

\begin{itemize}
  \item \textbf{Default}：Child 当前默认忽略；其他已冻结信号终止 Process，
...
信号无限递归堆叠；未来若支持 \code{nodefer}，必须新增冻结的 action flag，
不能偷偷改变现有行为。

\subsection{进程组不是进程树的另一种写法}
...
将用前台进程组产生 Ctrl-C，但 v1.14 只有身份和组发送，还没有 session、
控制终端或前后台作业。

\subsection{signal 怎样加入单赢家唤醒}

v1.13 已让 Blocked Thread 同时关联业务 WaitQueue 与 deadline。v1.14 不建立
...
\subsection{Interrupted 与 restart 必须按调用语义区分}
...
\subsection{用户 handler 的栈看起来像一次真正的 call}
...
\subsection{SignalFrame 与 x86-64 标志位}
...
位/字段 & 处理 & 原因\\
\midrule
CF/PF/AF/ZF/SF/OF & 恢复 & 普通整数运算结果属于用户现场\\
DF & 按统一白名单恢复 & 影响字符串指令方向，必须与通用返回契约一致\\
IF & 要求用户许可形态 & 不能让伪造 frame 破坏 Kernel 中断策略\\
IOPL/NT/VM & 拒绝危险值 & 不能通过 sigreturn 获得端口或旧任务/虚拟 8086 语义\\
RF/TF & 按项目白名单处理 & 调试与异常控制必须显式受控\\
CS/SS & 固定用户 selector & 返回只能进入 CPL3 代码/数据段\\
\bottomrule
\end{tabularx}
...
\subsection{主栈按需扩展与用户 Thread 栈边界}

主用户栈保留 8 MiB VMA，启动只提交参数真正占用的高端页面。SignalFrame 位于
当前 RSP 下方，可能跨入紧邻未提交页。v1.14 的
\code{PrepareUserStackRange} 验证 frame 与当前 RSP 均在冻结主栈窗口，然后
...
\subsection{为什么 magic 之外还需要一次性 cookie}
...
\subsection{fork、exec 与退出怎样迁移信号状态}
...
pending、活动 frame 和被删除兄弟 Thread 的状态清空，survivor mask 保留。
PID、父子关系与进程组没有变化，因为 exec 替换程序映像而非 Process 身份。
...
调用 & 编号 & 语义\\
\midrule
\code{SetSignalAction} & 57 & 安装/读取 Process 处置\\
\code{SetSignalMask} & 58 & 替换当前 Thread mask 并返回旧值\\
\code{SendProcessSignal} & 59 & 向指定 PID 或自身发送普通信号\\
\code{SendProcessGroupSignal} & 60 & 向平坦进程组成员逐个发送\\
\code{SignalReturn} & 61 & 从精确活动 frame 恢复用户现场\\
\code{GetProcessGroup} & 62 & 读取 Process 组号\\
\code{SetProcessGroup} & 63 & 设置本 Process 组号\\
\bottomrule
\end{tabularx}
\caption{v1.14 固定宽度系统调用增量}
\end{table}

\code{Interrupted=-52}、\code{ProcessNotFound=-53} 与
\code{SignalStateInvalid=-54} 使用显式固定宽度枚举；action、context 与
frame 大小分别冻结为 40、176 与 240 字节。ABI 不包含平台相关的
\code{long}、\code{size\_t} 或 Kernel 指针。

\subsection{四层测试与真实机器证据}
...
v1.14 已完成普通信号、进程组、用户 handler、严格 sigreturn 和可中断/可重启
阻塞。TTY、session、前台组、Ctrl-C、stop/continue、alternate stack、实时
信号队列和 \code{siginfo\_t} 均留给后续阶段。下一阶段应让 i8042 输入先经过
TTY line discipline，再由 TTY 向前台进程组发送信号；键盘 IRQ 不应直接选择
某个 PID。
