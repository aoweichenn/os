...
\topic{为什么选择 ordered metadata，而不是记录全部数据}
常见 journal 策略可分成三类：
...
前一章已经建立 dirty/writeback/error page、BlockRequest 和 ATA FLUSH
路径。这里选择 ordered mode，因为它能直接复用 ``相关数据已稳定''
...
相对块 & 数量 & 含义\\ \hline
0 & 1 & superblock home\\ \hline
1--4226 & 4226 & 位图与 inode table\\ \hline
4227--4482 & 256 & journal\\ \hline
4483--524287 & 519805 & 目录和普通文件数据\\ \hline
\end{osgridtabular}
\end{center}
...
\begin{center}
\begin{osgridtabular}{lll}
journal 相对块 & 数量 & 内容\\ \hline
0 & 1 & header\\ \hline
1--4 & 4 & descriptor blocks\\ \hline
5--128 & 124 & metadata payload blocks\\ \hline
129 & 1 & commit record\\ \hline
130--255 & 126 & 必须保持为零的保留区\\ \hline
\end{osgridtabular}
\end{center}
...
\[
  4\times31=124.
\]
其余 descriptor 字节必须为零并纳入 CRC。这里的 124 来自盘面字段宽度，而不是
随手散落在代码中的数字；格式工具、Kernel 和测试读取同一显式常量契约。

\begin{contractbox}
教材中的容量必须以当前盘面字段为准。若未来把每个 descriptor 块扩到理论
最大 entry 数，那是盘面格式升级，不能只改一个循环上限。
\end{contractbox}

\topic{header、descriptor、payload 与 commit 怎样互相约束}
...
overlay 并恢复快照。若未来支持多个并发事务，应先规定锁顺序、reservation
所有权和 checkpoint 调度，不能简单把固定数组复制到每个 Thread。

\topic{overlay 让事务读到自己的写}
Create 可能先更新 inode block，随后又因目录插入读取同一块。如果 read 只看
BlockCache，就会观察旧字节。metadata read 顺序为：
...
\topic{断电以后为什么只接受两种磁盘状态}
\begin{center}
\begin{osgridtabularx}{\textwidth}{p{0.24\textwidth}XX}
断电窗口 & mount 看到的证据 & 恢复结果\\ \hline
data 或 prepared 写入前/中 &
没有有效 commit &
保留旧 metadata，丢弃完整 prepared 或拒绝损坏格式\\ \hline
prepared FLUSH 后 &
payload 完整但 commit 缺失 &
清理 incomplete，保留旧状态\\ \hline
commit 写入中 &
字段或 CRC 不完整 &
不能伪造 commit；按严格分类丢弃或拒绝\\ \hline
commit FLUSH 后 &
header、payload、commit 全部匹配 &
replay 全部 target，得到完整新状态\\ \hline
checkpoint 中 &
部分 home 新、commit 仍完整 &
重新 replay 全部 target，得到完整新状态\\ \hline
...
checkpoint 或其 FLUSH 失败时不能清除 commit；当前文件系统实例停止接受写入，下一
次 mount 仍有权威恢复材料。只有 home 全部稳定后才能清 journal，并再次
FLUSH 清理结果。

\section{怎样验证 replay 和再次断电}
...
规定。\code{fs/root\_file\_system\_format.*} 只编码/解析小端字段。
...
后变化，任何返回失败的操作都必须保持 durable 原值。固定 seed 使任何失败可复现，而不是用 ``随机
跑过一次'' 代替证明。
...
\topic{当前 journal 的范围}
当前不包含 data journal、后台 checkpoint、并行 transaction、SMP、
\code{fsync}/\code{fdatasync}/\code{msync}、snapshot、在线扩容、AHCI 或
NVMe。CRC 用于发现偶发损坏，不能抵抗主动篡改。

覆盖的断电点恢复后，多个 metadata 修改只会呈现旧状态或新状态。第 19 章
再从用户 ABI、procfs 观察面和畸形输入检查整个系统，不继续扩大 journal
本身的职责。
