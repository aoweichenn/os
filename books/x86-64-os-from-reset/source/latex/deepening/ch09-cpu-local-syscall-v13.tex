\section{v1.3 从通用中断入口走向 x86-64 原生系统调用}
...
\begin{longtable}{L{0.18\textwidth}L{0.23\textwidth}L{0.24\textwidth}L{0.25\textwidth}}
\toprule
机制 & 入口来源 & 是否自动换栈 & 返回与主要代价\\
\midrule
call gate & GDT/LDT gate & 按 TSS 与 gate 规则 & RETF；描述符语义复杂\\
\code{INT n} & IDT interrupt/trap gate & 是，使用 TSS.RSP0 & IRETQ；通用且路径较长\\
SYSENTER & SYSENTER MSR & 不自动保存完整返回现场 & SYSEXIT；主要面向 32 位\\
SYSCALL & STAR/LSTAR/FMASK & 否 & SYSRETQ；软件承担可信栈与返回校验\\
\bottomrule
\end{longtable}
...
\begin{longtable}{L{0.24\textwidth}L{0.16\textwidth}L{0.21\textwidth}L{0.27\textwidth}}
\toprule
CPUID 位置 & 位 & 名称 & 本阶段用途\\
\midrule
\code{01H:EDX} & 24 & FXSR & FXSAVE/FXRSTOR 存在\\
\code{01H:EDX} & 25 & SSE & XMM/MXCSR 架构状态存在\\
\code{01H:EDX} & 26 & SSE2 & 当前 64 位用户指令基线\\
\code{80000001H:EDX} & 11 & SYSCALL & 原生入口/返回存在\\
\code{80000001H:EDX} & 20 & NX & 页表执行禁止能力存在\\
\code{80000001H:EDX} & 29 & LM & long mode 存在\\
\code{80000008H:EAX} & 7:0 & physical width & 必须位于 36--52\\
\code{80000008H:EAX} & 15:8 & linear width & 当前精确要求 48\\
\bottomrule
\end{longtable}
...
\subsection{为什么当前只接受 48 位，不顺便开放 LA57}
...
MSR & 地址 & 当前合同\\
\midrule
IA32\_EFER & \code{0xC0000080} & 保留 LME/LMA/NXE，设置 SCE bit 0\\
IA32\_STAR & \code{0xC0000081} & kernel selector base 与 SYSRET selector base\\
IA32\_LSTAR & \code{0xC0000082} & 64 位入口 \code{OsKernelNativeSystemCallEntry}\\
IA32\_FMASK & \code{0xC0000084} & 进入时清 TF、IF、DF、NT、AC\\
IA32\_GS\_BASE & \code{0xC0000101} & 当前用户 GS base 为 0\\
IA32\_KERNEL\_GS\_BASE & \code{0xC0000102} & \code{CpuLocal} 地址\\
\bottomrule
\end{longtable}
...
\subsection{FMASK 改变内核活动标志，不抹掉用户原值}

SYSCALL 先把用户 RFLAGS 复制到 R11，再从当前活动 RFLAGS 清除 FMASK 指定
的位。当前 \code{FMASK=0x44700}：

\begin{longtable}{L{0.12\textwidth}L{0.18\textwidth}L{0.58\textwidth}}
\toprule
位 & 名称 & 入口必须清除的原因\\
\midrule
8 & TF & 防止用户单步在可信栈形成前触发调试异常\\
9 & IF & 完整保存现场前不能接受可屏蔽 IRQ\\
10 & DF & C++ 与字符串操作要求向高地址方向\\
14 & NT & 不继承遗留嵌套任务语义\\
18 & AC & 不把用户对齐检查状态带入内核\\
\bottomrule
\end{longtable}
...
\section{CpuLocal 把每 CPU 入口状态收束为一个对象}
...
\begin{longtable}{L{0.13\textwidth}L{0.31\textwidth}L{0.44\textwidth}}
\toprule
偏移 & 字段 & 不变量\\
\midrule
0 & self address & 初始化后等于对象自身地址\\
8 & current Thread index & 与 scheduler current 一致；无 Thread 为 UINT64\_MAX\\
16 & kernel entry RSP & 当前 Thread 动态栈顶，非零且 16 B 对齐\\
24 & syscall user RSP scratch & 只由原生入口换栈前暂存，退出入口时清零\\
\bottomrule
\end{longtable}
...
\subsection{SWAPGS 为什么能在不占通用寄存器时找到 CpuLocal}
...
\section{两条入口规范化为同一份 UserContext}

原 \code{ExceptionFrame} 保存 15 个通用寄存器、vector、error code、
RIP、CS 与 RFLAGS，共 160 字节。来自 Ring 3 的现场还具有 RSP 与 SS，
统一结构因此为 176 字节：

\begin{longtable}{L{0.17\textwidth}L{0.17\textwidth}L{0.26\textwidth}L{0.28\textwidth}}
\toprule
偏移 & 长度 & 字段组 & 来源\\
\midrule
0--119 & 120 B & R15..RAX & 汇编逐项保存/人工初始现场\\
120--127 & 8 B & vector & Initial、IRQ、INT80、SYSCALL\\
128--135 & 8 B & normalized error & 系统调用与 IRQ 为零\\
136--159 & 24 B & RIP、CS、RFLAGS & CPU 帧或 RCX/R11 规范化\\
160--175 & 16 B & RSP、SS & CPU 特权帧或原生入口人工补齐\\
\bottomrule
\end{longtable}

vector 不表示“发生异常”，而是统一现场的来源标签：

\begin{longtable}{L{0.28\textwidth}L{0.20\textwidth}L{0.39\textwidth}}
\toprule
来源 & vector & 最终返回基线\\
\midrule
首次进入用户态 & 0 & IRETQ\\
8259A IRQ & 32--47 & IRETQ\\
\code{INT 0x80} & \code{0x80} & IRETQ\\
\code{SYSCALL} & \code{0x81} & 满足快速白名单时 SYSRETQ，否则 IRETQ\\
\bottomrule
\end{longtable}
...
\subsection{非法返回为什么终止用户进程而不是内核 panic}
...
\section{IRQ 可以打断系统调用，但不能在任意内核调用链换栈}
...
\section{非局部返回暴露了 SWAPGS 的真正状态所有权}
...
\section{NMI 必须承认自己可能落在最危险的两条指令之间}
...
\section{四层测试从不同位置观察同一合同}

\begin{longtable}{L{0.17\textwidth}L{0.30\textwidth}L{0.41\textwidth}}
\toprule
层次 & 主要输入 & 能证明的事实\\
\midrule
单元 & CPUID 叶、UserContext 边界 & profile、canonical、段、RFLAGS 和返回选择\\
集成 & CpuLocal 与六 MSR 纯布局 & 深度/统计/清理、STAR 算术与回读比较\\
随机 & 100000 个确定性现场 & 400000 项规范性、合法性、选择和拒绝性质\\
QEMU & 真实 SYSCALL/SYSRET/IRQ/MSR & 可信换栈、双入口、IRQ 打断、调度与失败停止\\
\bottomrule
\end{longtable}
...
\section{调试 ELF 与启动 payload 分离是本阶段的工程修复}
...
CPUID 最大叶、六位 mask 和地址宽度为何是一份 profile？\\
\filepath{arch/cpu_local.hpp/.cpp} &
汇编 ABI 字段、current Thread、深度和统计如何保持一致？\\
\filepath{arch/native_system_call_layout.hpp/.cpp} &
为什么 STAR 用户基值是 \code{0x10}，六项回读如何比较？\\
\filepath{arch/native_system_call.hpp/.cpp} &
纯布局怎样落实为真实 RDMSR/WRMSR，又如何区分失败状态？\\
\filepath{arch/user_context.hpp/.cpp} &
四类来源如何共享 176 字节布局，合法集合与快速集合如何区分？\\
\filepath{arch/architecture.asm} &
SWAPGS、换栈、STI 时刻、返回弹栈和非局部恢复分别在哪里？\\
\filepath{user/system_calls.cpp} &
入口所有权、映射验证、need-resched、非法返回循环和返回选择如何分层？\\
\filepath{process/process_runtime.cpp} &
TSS/CpuLocal 如何同步，Thread 切换和永久栈恢复如何收束 GS？\\
\filepath{user/src/system_call.asm} &
默认、兼容和 DF 测试包装如何保持同一参数 ABI？\\
\bottomrule
\end{longtable}
...
系统调用背后的描述符仍是 v1.0 固定槽。v1.4 将建立类型化 KernelObject、
动态 FileTable 和共享 FileDescription，但不得改写本章已经冻结的架构入口、
UserContext 和返回验证。分层的意义就在这里：对象层可以演进，特权边界不需要
跟着重新冒险。
