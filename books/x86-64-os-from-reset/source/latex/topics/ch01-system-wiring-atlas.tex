\topic{把 learning 图册读成同一台机器}\par
图里的方框不是都代表一颗芯片，线也不是都代表 PCB 上的一根铜线。整机学习图
把“物理部件、处理器接口、软件状态和宿主测试边界”放在同一张因果地图中；
读图时先确认每个对象属于哪一层，再判断每条线表达哪种事务。箭头名称留到
这两步之后阅读。

本书把 \filepath{docs/learning/} 中的七张整机学习图全部纳入对应章节。它们与
“CPU 怎样找到内存和设备”一章的实体载板图承担不同职责：整机图解释 STUOS 在 QEMU PC 模型中看见的
逻辑硬件和软件交接，实体图解释 LattePanda Mu 载板上的真实器件引脚、电源和
PCB 网络。前者不能拿去布板，后者也不会自动证明内核已经实现设备驱动。

\topic{一、先认清图中的四类对象}\par
\begin{osgridlongtable}{L{0.18\linewidth}L{0.27\linewidth}L{0.27\linewidth}L{0.18\linewidth}}
对象 & 它在图中表示什么 & 它不表示什么 & 核对问题 \\ \hline
硬件卡片 & CPU、ROM、RAM、PIC、PIT、UART、磁盘等可见设备 & C++ 类本身就是芯片 & 谁提供寄存器或存储行为 \\ \hline
软件卡片 & 固件、Stage 1、内核、驱动、进程与 Shell 的状态阶段 & PCB 上存在同名器件 & 代码在哪个特权级执行 \\ \hline
边界/泳道 & Host、Guest hardware、Ring 0、Ring 3 等所有权区域 & 只是为了美观的背景色 & 数据跨边界时由什么协议验证 \\ \hline
连接线 & 一类控制流、地址事务、端口事务、IRQ 或测试注入 & 默认都是一根可焊接导线 & 源、目的、方向和完成条件是什么 \\ \hline
\end{osgridlongtable}
...
\topic{二、四种线为什么绝不能混为一谈}\par
\begin{osgridlongtable}{L{0.16\linewidth}L{0.24\linewidth}L{0.31\linewidth}L{0.19\linewidth}}
图中线型 & 表达的事务 & 典型例子 & 不能推出 \\ \hline
绿色实线 & 内存/物理地址或 MMIO 访问 & CPU 取 ROM、访问 RAM、读 Local APIC & 图上是一条共享物理线 \\ \hline
蓝色虚线 & x86 独立 Port I/O 事务 & \code{IN/OUT} 到 COM1、PIT、PIC、ATA & 端口号等于物理内存地址 \\ \hline
红色实线 & 异步 IRQ 通知路径 & PIT/键盘到 PIC，再桥接到 Local APIC & 数据字节本身沿 IRQ 线传输 \\ \hline
紫色虚线 & 宿主控制、注入或验证 & QMP 注入按键、新 QEMU 进程重启验证 & 宿主绕过客户机设备和驱动 \\ \hline
\end{osgridlongtable}
...
\topic{3.3 CPU 方框为什么同时出现 MMU、描述符表和 Ring}
...
\topic{3.4 Port I/O 总线是架构空间，不是一根现代排线}
...
\topic{3.5 PIC 与 Local APIC 为什么同时存在}
...
\begin{sidewaysfigure}[p]
  ...
  当前 v1.1--v1.8 的资源、Thread、SYSCALL、动态文件表、VFS、rootfs v2、
  PID1 进程树与匿名虚拟内存增量。}
  \label{fig:hardware-stage-roadmap}
\end{sidewaysfigure}
\clearpage

路线图把第一周期分成四组，避免同时引入太多还无法观察的状态：

\begin{osgridlongtable}{L{0.14\linewidth}L{0.21\linewidth}L{0.30\linewidth}L{0.25\linewidth}}
阶段 & 增量 & 新获得的能力 & 为什么不能提前 \\ \hline
A 工程与自举 & v0.0--v0.4 & 可复现构建、ROM、串口、Stage 1、长模式、ELF 与 BootInfo & 没有前一层便无法观察或装入后一层 \\ \hline
B Kernel 接管 & v0.5--v0.7 & GDT/TSS/IDT、物理内存、页表、异常、中断和设备 & 内核先要能隔离故障，才适合接受异步事件 \\ \hline
C 受保护并发 & v0.8--v0.10 & Ring 3、系统调用、进程、抢占、阻塞与 IPC & 用户边界依赖页权限、内核栈和异常入口 \\ \hline
D 持久化与交互 & v0.11--v1.0 & 文件系统、统一 fd、控制台与 Shell & 交互命令要依赖调度、驱动和持久对象的完整链 \\ \hline
\end{osgridlongtable}
...
则让 rootfs v2 承担可扩展持久化与完整命名空间修改，v1.7 再让 PID1
从磁盘启动，并以 \code{spawn/exec/wait} 建立可回收的父子树；v1.8
把 VMA 意图与 PTE 驻留分开，以真实页故障提交匿名页、连续栈和 program
break，并在 Ring 3 建立可回收用户堆。它们在
v1.0 已闭合的启动、用户、文件和
测试链上继续强化生命周期与接口，不改写“先有机器状态，后有软件抽象”的依赖
方向。

\topic{五、读任意一张硬件学习图的七问法}
