...
\code{0x00014000..0x00014050} & BootInfo & 独立页面内的 80 字节 ABI\\
...
要求 R 权限和 W\^X 不是当前大页已经能强制出来的最终安全策略；初始 2 MiB
大页仍以较粗权限映射。它先保证 ELF 自身给出的意图没有矛盾，等 v0.6 拆成
4 KiB 页时可直接依据段权限建立页表。
...
\code{0x08} & version & 1\\
\code{0x10} & structure size & 80 字节\\
...
\code{BOOT\_INFO\_READY} & 十字段交接结构已写完\\
...
\code{READY} & 当前 v0.5 的内核入口、描述符与异常自检全部完成\\
\bottomrule
\end{longtable}
...
当前完整命令运行 36 项 CTest。测试代码、宿主 Python、书稿和网站不计入操作
