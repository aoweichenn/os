...
固定 LBA 65 并不浪费重要容量：当前 2 MiB 教学磁盘包含 4096 个扇区，而稳定
...
\section{v0.4 的内存布局是一组同时成立的区间证明}

\subsection{为什么映射从 2 MiB 扩展到 64 MiB}
...
\subsection{物理区间表同时是所有权表}

\small
\begin{longtable}{L{0.27\textwidth}L{0.27\textwidth}L{0.34\textwidth}}
\toprule
\textbf{半开区间} & \textbf{所有者} & \textbf{为何放在这里}\\
\midrule
\endfirsthead
\toprule
\textbf{半开区间} & \textbf{所有者} & \textbf{为何放在这里}\\
\midrule
\endhead
\code{0x00008000..0x00010000} & Stage 1 & 与实模式加载位置一致，最大 32 KiB\\
\code{0x00010000..0x00013000} & 页表 & 三张 4 KiB 对齐表\\
\code{0x00013000..0x00013200} & Kernel 描述符 & 一个完整磁盘扇区\\
\code{0x00014000..0x00014068} & BootInfo v2 & 独立页面内的 104 字节 ABI\\
\code{0x00015000..0x00016000} & 加载器临时状态 & 程序头遍历索引和计数\\
\code{0x00016000..0x00017000} & 内存图元数据 & 交接条目数量\\
\code{0x00017000..0x00018000} & \code{fw\_cfg} 暂存 & 当前 56 字节目录名称\\
\code{0x00018000..0x00018C00} & 物理内存图 & 最多 128 个 24 字节项\\
\code{0x00100000..0x03E00000} & \code{PT\_LOAD} 窗口 & 与高地址源暂存区严格分离\\
\code{0x03E00000..0x03F00000} & Kernel 文件暂存 & 经 E820 验证的 1 MiB 上限\\
\code{0x03FEF000..0x03FFF000} & 初始内核栈 & 64 KiB，向低地址增长\\
\bottomrule
\end{longtable}
\normalsize

表中的空洞不是遗漏。描述符之后按页分开 BootInfo 和临时状态，使 GDB 可以
按稳定地址检查，也给结构增长留下明确边界。v0.10 把增长中的 Kernel 文件
暂存迁到 62--63 MiB；Stage 1 在读盘前要求该完整区间落入同一个 E820
usable 条目，不能只凭 \code{-m 64} 猜测可写。加载目标结束于
\code{0x03E00000}，因此两遍加载第二遍复制时不可能覆盖仍在读取的源 ELF。

区间全部采用半开语义。验证段结束地址时先证明
\(\mathrm{begin}+\mathrm{size}\) 不发生 64 位溢出，再比较结束地址不超过
窗口上界；不能先做可能回绕的加法，然后用回绕后的“小数”通过检查。

\section{长模式中的 ATA PIO 仍然是同一硬件协议}

\subsection{寄存器变宽不会让端口字段变宽}
...
当前每次等待最多轮询 \code{0x000FFFFF} 次。Kernel 接近暂存窗口上限时会
提交上千个单扇区事务，较早的 \code{0x0000FFFF} 次预算在宿主繁忙、TCG
线程被抢占时曾把正常设备误判为超时；扩大预算是在“正常调度抖动”与“永久忙
尽快失败”之间重新取界，并没有删除超时。固定次数仍不等于稳定墙钟，进入拥有
单调时钟的内核后必须改用 deadline；Stage 1 的外层 QEMU 测试也继续设置
独立墙钟上限。
...
\section{CRC32 从多项式除法落到逐位汇编}

\subsection{为什么它比简单字和更适合 Kernel}
...
路径可以直接与宿主标准库结果对照；当前最大 1 MiB 文件仍有明确上限。

\subsection{描述符、自身字段和尾部填充}
...
\section{两遍 ELF 验证把失败保持为无副作用}

\subsection{第一层从文件身份建立边界}
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 内存区间完整落入 \code{0x00100000..0x03E00000}。
\end{enumerate}
...
\section{平台配置接口把物理内存事实交给自研加载器}

\subsection{没有外部 BIOS，就不能调用一个不存在的 E820 服务}
...
\subsection{目录流式遍历避免新的早期内存依赖}

每个目录项固定 64 字节：

\begin{longtable}{L{0.18\textwidth}L{0.19\textwidth}L{0.49\textwidth}}
\toprule
\textbf{偏移} & \textbf{宽度} & \textbf{语义}\\
\midrule
\endfirsthead
\toprule
\textbf{偏移} & \textbf{宽度} & \textbf{语义}\\
\midrule
\endhead
\code{0x00} & 32 bit & 文件 size，大端\\
\code{0x04} & 16 bit & 重新选择文件所需 selector，大端\\
\code{0x06} & 16 bit & reserved\\
\code{0x08} & 56 byte & NUL 结尾的文件名\\
\bottomrule
\end{longtable}
...
\section{BootInfo 把寄存器瞬间变成版本化事实}

\subsection{十三个 64 位字段为何全部显式}

\small
\begin{longtable}{L{0.14\textwidth}L{0.33\textwidth}L{0.39\textwidth}}
\toprule
\textbf{偏移} & \textbf{字段} & \textbf{当前值或来源}\\
\midrule
\endfirsthead
\toprule
\textbf{偏移} & \textbf{字段} & \textbf{当前值或来源}\\
\midrule
\endhead
\code{0x00} & magic & 内存字节为 \code{OSBOOT64}\\
\code{0x08} & version & 2\\
\code{0x10} & structure size & 104 字节\\
\code{0x18} & Kernel 文件地址 & \code{0x03E00000}\\
\code{0x20} & Kernel 文件长度 & 已通过 CRC 的精确长度\\
\code{0x28} & Kernel 入口 & \code{0x100000}\\
\code{0x30} & 加载段数量 & 第一遍验证所得，1 到 64\\
\code{0x38} & 页表根 & \code{0x10000}\\
\code{0x40} & 恒等映射大小 & \code{0x4000000}，即 64 MiB\\
\code{0x48} & 初始栈顶 & \code{0x3FFF000}\\
\code{0x50} & 物理内存图地址 & \code{0x18000}\\
\code{0x58} & 物理内存图条目数 & 已验证的 1 到 128\\
\code{0x60} & 内存图条目宽度 & 24 字节\\
\bottomrule
\end{longtable}
\normalsize

这些字段都用 64 位不是为了“数值越大越好”，而是为了统一跨阶段 ABI 并给
...
\subsection{System V AMD64 ABI 规定第一次函数调用}
...
\section{内核入口建立独立可观察性}

\subsection{重新初始化串口不是重复劳动}
...
\subsection{成功日志是一条逐步增强的证据链}

\begin{longtable}{L{0.38\textwidth}L{0.48\textwidth}}
\toprule
\textbf{标记} & \textbf{它实际证明的事实}\\
\midrule
\endfirsthead
\toprule
\textbf{标记} & \textbf{它实际证明的事实}\\
\midrule
\endhead
\code{KERNEL\_HEADER\_VALID} & 目标 ATA 读取和描述符字段/CRC 成功\\
\code{KERNEL\_PAYLOAD\_VALID} & 精确 ELF CRC 和扇区补零成功\\
\code{KERNEL\_ELF\_VALID} & 第一遍 ELF 与全部段关系成功\\
\code{KERNEL\_SEGMENTS\_LOADED} & 第二遍复制与 BSS 清零完成\\
\code{BOOT\_INFO\_READY} & 十三字段 BootInfo v2 已写完\\
\code{KERNEL\_TRANSFER} & RDI、RSP 已准备，即将调用入口\\
\code{ENTERED} & C++ 入口和独立 COM1 代码确实执行\\
\code{BOOT\_INFO\_VALID} & 内核逐字段接受交接 ABI\\
\code{BSS\_ZEROED} & 加载器满足 C++ 零初始化前提\\
\code{CR3\_VALID} & 处理器实际页表根与交接事实一致\\
\code{MEMORY\_MAP\_VALID} & 内核接受排序、无重叠且无溢出的物理内存图\\
\code{PAGING\_READY} & 内核四级 4 KiB 页表已经激活\\
\code{HEAP\_SELF\_TEST\_PASSED} & 高半区堆真实写入、释放、合并与一致性检查成功\\
\code{USER\_RETURNED\_TO\_KERNEL} & 用户 exit 或异常已恢复原内核调用链\\
\code{READY} & 当前 v1.0 的入口、异常、内存、设备、进程、IPC、文件系统与交互环境自检全部完成\\
\bottomrule
\end{longtable}

每扇区、每字节和每次轮询都不打印，避免串口耗时反过来改变启动行为。QEMU
宿主读取线程给每行加 \code{[QEMU][T+...ms]} 单调时间，这只表示宿主观察到
日志的时刻；来宾尚未有 IRQ0 软件滴答，因此不伪造操作系统时间。

\section{四层测试共同证明完整交接}
...
CRC，使目标必须通过两层完整性检查后才在 ELF 语义处失败；ATA 超时和 ERR
变体则直接作用于 Stage 1 的轮询函数。成功路径与五类 Kernel 失败路径共同
保证日志没有把错误归入笼统的“启动失败”。

当前完整命令运行 107 项 CTest。测试代码、宿主 Python、书稿和网站不计入操作
系统本体代码量；统计只覆盖 \filepath{source/} 中实际进入目标系统的 C++、
头文件和 Intel 语法汇编。这样增长曲线描述的是机器上真实执行的实现，不会被
测试数量和文档篇幅稀释。
