...
\code{0x00020000..0x000A0000} & Kernel 文件暂存 & 512 KiB 的连续上限\\
\code{0x00100000..0x03F00000} & \code{PT\_LOAD} 窗口 & 避开低端加载器与高端栈\\
...
按稳定地址检查，也给结构增长留下明确边界。暂存区结束于传统 PC 的
\code{0xA0000}，避免把普通 RAM 假设延伸进 VGA 与固件保留窗口。加载窗口
结束于 \code{0x03F00000}，在 64 MiB 映射末尾之前给内核栈留出空间。
...
路径可以直接与宿主标准库结果对照；当前最大 512 KiB 文件仍有明确上限。
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item 内存区间完整落入 \code{0x00100000..0x03F00000}。
\end{enumerate}
...
\code{0x18} & Kernel 文件地址 & \code{0x20000}\\
...
同时，大类型也不取消语义上限：文件仍最多 512 KiB，段数仍最多 64。
...
\code{READY} & 当前 v0.9 的入口、异常、内存、设备、用户边界与进程调度自检全部完成\\
\bottomrule
\end{longtable}
...
当前完整命令运行 59 项 CTest。测试代码、宿主 Python、书稿和网站不计入操作
