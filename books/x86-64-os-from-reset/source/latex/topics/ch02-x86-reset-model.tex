\section{x86 复位状态与第一条指令}

\subsection{复位是契约，不是初始化完成}
...
\begin{longtable}{L{0.16\textwidth}L{0.22\textwidth}L{0.48\textwidth}}
\toprule
\textbf{字节位置} & \textbf{数值} & \textbf{处理器解释}\\
\midrule
\endfirsthead
\toprule
\textbf{字节位置} & \textbf{数值} & \textbf{处理器解释}\\
\midrule
\endhead
\code{0xFFFFFFF0} & \code{E9} & 选择近相对跳转指令\\
\code{0xFFFFFFF1} & \code{0D} & 位移低字节\\
\code{0xFFFFFFF2} & \code{F0} & 位移高字节；组合为 \code{0xF00D}\\
\code{0xFFFFFFF3} & \code{FF} & 不属于该指令；正常执行不会到达\\
\bottomrule
\end{longtable}
