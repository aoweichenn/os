...
\toprule
位模式 & 无符号解释 & 16 位补码解释 & 固件含义 \\
\midrule
\code{0x0003} & 3 & 3 & near jump 指令长度 \\
\code{0xF000} & 61440 & -4096 & 目标 IP 按无符号偏移使用 \\
\code{0xF00D} & 61453 & -4083 & near jump 有符号位移 \\
\code{0xFFFF} & 65535 & -1 & 串口轮询次数按无符号使用 \\
\bottomrule
\end{longtable}

\subsection{三、小端序来自字节地址顺序}
...
对象 & 常见对齐 & 空出的地址低位 & 用途 \\
\midrule
16 位端口字 & 2 字节 & bit 0 & 总线字节通道选择 \\
32 位字段 & 4 字节 & bit 1--0 & ABI 与访存效率 \\
4 KiB 页面 & 4096 字节 & bit 11--0 & 页内偏移或页表标志 \\
2 MiB 大页 & 2097152 字节 & bit 20--0 & 大页页内偏移 \\
\bottomrule
\end{longtable}

\subsection{六、文本最终仍是字节协议}
