...
因此 Stage 1 要经过两个跳转。第一次设置 CR0.PE，并通过远跳转装入 32 位
代码段；第二次在页表和长模式控制位准备好以后，再远跳到 L 位代码段。只修改
汇编文件中的 \code{BITS 32} 或 \code{BITS 64} 不会改变 CPU 模式，那只是
告诉 NASM 应该生成哪一种指令编码。
...
\begin{tikzpicture}[os diagram]
  ...
  \node[os block,right=of protected] (paging) {PAE 与四级页表};
...
\begin{pdfdiagram}
  ...
  \node[os state,below=of lme] (paging) {CR0.PG=1\\硬件置 LMA=1};
...
第十二章建立 4 KiB 页面时会继续使用 20:12 位的 PT 索引。
