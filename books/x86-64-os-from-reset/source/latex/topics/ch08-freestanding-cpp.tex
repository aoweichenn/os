...
\begin{longtable}{L{0.23\textwidth}L{0.29\textwidth}L{0.32\textwidth}}
\toprule
\textbf{能力} & \textbf{普通应用中的来源} & \textbf{本项目中的建立方式}\\
\midrule
\endfirsthead
\toprule
\textbf{能力} & \textbf{普通应用中的来源} & \textbf{本项目中的建立方式}\\
\midrule
\endhead
初始栈 & 进程装载器与内核 & 启动汇编选择保留 RAM 并对齐\\
零初始化区 & 装载器或 C 运行时 & 入口按链接符号主动清零\\
全局构造 & CRT 初始化数组 & 后续自研初始化遍历或避免依赖\\
动态内存 & libc 与系统调用 & 物理页、虚拟内存和内核堆\\
异常展开 & ABI 运行库与展开表 & 初期禁用，使用显式状态\\
程序退出 & 运行库调用操作系统 & 内核停机、重启或结束任务\\
\bottomrule
\end{longtable}

\subsection{二、编译器选项固定生成代码边界}
...
\[
  \mathrm{maximumSize}=UINT64\_MAX-begin.
\]
只有 \code{size <= maximumSize} 时才执行加法。失败通过强枚举
\code{AddressRangeCreationStatus} 返回，不依赖异常、全局错误码或宿主
运行库。

\subsection{六、头文件与源文件承担不同责任}
...
\subsection{八、volatile、原子操作与内存顺序}
