...
\toprule
ROM 字节 & Intel 语法 & 架构变化 & 固件目的 \\
\midrule
\code{FA} & \code{cli} & IF 清零 & IDT 建立前关闭中断 \\
\code{FC} & \code{cld} & DF 清零 & 字符串读取向高地址推进 \\
\code{31 C0} & \code{xor ax,ax} & AX 变为零 & 为段寄存器准备零值 \\
\code{8E D8} & \code{mov ds,ax} & DS 缓存重载 & 数据段基址归零 \\
\code{BC 00 70} & \code{mov sp,0x7000} & SP 变为 0x7000 & 建立向低地址增长的栈 \\
\bottomrule
\end{longtable}

\subsection{二、Intel 语法的操作数顺序}
...
\subsection{九、宏、常量与编译边界}

NASM 的 \code{equ} 在汇编期建立具名常量，不产生存储。端口、位掩码、消息
地址和轮询上限都使用带项目与模块前缀的名称。故障镜像使用一个构建期条件符号
改变测试状态；它被限制在固件构建边界，不承担普通控制流或领域建模。

\subsection{十、逐条执行 trace 的记录格式}

可靠走读同时记录地址、字节、指令、输入状态、输出状态和下一地址。仅列汇编
助记符会漏掉链接地址和机器码，仅列十六进制又无法表达寄存器意图。

\begin{longtable}{L{0.13\linewidth}L{0.16\linewidth}L{0.18\linewidth}L{0.23\linewidth}L{0.20\linewidth}}
\toprule
物理地址 & 字节 & 指令 & 状态变化 & 下一地址 \\
\midrule
FFFFF000 & FA & \code{cli} & IF=0 & FFFFF001 \\
FFFFF001 & FC & \code{cld} & DF=0 & FFFFF002 \\
FFFFF002 & 31 C0 & \code{xor ax,ax} & AX=0 & FFFFF004 \\
FFFFF004 & 8E D8 & \code{mov ds,ax} & DS base=0 & FFFFF006 \\
FFFFF00A & BC 00 70 & \code{mov sp,0x7000} & SP=7000 & FFFFF00D \\
\bottomrule
\end{longtable}
