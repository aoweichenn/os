...
\topic{七次写操作在源码中的顺序}
下面的清单只保留 UART 初始化过程。先写 DLAB，再写除数，最后用
\code{0x03} 同时选择 8N1 并清除 DLAB：

\lstinputlisting[
  language={[x86masm]Assembler},
  firstline=487,
  lastline=522,
  caption={COM1 初始化的七次端口写入}
...
\lstinputlisting[
  ...
  firstline=566,
  lastline=586,
  caption={等待 THRE，超时后明确返回失败}
]{../../../../source/firmware/src/reset_and_serial.asm}

\topic{字符串发送把一个问题拆成两层}
\code{os_firmware_write_string} 负责读取 NUL 结尾字符串，
\code{os_firmware_write_byte} 负责等待并写一个字节。这样任何一次 THRE
超时都会清除 CF，并逐层返回给调用者：
