\section{把操作系统学习变成可验收工程}

\subsection{学习对象不是代码量}
...
\midrule
\endhead
平台模型 & 地址译码、RAM、ROM、设备和复位行为 & 固件策略、磁盘格式、内核接口\\
固件 & 从复位环境建立最小执行环境 & 通用进程、文件系统、用户态 ABI\\
引导阶段 & 定位并校验内核，准备交接数据 & 长期设备管理与应用隔离\\
内核 & 内存、异常、中断、调度、驱动与系统调用 & 应用业务逻辑\\
用户空间 & 服务、命令和应用 & 任意访问物理内存或设备寄存器\\
\bottomrule
\end{longtable}
