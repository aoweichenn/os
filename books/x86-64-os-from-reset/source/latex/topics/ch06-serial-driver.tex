...
\subsection{初始化事务的状态轨迹}

\begin{longtable}{L{0.12\textwidth}L{0.18\textwidth}L{0.20\textwidth}L{0.34\textwidth}}
\toprule
\textbf{序号} & \textbf{端口} & \textbf{写入值} & \textbf{新建立的设备状态}\\
\midrule
\endfirsthead
\toprule
\textbf{序号} & \textbf{端口} & \textbf{写入值} & \textbf{新建立的设备状态}\\
\midrule
\endhead
1 & \code{0x3F9} & \code{0x00} & 关闭 UART 中断源\\
2 & \code{0x3FB} & \code{0x80} & DLAB 打开，偏移 0/1 指向除数\\
3 & \code{0x3F8} & \code{0x01} & 除数低字节为 1\\
4 & \code{0x3F9} & \code{0x00} & 除数高字节为 0\\
5 & \code{0x3FB} & \code{0x03} & DLAB 关闭，同时选择 8N1\\
6 & \code{0x3FA} & \code{0xC7} & 启用并清空 FIFO，设置触发级别\\
7 & \code{0x3FC} & \code{0x0B} & 设置 DTR、RTS 和 OUT2\\
\bottomrule
\end{longtable}
