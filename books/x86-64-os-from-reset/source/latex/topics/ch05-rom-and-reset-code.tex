\section{ROM 布局与复位代码走读}

\subsection{入口先建立确定状态}
...
\subsection{入口指令建立的逐项不变量}

\begin{longtable}{L{0.20\textwidth}L{0.28\textwidth}L{0.36\textwidth}}
\toprule
\textbf{指令或动作} & \textbf{建立的状态} & \textbf{缺失时的后果}\\
\midrule
\endfirsthead
\toprule
\textbf{指令或动作} & \textbf{建立的状态} & \textbf{缺失时的后果}\\
\midrule
\endhead
\code{cli} & \code{IF=0} & 外部中断可能在 IDT 建立前改变控制流\\
\code{cld} & \code{DF=0} & \code{lodsb} 可能向低地址移动并越过字符串起点\\
清零 \code{DS/ES/SS} & 低地址数据和栈采用已知段基址 & 复位残留状态进入地址计算\\
设置 \code{SP=0x7000} & 栈位于约定 RAM 范围 & \code{call} 把返回地址写入未知位置\\
初始化 UART & 建立 8N1 轮询发送协议 & 串口输出不能作为可靠证据\\
\bottomrule
\end{longtable}
