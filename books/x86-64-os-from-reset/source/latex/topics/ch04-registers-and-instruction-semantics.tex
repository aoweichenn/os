...
\subsection{通用寄存器的名称还编码访问宽度}

\begin{longtable}{L{0.18\linewidth}L{0.18\linewidth}L{0.18\linewidth}L{0.36\linewidth}}
\toprule
64 位 & 32 位 & 16/8 位 & 常见系统职责 \\
\midrule
\endfirsthead
\toprule
64 位 & 32 位 & 16/8 位 & 常见系统职责 \\
\midrule
\endhead
\code{RAX} & \code{EAX} & \code{AX/AL/AH} & 返回值、累加器、系统调用号 \\
\code{RBX} & \code{EBX} & \code{BX/BL/BH} & 被调用者保存状态或普通基址 \\
\code{RCX} & \code{ECX} & \code{CX/CL/CH} & 计数；SYSCALL 自动保存用户 RIP \\
\code{RDX} & \code{EDX} & \code{DX/DL/DH} & 扩展算术、端口地址、参数 \\
\code{RSI} & \code{ESI} & \code{SI/SIL} & 源地址或第二参数 \\
\code{RDI} & \code{EDI} & \code{DI/DIL} & 目的地址或第一参数 \\
\code{RBP} & \code{EBP} & \code{BP/BPL} & 可选帧指针或普通被调用者保存寄存器 \\
\code{RSP} & \code{ESP} & \code{SP/SPL} & 当前栈顶，控制流与异常边界的核心状态 \\
\code{R8--R15} & \code{R8D--R15D} & \code{R8W/R8B} 等 & x86-64 新增通用状态 \\
\bottomrule
\end{longtable}
...
\subsection{RIP、RSP 与 RFLAGS 不是普通三元组}
...
\section{RFLAGS 把数据通路连接到控制流}

\begin{longtable}{L{0.13\linewidth}L{0.20\linewidth}L{0.57\linewidth}}
\toprule
标志 & 名称 & 典型意义 \\
\midrule
\endfirsthead
...
\midrule
\endhead
\code{CF} & Carry & 无符号加法进位、减法借位；也被移位和旋转使用 \\
\code{PF} & Parity & 结果低字节中 1 的数量是否为偶数，属于兼容遗产 \\
\code{AF} & Auxiliary carry & 低四位进位，主要服务早期 BCD 语义 \\
\code{ZF} & Zero & 截断到操作数宽度后的结果是否为零 \\
\code{SF} & Sign & 结果最高位，只有结合操作数宽度才有意义 \\
\code{OF} & Overflow & 有符号结果是否超出当前宽度 \\
\code{DF} & Direction & 字符串指令地址递增还是递减；跨语言入口通常先 CLD \\
\code{IF} & Interrupt enable & 是否接受可屏蔽外部中断；不是“当前没有中断” \\
\code{TF} & Trap & 单步调试请求，返回用户态前必须受控 \\
\code{IOPL} & I/O privilege level & 端口 I/O 权限边界，用户返回不能任意提升 \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
数据搬运 & MOV、MOVZX、MOVSX、XCHG & 宽度、扩展规则、内存副作用 \\
算术 & ADD、SUB、ADC、SBB、MUL、DIV & 溢出、除零、隐式寄存器、RFLAGS \\
位操作 & AND、OR、XOR、TEST、BT、BTS & 掩码含义、位号、并发原子性 \\
移位旋转 & SHL、SHR、SAR、ROL、ROR & 算术/逻辑右移、计数截断、CF \\
控制流 & JMP、Jcc、CALL、RET & 下一 RIP、栈、canonical 目标 \\
字符串 & MOVS、STOS、LODS、SCAS & RSI/RDI/RCX、DF、REP、缺页位置 \\
系统状态 & CLI、STI、HLT、LGDT、LIDT & CPL、序列化要求、失败后是否可观察 \\
地址转换 & MOV CR3、INVLPG & 当前根、TLB、共享映射与 shootdown 边界 \\
端口 I/O & IN、OUT & 端口、宽度、IOPL、设备状态机 \\
原子同步 & LOCK CMPXCHG、XADD、MFENCE & 对齐、缓存一致性、内存顺序 \\
\bottomrule
\end{longtable}

\section{CALL、RET 与 ABI 共同定义函数}
...
\section{原子指令、缓存一致性与 C++ 内存顺序是三层问题}
...
\midrule
\endhead
地址 & 指令开始的线性/物理地址以及所在模式 \\
字节 & 前缀、opcode、ModR/M、SIB、位移和立即数 \\
反汇编 & Intel 语法助记符、显式和隐式操作数 \\
输入 & 被读取的寄存器、内存、端口与标志 \\
输出 & 写回的寄存器、内存、端口、RIP 与标志 \\
失败 & 可能出现的 \#UD、\#GP、\#PF、设备超时或边界拒绝 \\
\bottomrule
\end{longtable}
