...
\begin{center}
\begin{tikzpicture}[font=\footnotesize\sffamily,>=Latex]
  \node[os state] (idle) at (0,0) {Idle};
  \node[os state] (address) at (3.4,0) {Address};
  \node[os state] (wait) at (6.8,0) {Wait};
  \node[os state,minimum width=28mm] (complete) at (10.5,1.45)
    {Complete\\done 后回 Idle};
  \node[os state,minimum width=28mm] (error) at (10.5,-1.45)
    {Error\\clear 后回 Idle};
  \draw[os arrow] (idle) -- node[above]{request} (address);
  \draw[os arrow] (address) -- node[above]{发出 read} (wait);
  \draw[os arrow] (wait) -- node[above,sloped]
    {response \(\land\lnot error\)} (complete);
  \draw[os arrow] (wait) -- node[below,sloped]
    {response \(\land error\)} (error);
  \draw[os arrow,loop below,min distance=11mm]
    (wait) to node[below]{\(\lnot response\)} (wait);
\end{tikzpicture}
\end{center}
\diagramnote{主路径从左向右推进。Complete 在发出 done 后回到 Idle；Error
保持错误，直到 clear 后回到 Idle。返回动作写进节点，避免两条长回边遮住
response 条件。}
