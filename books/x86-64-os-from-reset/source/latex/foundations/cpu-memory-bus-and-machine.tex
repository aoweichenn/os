...
\input{foundations/expanded/ch04-cpu-datapath-and-instruction-cycle}
...
\begin{enumerate}[leftmargin=2.2em]
  \item CPU 给出地址和“读”方向，并在等待期间保持这些信号稳定；
  \item 地址译码器比较地址范围，只选择负责该范围的 ROM；
  \item ROM 根据低位地址找到片内字节，并开始驱动数据通路；
  \item 若数据尚未稳定，ROM 或互连保持 \code{ready} 未完成，CPU 继续等待；
  \item 数据有效后，ROM 给出完成信号，CPU 在协议规定的边沿采样；
  \item 事务结束，ROM 撤销输出使能并释放共享数据通路，下一目标才可驱动它。
\end{enumerate}

这里的“释放”不是清零数据线，而是让输出进入高阻态或由互连停止转发，避免
两个目标同时驱动相反电平。现代封装内部未必还能看到一组共享并行数据线，但
地址选择、等待完成、采样结果和结束事务这几个逻辑阶段仍然存在。
...
\input{foundations/expanded/ch04-interconnect-and-transactions}
...
\input{foundations/expanded/ch04-reset-rom-mapping-and-debug}

\topic{把教学 CPU 的取指接到项目中的 x86}
\projectconnection
前面的教学模型刻意省略了缓存、预取、乱序和复杂指令译码。回到项目时，可以
先建立下面的对应关系，再逐项补入 x86 的特殊规则：

\begin{osgridlongtable}{L{0.24\linewidth}L{0.30\linewidth}L{0.36\linewidth}}
教学模型 & x86-64 与本项目 & 仍要补充的事实 \\ \hline
\endfirsthead
教学模型 & x86-64 与本项目 & 仍要补充的事实 \\ \hline
\endhead
程序计数器 PC & 当前模式下的 IP、EIP 或 RIP & 复位时 CS 隐藏基址也参与形成取指地址 \\ \hline
保存指令的 ROM & 项目生成的 128 KiB firmware ROM & QEMU 把镜像映射到复位窗口 \\ \hline
固定的第一取指地址 & 物理地址 \code{0xFFFFFFF0} & 它位于 4 GiB 边界下方 16 字节 \\ \hline
取指、译码、执行 & x86 前端、执行单元与架构提交 & 内部可以并行和乱序，提交结果仍须符合 ISA \\ \hline
跳转到下一段程序 & 复位向量中的相对跳转 & 位移按“下一条指令地址”计算 \\ \hline
\end{osgridlongtable}

因此，CPU 能从 ROM 执行第一条指令并不依赖一句“固件会启动”的假设。复位
状态给出地址，平台译码选中 ROM，ROM 返回项目构建出的机器字节，CPU 译码
并执行其中的跳转；后面的实模式和长模式章节只是在这条链上继续增加 x86
架构状态。
...
\input{foundations/workshops/teaching-cpu-lab}
