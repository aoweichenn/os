...
- `source/latex/chapters/`：十六个完整主题章的入口，负责组织一章的学习主线。
...
当前 v2.0 教材结构修订版 PDF 为 450 页、5198356 字节，SHA-256 为
`bbe801cf23b0a98b50461fbf552f41fbf28bdf9bdb34ace60200872bd90fc0c0`。
手机书库和网站下载件必须与书稿产物逐字节同哈希，不能把旧版 PDF 与新版
源码组合发布。
...
当前书稿为 6 部 16 个完整主题章。当前生产目标包含 208 个核心
...
六部分依次回答“机器为什么能开始工作、源码怎样成为硬件行为、内核怎样取得
并保护机器、多个程序怎样共享执行和对象、状态怎样等待并跨重启存活、怎样证明
这些内容属于同一 v2.0”。原先集中在单章中的进程、VFS、虚拟内存、持久化、
并发和用户环境已经按所有权与前置依赖拆为第 10--15 章；第 16 章只负责工程
治理与发布证据，不再混入新的 Kernel 机制。完整编辑边界、章节输入输出和
旧结构迁移关系见 `STRUCTURE.md`。
...
前三章另加入一个完整实体硬件学习案例：LattePanda Mu N100/N305 计算模块与
...
环形管道、持久文件系统、统一 fd、idle 和交互式 Shell。第十六章进一步冻结
...
第八章已同步 v1.1 可回收内核堆，完整解释 boundary tag、best-fit、对齐
...
v2.0 不再加入 Kernel 机制，而是在第十六章把项目/ABI/盘面版本分离、三档机器、
