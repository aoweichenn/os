...
当前书稿为 5 部 10 个完整主题章、188 页。当前生产目标包含 103 个核心
`.asm`、`.cpp`、`.hpp` 文件，共 18,632 行有效代码；该口径严格排除测试、
...
映射失败事务回滚、进程递归销毁与十万步独立层级模型。完整回归现为 92 项
CTest；动态栈与页表回收的单元、循环集成、十万步随机模型和正常/用户故障
QEMU 路径共同证明活动资源最终归零，并有意保留仍被进程根借用的共享 PDPT。
