...
当前书稿为 5 部 10 个完整主题章、194 页。当前生产目标包含 109 个核心
`.asm`、`.cpp`、`.hpp` 文件，共 19,549 行有效代码；该口径严格排除测试、
...
资源快照和所有权守恒方程。完整回归现为 97 项 CTest；动态栈、页表回收与资源
快照的单元、循环集成、十万步随机模型、256 MiB functional 和 64 GiB
capacity QEMU 路径共同证明活动资源最终归零，并有意保留仍被进程根借用的
共享 PDPT。
