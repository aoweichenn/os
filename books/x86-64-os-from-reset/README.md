...
当前书稿为 5 部 10 个完整主题章、184 页。当前生产目标包含 103 个核心
`.asm`、`.cpp`、`.hpp` 文件，共 18,016 行有效代码；该口径严格排除测试、
...
以及为何必须回到永久启动栈安全点后才能撤映射、清零并回收。完整回归现为
89 项 CTest，动态栈的单元、四 CR3 集成、十万步随机模型和正常/用户故障
QEMU 路径共同证明活动资源最终归零。
