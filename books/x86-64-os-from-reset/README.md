...
当前书稿为 5 部 10 个完整主题章。当前生产目标包含 170 个核心
`.asm`、`.cpp`、`.hpp` 文件，共 42,684 行有效代码；该口径严格排除测试、
...
当前实现内容与 v1.10 对齐：中断/设备/用户边界章包含用户 ELF、
...
正常 Kernel 不再内嵌普通用户程序；十八个合法用户 ELF 逐个审计后由离线
