...
可延迟销毁的动态对象。本 ADR 落地时记录的空中间页表限制已由
[ADR 0026](0026-owned-page-table-branch-reclamation.md) 消除。
...
本 ADR 落地时页表层尚不回收变空的中间表，启动 KVA 自检曾先暖机同一条
KVA 路径以隔离该差额。ADR 0026 落地后，栈最后一个叶撤销会回收空 PT 与
PD；内核共享根只保留仍可能被进程 PML4 引用的 PDPT。创建失败时
`MapPage` 自身也会回滚本事务新建的表和父项权限。
...
- 本 ADR 三项测试加入后完整 CTest 集合当时为 89 项；页表回收增量完成后
  当前为 92 项，Kernel ELF 仍通过 freestanding 符号、段权限和无未解析
  运行时依赖审计。
...
- 管理槽和 KVA 描述符仍由固定 BSS 数组提供，v1.2 的动态 Thread 容量会把
  元数据迁移到类型化对象层；
- 本 ADR 落地时空中间页表尚不回收；该历史限制已由 ADR 0026 消除。当前
  保留的共享 PDPT 是跨进程 PML4 引用所要求的稳定所有权边界。
