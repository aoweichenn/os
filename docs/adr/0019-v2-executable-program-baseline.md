...
v1.11 已交付 19 个能代表 I/O 组合面的 multi-call 核心工具路径；v1.18
只做用户空间收口和补充，不得以增加工具为由引入新的内核机制。
...
主项目、网站和托管版本拥有不同身份。每个小版本都是一次公开发布，必须同步
网站并记录：
...
只有这些证据指向同一次内容同步，且公网已经验证当前发布文档、新增代码与
教材，项目和网站才可以把版本标为已发布。不得先结束小版本，再把网站更新
延期到下一阶段。
