...
按根所有权回收取代。v1.2 又因 512 个动态 Thread 栈把描述符存储从 256 项
提升到 1024 项；该容量演进记录于
[ADR 0029](0029-process-thread-waitqueue-fxsave.md)，区间协议与算法不变。
...
`Unused + 0 + 0`。本 ADR 落地时内核在 BSS 中提供 256 项，即 6144 字节；
当前 v1.2 提供 1024 项，即 24576 字节。描述符容量
...
操作，也不存在空闲链断裂。代价是申请需要扫描至多“调用方配置容量”个描述符
（v1.1 为 256，v1.2 为 1024），插入和删除需要移动数组尾部，复杂度为
\(O(N)\)。对当前有界串行阶段，这比引入树节点、
节点分配失败和并发旋转更容易审计。
...
当前 v1.2 的同名 `KVA_DESCRIPTOR_CAPACITY` 日志为 1024；其余窗口、权限和
自检事务语义保持不变。
...
- 32 TiB 窗口在 v1.1 只消耗 6 KiB、当前 v1.2 只消耗 24 KiB 固定描述符，
  而不是 GiB 级逐页位图；
...
- 本 ADR 初始最多保存 256 个分配或保留描述符；当前 v1.2 上限为 1024；
