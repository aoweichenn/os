...
- 接管固件建立的 VGA 光标与内存日志并输出 `[OS][STAGE1] ENTERED`。
...
- 在 16、32、64 位三种模式下直接写 VGA 文本显存并输出阶段标记。
...
| `0x00020000..0x0009FFFF` | VGA 共享光标、输出模式与只追加内存日志 |
| `0x000B8000..0x000B8F9F` | 当前可见的 80×25 VGA 文本页 |
