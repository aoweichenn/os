...
| `0x00100000..0x03DFFFFF` | `PT_LOAD` 目标窗口 |
| `0x03E00000..0x03EFFFFF` | Kernel ELF 暂存区，最大 1 MiB |
...
暂存区位于 62--63 MiB，而不是传统 VGA 孔洞前的低端 RAM。Stage 1 在读取
Kernel 前遍历已经规范化的 E820 图，要求完整的 1 MiB 半开区间落在同一个
usable 条目中；不能仅因 QEMU 使用 `-m 64` 就假定该地址可写。`PT_LOAD`
目标上界同步收紧到 `0x03E00000`（不含），从格式上禁止复制目标覆盖仍作为
源文件使用的暂存区。
