...
| v2.2 | 终端、Shell 与本地命令环境 | 开发中 |
| v2.3 | 使用完整 128 GiB 的 rootfs v4 与可靠持久化 | 未开始 |
...
- `/bin/err` 提供确定性的 stderr 组合验证，rootfs 工具路径增至 33 个。
...
在固定参考机上实现页/缓存回收、水位、swap、overcommit 策略、OOM 选择与
宿主 RSS 保护；验证耗尽、回滚、缓存收缩、交换损坏和长时间压力。SMP 和
NUMA 不进入本版。
