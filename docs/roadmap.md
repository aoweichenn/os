...
v2.19 的 variable dirent/HTree、inode extension、xattr/ACL/quota 独立模型已完成；v2.20 已把
mkfs/installer、完整 fsck、copy migration、journal/extent/dirent backend 和生产启动根切到 v5。
v4 只保留回归夹具，V2 阶段实现范围完成；公开发布仍以完整 CAW 矩阵和交付证据为准。

每个阶段先在独立格式模型和小几何实验后端闭环；v2.20 起整机生产回归使用 rootfs v5。
V2 不追求 Linux ext4 盘面兼容，也不加入网络、GUI、更多设备、快照、压缩、加密或多设备。
范围由 [ADR 0079](adr/0079-v2-mini-ext4-rootfs-v5-program.md) 冻结。
生产切换由 [ADR 0085](adr/0085-v2-20-rootfs-v5-production-switch.md) 记录。
