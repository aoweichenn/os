...
当前已完成 `v0.8 用户边界`：Stage 1 在自研长模式环境中通过 ATA PIO 读取
...
用户 `#UD` 和 `#PF` 只结束当前用户执行并恢复内核调用链，Ring 0 异常仍
panic。正常、非法指针、未知系统调用、用户非法指令、用户页故障和截断 ELF
均有 QEMU 证据。下一阶段 `v0.9` 将在这条可信边界上增加进程状态、上下文
切换、抢占与资源回收。
