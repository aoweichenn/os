...
生命周期，下一阶段为 v1.2 Process/Thread 与等待模型。Stage 1 在自研长模式
环境中通过 ATA PIO 读取
...
pwd、ls、mkdir、write、cat、sync 和 exit。当前完整回归为 97 项 CTest，
...
构成阶段证据。正常启动累计动态栈创建/销毁各五次，其中一次属于资源自检，
四次属于进程，峰值仍为四栈。v1.1 明确保留当前四 PCB 用户路径，直到 v1.2
的 Process/Thread 模型通过对等测试后再迁移删除。v2 路线按
