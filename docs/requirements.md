...
当前已完成 `v0.9 进程调度`：Stage 1 在自研长模式环境中通过 ATA PIO 读取
...
59 项 CTest 覆盖调度纯模型、模块集成、4096 轮固定种子随机性质、四个用户
ELF 审计、QEMU 真实 PIT 抢占、旧失败路径和资源回收。下一阶段 `v0.10`
将引入阻塞/唤醒、等待队列、同步原语与 IPC。
