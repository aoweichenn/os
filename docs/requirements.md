...
当前已完成 `v0.5 内核基础`：Stage 1 在自研长模式环境中通过 ATA PIO 读取
...
但 ELF 语义损坏路径均有 QEMU TCG 回归。v0.5 另外用真实 `INT3`、`UD2`
和页故障验证可恢复异常、无错误码异常、带错误码异常、CR2 与 panic。
下一阶段 `v0.6` 将规范化物理内存图，实现物理页分配、内核页表和最小堆。
